...
import { ZYRA_PAGE_SIZE } from 'src/logic-functions/constants/zyra-page-size';
...
export const convergeDivergedCallRecordings = async ({
  ...
  for (const candidate of candidates) {
    if (isOutsideConvergenceBound(candidate, convergenceLowerBound)) {
      console.warn(
        `[zyra-meeting-bot] call recording ${candidate.id} diverged but its meeting ended more than ${CONVERGENCE_LOOKBACK_DAYS} days ago; it will not converge automatically`,
      );
    ...
    if (isUndefined(candidate.externalBotId)) {
      console.warn(
        `[zyra-meeting-bot] call recording ${candidate.id} diverged but has no Recall bot id; it will not converge automatically`,
      );
      result.unconvergeableCallRecordingIds.push(candidate.id);
      continue;
    }
...
const convergeCallRecording = async ({
  ...
  if (!botResult.ok) {
    ...
    console.warn(
      `[zyra-meeting-bot] failed to fetch Recall bot ${externalBotId} for call recording ${candidate.id}: ${botResult.errorMessage}`,
    );
...
const markCallRecordingFailedAfterBotLoss = async ({
  ...
  console.warn(
    `[zyra-meeting-bot] Recall bot ${externalBotId} for call recording ${candidate.id} no longer exists; it will not converge automatically`,
  );
