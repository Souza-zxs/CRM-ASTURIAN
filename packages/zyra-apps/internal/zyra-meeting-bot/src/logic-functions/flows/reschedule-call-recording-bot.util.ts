...
export const rescheduleCallRecordingBot = async (
  ...
  if (isUndefined(workspaceId)) {
    console.warn(
      `[zyra-meeting-bot] cannot reschedule Recall bot for callRecording ${callRecording.id}: workspace id unavailable`,
    );
  ...
  console.warn(
    `[zyra-meeting-bot] failed to update Recall bot for callRecording ${callRecording.id}: ${rescheduleResult.errorMessage}`,
  );
};
