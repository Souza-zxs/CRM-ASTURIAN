...
export const reconcileCallRecordingTranscriptArtifact = async ({
  ...
  if (!listResult.ok) {
    console.warn(
      `[zyra-meeting-bot] failed to list Recall transcripts for recording ${externalRecordingId}: ${listResult.errorMessage}`,
    );
  ...
  if (
    ...
    if (!createResult.ok) {
      console.warn(
        `[zyra-meeting-bot] failed to request transcript for Recall recording ${externalRecordingId}: ${createResult.errorMessage}`,
      );
  ...
  if (downloadResult.outcome === 'error') {
    console.warn(
      `[zyra-meeting-bot] could not fill transcript for call recording ${callRecordingId}: ${downloadResult.errorMessage}`,
    );
  }
