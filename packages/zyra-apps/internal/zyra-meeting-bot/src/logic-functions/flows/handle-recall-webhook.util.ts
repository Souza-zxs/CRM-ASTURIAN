...
const buildMediaIngestionUpdate = async ({
  ...
  if (isUndefined(externalRecordingId)) {
    console.warn(
      `[zyra-meeting-bot] cannot ingest media for call recording ${callRecording.id}: no Recall recording id available`,
    );
...
const buildTranscriptArtifactUpdate = async ({
  ...
  if (isUndefined(externalRecordingId)) {
    console.warn(
      `[zyra-meeting-bot] cannot reconcile transcript for call recording ${callRecording.id}: no Recall recording id available`,
    );
...
const fetchExternalRecordingIdFromRecallBot = async (
  ...
  if (!botResult.ok) {
    console.warn(
      `[zyra-meeting-bot] failed to fetch Recall bot ${externalBotId} while resolving a recording id: ${botResult.errorMessage}`,
    );
...
const handleRecallTranscriptEvent = async ({
  ...
  switch (downloadResult.outcome) {
    ...
    case 'error': {
      ...
      console.warn(
        `[zyra-meeting-bot] could not fill transcript for call recording ${callRecording.id}: ${reason}`,
      );
...
const applyTranscriptFailure = async ({
  ...
  console.warn(
    `[zyra-meeting-bot] transcript failed for call recording ${callRecording.id}${isNull(subCode) ? '' : ` (${subCode})`}`,
  );
