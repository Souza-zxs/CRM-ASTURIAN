...
export const reapOrphanedMeetingBots = async ({
  ...
  if (!listResult.ok) {
    console.warn(
      `[zyra-meeting-bot] failed to list Recall bots for orphan reaping: ${listResult.errorMessage}`,
    );
  ...
  if (isUndefined(currentWorkspaceId)) {
    console.warn(
      '[zyra-meeting-bot] cannot reap orphaned Recall bots: workspace id unavailable',
    );
  ...
  for (const bot of workspaceManagedBots) {
    ...
    console.warn(
      `[zyra-meeting-bot] canceling orphaned Recall bot ${bot.id} (claimed callRecording: ${claimedCallRecordingId})`,
    );
...
const getClaimedCallRecordingId = (
  ...
  const claimedCallRecordingId = bot.metadata.zyraCallRecordingId;
...
const getClaimedWorkspaceId = (
  ...
  const claimedWorkspaceId = bot.metadata.zyraWorkspaceId;
...
const cancelOrEjectRecallBot = async (
  ...
  console.warn(
    `[zyra-meeting-bot] failed to cancel orphaned Recall bot ${externalBotId}: ${cancelResult.errorMessage}`,
  );
