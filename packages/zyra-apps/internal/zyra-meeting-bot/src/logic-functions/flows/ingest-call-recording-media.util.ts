...
export const ingestCallRecordingMedia = async ({
  ...
  if (!recordingResult.ok) {
    console.warn(
      `[zyra-meeting-bot] failed to fetch Recall recording ${externalRecordingId} while ingesting media for call recording ${callRecordingId}: ${recordingResult.errorMessage}`,
    );
...
const ingestMediaArtifact = async ({
  ...
  try {
    ...
    console.warn(
      `[zyra-meeting-bot] failed to ingest ${fileName} for call recording ${callRecordingId}: ${error instanceof Error ? error.message : String(error)}`,
    );
