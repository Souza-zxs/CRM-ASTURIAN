...
import { chargeCredits } from 'zyra-sdk/billing';
...
export const chargeCompletedCallRecording = async ({
  ...
  if (isUndefined(charge)) {
    console.warn(
      `[zyra-meeting-bot] call recording ${callRecordingId} completed without usable recording timestamps; it will not be billed`,
    );
