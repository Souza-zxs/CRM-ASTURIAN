...
const buildBot = ({
  ...
  id,
  metadata: {
    ...(zyraCallRecordingId === undefined ? {} : { zyraCallRecordingId }),
    ...(zyraWorkspaceId === undefined ? {} : { zyraWorkspaceId }),
  },
});
...
const buildCurrentWorkspaceBot = ({
  ...
  buildBot({
    id,
    zyraCallRecordingId,
    zyraWorkspaceId: CURRENT_WORKSPACE_ID,
  });
    ...
    listScheduledRecallBotsMock.mockResolvedValue({
      ...
      bots: [
        buildCurrentWorkspaceBot({
          id: 'claimed-bot',
          zyraCallRecordingId: 'call-recording-1',
        }),
      ],
    });
  ...
  it('cancels bots whose call recording no longer exists', async () => {
    listScheduledRecallBotsMock.mockResolvedValue({
      ...
      bots: [
        buildCurrentWorkspaceBot({
          id: 'orphan-bot',
          zyraCallRecordingId: 'call-recording-gone',
        }),
      ],
    });
    ...
    listScheduledRecallBotsMock.mockResolvedValue({
      ...
      bots: [
        buildBot({
          ...
          zyraCallRecordingId: 'call-recording-gone',
          zyraWorkspaceId: OTHER_WORKSPACE_ID,
        }),
      ],
    });
