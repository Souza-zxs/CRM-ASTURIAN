...
export const ensureMeetingBot = async (
  ...
  if (isUndefined(workspaceId)) {
    console.error(
      `[zyra-meeting-bot] cannot schedule Recall bot for callRecording ${callRecording.id}: workspace id unavailable, the shared webhook could not be routed back`,
    );
  ...
  if (!scheduleResult.ok) {
    console.warn(
      `[zyra-meeting-bot] failed to schedule Recall bot for callRecording ${callRecording.id}: ${scheduleResult.errorMessage}`,
    );
