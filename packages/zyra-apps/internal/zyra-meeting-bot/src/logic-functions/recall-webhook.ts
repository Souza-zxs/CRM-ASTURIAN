...
import { Response } from 'zyra-sdk/logic-function';
...
const rejectWebhook = (status: number, error: string): Response => {
  console.error(`[zyra-meeting-bot] webhook rejected: ${error}`);
...
export const recallWebhookRouteHandler = async (
  ...
  if (!isNonEmptyString(webhookSecret)) {
    return rejectWebhook(
      500,
      'RECALL_WEBHOOK_SECRET server variable is not set. A server admin must copy it from the Recall webhook endpoint settings and set it on the Zyra Meeting Bot application registration.',
    );
  }
...
const recallWebhookLogicFunctionConfig = {
  ...
  serverWebhookTriggerSettings: {
    workspaceIdResolver: {
      source: 'body',
      path: 'data.bot.metadata.zyraWorkspaceId',
    },
