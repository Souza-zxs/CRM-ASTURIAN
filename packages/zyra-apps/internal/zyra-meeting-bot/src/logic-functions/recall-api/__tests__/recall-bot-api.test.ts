...
describe('recall bot api', () => {
  ...
  it('creates Recall bot requests with the Token authorization scheme', async () => {
    const result = await scheduleRecallBot({
      ...
      metadata: {
        zyraWorkspaceId: WORKSPACE_ID,
        zyraCallRecordingId: 'call-recording-id',
        zyraCalendarEventId: 'calendar-event-id',
        zyraRealMeetingKey: 'meeting-key',
      },
    });
    ...
    fetchMock
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          results: [
            { id: 'bot-1', metadata: { zyraCallRecordingId: 'recording-1' } },
          ],
        }),
      })
      .mockResolvedValueOnce({
  ...
  it('adds call recording metadata when convergence creates an async transcript', async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual({
      provider: { recallai_async: { language_code: 'auto' } },
      diarization: { use_separate_streams_when_available: true },
      metadata: { zyraCallRecordingId: 'call-recording-id' },
    });
  });
