...
Slack tools for **Zyra workflows** and **agents** (the same logic functions
...
Once the app is installed and Slack is **connected** (see **Zyra setup**
below):
...
- **Agents / AI** — when your Zyra instance surfaces app tools to the model,
...
Both routes require an authenticated Zyra user and use the same shared Slack connection as the workflow tools.
...
1. Create a Slack app at [api.slack.com/apps](https://api.slack.com/apps)
   (dedicated to this Zyra app — do not reuse for other Zyra apps).
2. **OAuth & Permissions** → **Bot Token Scopes**. Zyra uses Slack’s **bot**
...
3. Set the **Redirect URL** on the Slack app to
   `<YOUR_ZYRA_SERVER_URL>/auth/apps/callback` — the same origin your
   Zyra **server** uses for API routes (the callback is not served by the SPA
   alone). Local monorepo dev often uses `http://localhost:3000` (confirm the
   port your `zyra-server` / `SERVER_URL` actually uses).
   ...
   **Slack “PKCE” app setting vs `localhost`:** If you turn on **PKCE** for the
   ...
   register that URL in the Slack app, and point Zyra’s `SERVER_URL` at the
   same public base URL. See Slack’s [Using
   PKCE](https://docs.slack.dev/authentication/using-pkce) docs (this is
   separate from Zyra sending a PKCE challenge on the authorize request).

4. Copy the Slack **Client ID** and **Client Secret**.

## Zyra setup

1. Register / install this app on your Zyra server (`zyra-slack`).
2. In **Settings → Applications → Zyra Slack**, open the **Application registration**
...
```bash
cd packages/zyra-apps/internal/zyra-slack
yarn install
yarn lint
yarn test
```

Use `yarn zyra dev` from this directory to develop against a local Zyra
