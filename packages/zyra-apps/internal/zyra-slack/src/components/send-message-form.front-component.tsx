...
import { closeSidePanel, enqueueSnackbar, unmountFrontComponent } from 'zyra-sdk/front-component';

import { SEND_MESSAGE_FORM_FRONT_COMPONENT_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';

// Workaround: 'zyra-sdk/ui' currently fails typecheck because it re-exports
// from the unresolvable 'zyra-ui-deprecated'. Inline only the theme tokens
// this component uses, keeping the same runtime CSS-variable values. Revert to
// `import { themeCssVariables } from 'zyra-sdk/ui'` once the SDK export is fixed.
...
const callAppRoute = async <TResponse,>(
  ...
  const apiBaseUrl = process.env.ZYRA_API_URL;
  const token =
    process.env.ZYRA_APP_ACCESS_TOKEN ?? process.env.ZYRA_API_KEY;
