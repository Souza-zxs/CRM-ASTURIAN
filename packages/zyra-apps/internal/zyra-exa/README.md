...
Exposes [Exa](https://exa.ai) structured web search to Zyra AI agents
(chat + workflow agents + MCP) as the `app_exa_web_search` tool.

## Installation

1. Register the app on the Zyra server once (admin API / UI):
   `zyra-exa` from npm.
...
The handler calls Zyra's generic app billing endpoint
