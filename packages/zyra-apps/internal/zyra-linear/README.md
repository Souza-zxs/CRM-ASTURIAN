...
Connect your Linear account to Zyra to create issues and look up teams
...
> notice on the Connections tab, ask your Zyra admin to follow the
...
### 2. Wire the credentials into Zyra
...
```bash
cd packages/zyra-apps/internal/zyra-linear
...
yarn zyra app:publish --private
yarn zyra app:install
```

`zyra dev` is recommended for iteration — it publishes, installs, and
watches for changes in one command. Use `zyra app:publish --private` +
`zyra app:install` when you want to control each step separately (e.g.
deploying to a production server without auto-installing).

This serves as the reference implementation for Zyra's
