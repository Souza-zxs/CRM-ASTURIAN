...
describe('toEmailsField', () => {
  it('returns a plain email unchanged', () => {
    expect(toEmailsField('thomas@mail.zyra.com')).toEqual({
      primaryEmail: 'thomas@mail.zyra.com',
      additionalEmails: null,
    });
  });

  it('extracts the email from "Name <email>" format', () => {
    expect(toEmailsField('Thomas <thomas@mail.zyra.com>')).toEqual({
      primaryEmail: 'thomas@mail.zyra.com',
      additionalEmails: null,
    });
  });
  ...
  it('extracts the email from a quoted display name', () => {
    expect(toEmailsField('"Last, First" <thomas@mail.zyra.com>')).toEqual({
      primaryEmail: 'thomas@mail.zyra.com',
      additionalEmails: null,
    });
  });
