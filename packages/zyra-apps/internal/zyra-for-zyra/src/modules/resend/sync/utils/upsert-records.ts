...
import { fetchExistingZyraIdsByResendIds } from '@modules/resend/sync/utils/fetch-existing-zyra-ids';
...
export type UpsertRecordsPageOutcome = {
  result: SyncResult;
  ok: boolean;
  zyraIdByResendId: Map<string, string>;
};
...
>(
  ...
  const zyraIdByResendId = await fetchExistingZyraIdsByResendIds(
    client,
    objectNamePlural,
    resendIds,
  );
  ...
  for (const item of items) {
    ...
    try {
      const isNew = !zyraIdByResendId.has(resendId);
