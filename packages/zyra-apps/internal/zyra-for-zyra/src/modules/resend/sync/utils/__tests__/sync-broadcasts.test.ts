...
describe('syncBroadcasts', () => {
  ...
  it('captures html, text, and topicId from the broadcast detail', async () => {
    ...
    mockUpsertRecords.mockResolvedValue({
      ...
      ok: true,
      zyraIdByResendId: new Map([['broadcast-1', 'zyra-broadcast-1']]),
    });

    mockFindZyraIdsByResendId.mockImplementation(
      async (_client: unknown, plural: string) => {
        if (plural === 'resendSegments') {
          return new Map([['seg-1', 'zyra-seg-1']]);
        }

        if (plural === 'resendTopics') {
          return new Map([['topic-1', 'zyra-topic-1']]);
        }
    ...
    expect(createDto).toMatchObject({
      htmlBody: '<p>Hello!</p>',
      textBody: 'Hello!',
      segmentId: 'zyra-seg-1',
      topicId: 'zyra-topic-1',
      subject: 'hello world',
