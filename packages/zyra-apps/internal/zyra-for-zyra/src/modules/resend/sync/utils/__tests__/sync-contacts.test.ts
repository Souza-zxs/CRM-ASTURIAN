...
describe('syncContacts', () => {
  beforeEach(() => {
    ...
    mockFindZyraIdsByResendId.mockReset();
  ...
  it('backfills resendEmails with each upserted contact, including personId when known', async () => {
    ...
    mockUpsertRecords.mockResolvedValue({
      ...
      ok: true,
      zyraIdByResendId: new Map([
        ['contact-1', 'zyra-contact-1'],
        ['contact-2', 'zyra-contact-2'],
      ]),
    });
    ...
    expect(entriesByEmail).toEqual(
      new Map([
        [
          'matched@example.com',
          { contactId: 'zyra-contact-1', personId: 'person-1' },
        ],
        ['unmatched@example.com', { contactId: 'zyra-contact-2' }],
      ]),
    );
  });

  it("inlines the first segment's Zyra id into the upsert payload when the contact has segments", async () => {
    const pageContacts = [
      {
        id: 'contact-1',
    ...
    expect(mockFindZyraIdsByResendId).toHaveBeenCalledTimes(1);
    expect(mockFindZyraIdsByResendId.mock.calls[0][1]).toBe('resendSegments');
    expect(
      Array.from(mockFindZyraIdsByResendId.mock.calls[0][2]).sort(),
    ...
    expect(segmented.segmentId).toBe('zyra-segment-1');
    expect(unsegmented.segmentId).toBeUndefined();
    expect(orphan.segmentId).toBeUndefined();
  });

  it('does not call findZyraIdsByResendId when no contacts have segments', async () => {
    const pageContacts = [
      {
        id: 'contact-1',
  ...
  it('skips backfill entries for contacts whose upsert did not return a Zyra id', async () => {
    const pageContacts = [
      {
        id: 'contact-1',
        email: 'one@example.com',
    ...
    mockUpsertRecords.mockResolvedValue({
      ...
      ok: true,
      zyraIdByResendId: new Map([['contact-1', 'zyra-contact-1']]),
    });
    ...
    expect(entriesByEmail).toEqual(
      new Map([['one@example.com', { contactId: 'zyra-contact-1' }]]),
    );
  });
});
