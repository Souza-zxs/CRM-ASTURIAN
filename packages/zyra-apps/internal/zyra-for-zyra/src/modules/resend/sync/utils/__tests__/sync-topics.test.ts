...
describe('syncTopics', () => {
  ...
  it('upserts topics with the correct DTO and returns the resend->zyra id map', async () => {
    const resend = buildResend([
      {
        id: 'resend-topic-1',
    ...
    mockUpsertRecords.mockResolvedValue({
      ...
      ok: true,
      zyraIdByResendId: new Map([['resend-topic-1', 'zyra-topic-1']]),
    });

    const { value } = await syncTopics(resend, client, SYNCED_AT);

    expect(value.get('resend-topic-1')).toBe('zyra-topic-1');
