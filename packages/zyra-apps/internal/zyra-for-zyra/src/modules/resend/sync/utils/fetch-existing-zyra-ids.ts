...
import { ZYRA_PAGE_SIZE } from '@modules/resend/constants/sync-config';
...
export const fetchExistingZyraIdsByResendIds = async (
  ...
  const result = await client.query({
    [objectNamePlural]: {
      __args: {
        ...
        first: Math.max(resendIds.length, ZYRA_PAGE_SIZE),
      },
      edges: {
        node: {
          id: true,
