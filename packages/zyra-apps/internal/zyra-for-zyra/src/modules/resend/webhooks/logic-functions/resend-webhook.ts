...
import { defineLogicFunction, type RoutePayload } from 'zyra-sdk/define';
...
export const handleContactCreatedOrUpdated = async (
  ...
  if (isDefined(firstSegmentResendId)) {
    const segmentIdMap = await findZyraIdsByResendId(
      client,
      'resendSegments',
  ...
  if (isDefined(existingId)) {
    ...
    return { action: 'updated', zyraId: existingId, resendId: data.id, personId };
  }
  ...
  return {
    action: 'created',
    zyraId: createResult.createResendContact?.id,
    resendId: data.id,
    personId,
  };
};
...
const handleContactDeleted = async (
  ...
  if (!isDefined(existingId)) {
    return { skipped: true, reason: 'contact not found in Zyra' };
  }
  ...
  return { action: 'deleted', zyraId: existingId, resendId: data.id };
};
...
const handleEmailEvent = async (
  ...
  if (!isDefined(existingId)) {
    return {
      skipped: true,
      reason: `email ${data.email_id} not found in Zyra`,
    };
  }
