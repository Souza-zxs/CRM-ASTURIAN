...
A Zyra app that turns the CRM into the operating system for the Zyra partner program:
intake partner-eligible deals, match them to vetted marketplace partners, and track the
matching pipeline end-to-end.

Built on [Zyra](https://zyra.com) with [`zyra-sdk`](https://www.npmjs.com/package/zyra-sdk) v2.5.
...
- **Roles** (`src/roles/`)
  - **Zyra Partner Ops** — internal team role, full CRUD on Partner/Company/Person/Opportunity.
  - **Partner** — placeholder external-partner role. *Do not assign until Zyra ships
...
Requires a local Zyra server at `http://localhost:2020` and Node `^24.5`.
...
Run `yarn zyra help` for the full CLI reference.

## Common commands

| Command | What it does |
| --- | --- |
| `yarn zyra dev` | Start the dev server and sync the app on file changes |
| `yarn zyra server status` | Check the local Zyra server |
...
- [`zyra-sdk` on npm](https://www.npmjs.com/package/zyra-sdk)
