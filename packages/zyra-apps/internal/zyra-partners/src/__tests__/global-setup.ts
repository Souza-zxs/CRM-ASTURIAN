...
import { appDevOnce, appUninstall } from 'zyra-sdk/cli';

const APP_PATH = process.cwd();
const CONFIG_DIR = path.join(os.homedir(), '.zyra');

function validateEnv(): { apiUrl: string; apiKey: string } {
  const apiUrl = process.env.ZYRA_API_URL;
  const apiKey = process.env.ZYRA_API_KEY;

  if (!apiUrl || !apiKey) {
    throw new Error(
      'ZYRA_API_URL and ZYRA_API_KEY must be set.\n' +
        'Start a local server: yarn zyra server start\n' +
...
async function checkServer(apiUrl: string) {
  ...
  try {
    ...
    throw new Error(
      `Zyra server is not reachable at ${apiUrl}. ` +
