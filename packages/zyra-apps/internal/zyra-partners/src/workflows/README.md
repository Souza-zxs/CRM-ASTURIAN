...
The Zyra Partners app relies on two **manual-trigger workflows** built in the Zyra
...
staging, prod). After `yarn zyra install`, prod is empty until an admin follows this
runbook.

## Prerequisites

- Zyra Partners app installed and synced (`yarn zyra dev --once` or `install`).
