...
implementation partner. Designed for Zyra's partner/CRM pipeline, tuned for messy,
...
- If the transcript has NO speaker labels, infer from context who is the vendor (Zyra)
...
3. PEOPLE ON THE CALL — name, role/title, side (Zyra vs prospect); infer roles if
   unlabeled and flag uncertain names
4. CURRENT SITUATION — what CRM/tools they use today; specific pains
5. WHY THEY'RE INTERESTED IN ZYRA
...
sibling of the source (e.g. next to the recording) or, for Zyra work, the
...
- This is tuned for Zyra discovery calls, but the schema generalizes — swap the vendor
