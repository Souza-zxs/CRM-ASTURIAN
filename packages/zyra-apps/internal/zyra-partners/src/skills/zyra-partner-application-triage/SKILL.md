---
name: zyra-partner-application-triage
...
trigger: /zyra-partner-application-triage
---

# zyra-partner-application-triage

Rank `APPLICATION`-stage partners by the value they would *add* — geographies and
languages we don't yet cover, plus proof of real Zyra work — and hand back a short
...
Needs `~/.zyra/credentials.env` (same file the other partner skills use):
...
The key lives in `packages/zyra-apps/internal/zyra-partners/.env.prod` (gitignored) —
copy it to `~/.zyra/credentials.env` on first setup.
...
Zyra work" proof signal in the notes, scores, and prints ranked JSON. It does not call an
...
- **Rescue the motivated-but-unobvious.** Someone whose checkboxes are thin but whose notes
  show real intent, a live Zyra instance, named customers, or a thoughtful pitch is a
