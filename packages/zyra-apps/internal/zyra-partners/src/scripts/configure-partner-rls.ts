...
async function main() {
  const baseUrl = requireEnv('ZYRA_PARTNERS_API_URL').replace(/\/$/, '');
  const apiKey = requireEnv('ZYRA_PARTNERS_API_KEY');
  ...
  if (missingLocks.length > 0) {
    console.error(
      `\n[rls:configure] DRIFT DETECTED: ${missingLocks.length} Opportunity field(s) ` +
        ...
        `  yarn zyra dev --once -r <remote>\n\n` +
        `(e.g. \`yarn zyra dev --once\` for local, ` +
        `\`yarn zyra dev --once -r partner-zyra-com\` for prod)\n`,
    );
    process.exitCode = 1;
    return;
  }
