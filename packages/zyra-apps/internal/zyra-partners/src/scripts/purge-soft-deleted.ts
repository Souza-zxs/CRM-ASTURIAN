...
//
// Zyra SOFT-deletes (sets deletedAt); the row stays in the DB and keeps holding
...
async function main() {
  const url = requireEnv('ZYRA_PARTNERS_API_URL');
  const key = requireEnv('ZYRA_PARTNERS_API_KEY');
