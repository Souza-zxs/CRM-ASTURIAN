...
Run locally: `yarn zyra dev:function:exec -n enrich-people -p '{"records":[{"id":"<id>"}]}'` (bulk)
or `yarn zyra dev:function:exec -n enrich-person -p '{"recordId":"<id>"}'` (single record).

### Billing

Each **successful match** is billed to the workspace in Zyra credits via
`chargeCredits` (`zyra-sdk/billing`), mirroring PDL's own model — PDL only consumes a
