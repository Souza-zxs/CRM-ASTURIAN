...
Connect Discord to your Zyra workflows. Post messages, edit them, react,
...
> your Zyra admin needs to follow the [Self-hosting setup](#self-hosting-setup-admin-only)
...
- **Slash commands / interactions.** The bot doesn't register or respond to
  `/commands`.
- **Per-workspace identity.** All Zyra workspaces in the same Zyra
...
1. Visit https://discord.com/developers/applications and click
   **New Application**.
2. Name it (e.g. *"Zyra Bot"*) and create. Use a dedicated app — don't
...
### 3. Wire the bot token into Zyra

1. In Zyra: **Settings → Applications → Discord → Settings tab**
   (gear icon).
...
(`defineConnectionProvider({ type: 'oauth' })`) so each Zyra workspace
...
- That bot token is global to the Discord application (and therefore to
  the Zyra deployment). Discord deprecated per-install bot tokens years
  ago.
...
```bash
cd packages/zyra-apps/internal/zyra-discord
...
`zyra dev` is recommended for iteration — it publishes to your local
Zyra server, installs the app, and watches for changes in one command.
