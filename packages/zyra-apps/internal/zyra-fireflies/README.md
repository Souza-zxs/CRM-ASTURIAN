...
the matching `CalendarEvent` in your Zyra CRM — searchable, in context, and
...
3. For each event, this app fetches the relevant data via the Fireflies
   GraphQL API.
4. It finds the matching `CalendarEvent` in Zyra and writes the content
...
1. **Provider-native event ID** — Fireflies' `calendar_id` / `cal_id` is
   ...
   events synced into Zyra from Google Calendar (including individual
...
Both identifiers are populated by Zyra's calendar drivers on every synced
...
## What gets added to your Zyra workspace
...
1. Open **Settings → Applications** in your Zyra workspace.
...
> webhook, your Zyra admin needs to follow the
...
- **Per-user Fireflies accounts.** All transcripts come through one
  ...
  connections require extending Zyra's connection provider system and are
  planned once we have evidence that workspace-shared is too coarse.
- **Editing transcripts or summaries in Zyra.** The fields are writable
...
| Webhook returns `Invalid webhook signature` | `FIREFLIES_WEBHOOK_SECRET` mismatch between Fireflies and Zyra | Re-copy the signing secret from the Fireflies webhook configuration and paste it into the Zyra app settings |
| Webhook returns `skipped: No CalendarEvent matched the transcript by external ID or iCalUid` | The meeting was never on a synced calendar in Zyra, or the workspace has no Google/Outlook/CalDAV calendar connection set up | Connect the relevant calendar provider in **Settings → Accounts** so the calendar event lands in Zyra with `eventExternalId` and `iCalUid` populated. Manually-created CalendarEvents are intentionally not matched in v1 |
...
| **List / Search** tools return `count: 0` for a contact you've definitely talked to | Email mismatch — Fireflies stores the address as the participant joined the meeting with, which may differ from the contact's primary address in Zyra (aliases, plus-addressing, work vs. personal) | Try the contact's other known email addresses; cross-check the `participants` list on a known matching call |

---

## Self-hosting setup (admin-only)

This section is for Zyra server admins. If you're on Zyra Cloud, skip
...
2. Set the **Webhook URL** to your Zyra deployment's webhook endpoint:
   `https://<your-zyra-domain>/webhook/fireflies`. Zyra resolves the
...
3. Set a **Signing Secret** (a long random string — generate one with
   `openssl rand -hex 32`). Save it; you'll paste it into Zyra next.
...
### 3. Wire the credentials into Zyra

1. In Zyra: **Settings → Applications → Fireflies → Settings tab**.
...
```bash
cd packages/zyra-apps/internal/zyra-fireflies
...
`zyra dev` is recommended for iteration — it publishes to your local Zyra
...
```bash
export FIREFLIES_WEBHOOK_SECRET='<the secret you set in Zyra app settings>'
...
computed above. Zyra resolves the workspace from the `Host` header, so
...
in your local Zyra workspace. The easiest local seed is to manually
