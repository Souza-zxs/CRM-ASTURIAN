export const ABOUT_DESCRIPTION = `Bring your Fireflies meeting recordings into Zyra. When Fireflies finishes processing a call, the transcript and AI summary land on the matching CalendarEvent automatically — no copy-pasting, no extra tabs.
...
Both identifiers are populated automatically when calendars are synced into Zyra. If a recording can't be matched (orphan recording, no calendar sync configured), the webhook reports a clear skip reason and writes nothing.
...
2. Find **Zyra Fireflies** in the available apps and click **Install**.
...
3. Configure a Webhooks V2 endpoint at [Fireflies → Integrations → Webhooks V2](https://app.fireflies.ai/integrations/api/webhook):
   - **Webhook URL**: \`https://<your-zyra-domain>/webhook/fireflies\`
...
- **Orphan calls** (recordings with no matching CalendarEvent in Zyra) are skipped — fuzzy URL matching is avoided so transcripts never land on the wrong event.
- **Per-user Fireflies accounts** — all sync goes through one workspace-shared API key set by the admin.
- **Editing transcripts in Zyra** — the field is writable in principle, but future Fireflies syncs will overwrite manual edits.
