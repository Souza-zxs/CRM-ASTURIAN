#!/bin/bash
# =============================================================================
# Zyra CRM — Development Environment Setup
# =============================================================================
...
#   bash packages/zyra-utils/setup-dev-env.sh          # start + configure
#   bash packages/zyra-utils/setup-dev-env.sh --down    # stop services
#   bash packages/zyra-utils/setup-dev-env.sh --reset   # wipe data + restart
#   bash packages/zyra-utils/setup-dev-env.sh --docker  # force Docker mode
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$REPO_ROOT/packages/zyra-docker/docker-compose.dev.yml"
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  if schema_exists; then
    ...
    if npx nx database:init zyra-server; then
  ...
  info "Run 'npx nx database:init zyra-server' to initialize the schema"
fi
