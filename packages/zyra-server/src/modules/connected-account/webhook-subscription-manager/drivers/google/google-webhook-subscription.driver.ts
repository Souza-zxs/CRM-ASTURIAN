...
import { WebhookSubscriptionChannelType } from 'zyra-shared/types';
...
export class GoogleWebhookSubscriptionDriver implements WebhookSubscriptionDriver {
  ...
  private async watchGmailMailbox(
    ...
    const pubSubTopicName = this.zyraConfigService.get(
  ...
  private async watchPrimaryCalendar(
    ...
    const notificationAddress = `${this.zyraConfigService.get('SERVER_URL')}/webhooks/google/calendar`;
