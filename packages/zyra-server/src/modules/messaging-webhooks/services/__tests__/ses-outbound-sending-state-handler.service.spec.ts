...
describe('SesOutboundSendingStateHandlerService.handle', () => {
  ...
  // the event, since every zyra-managed resource shares the same prefix.
  describe.each([
    {
  ...
  // zyra-managed name prefix; the handler must noop in that case.
  it('does not update any workspace when the ARN does not carry the zyra-managed prefix', async () => {
    const { service, emailingDomainTenantStatusService } = setUp();
