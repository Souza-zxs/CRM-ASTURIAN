...
export class MessageCampaignService {
  ...
  // Lazy, not a class-field initializer: a class field runs in the
  // constructor, which Nest invokes synchronously while resolving the
  // provider graph during NestFactory.create() — before the app has
  // finished booting. createHtmlToTextConverter() transitively
  // `new JSDOM('')`s, which requires parse5 synchronously; on Vercel's
  // serverless runtime that raced against the async parse5 preload in
  // bootstrap-app.ts (see patch-node-require-for-esm-only-modules.util.ts).
  // Deferring construction to first actual use — always well after
  // app.init() has resolved and the preload has long since completed —
  // sidesteps that race entirely instead of trying to win it.
  private cachedHtmlToText: ((html: string) => string) | null = null;

  private get htmlToText(): (html: string) => string {
    this.cachedHtmlToText ??= createHtmlToTextConverter();

    return this.cachedHtmlToText;
  }
