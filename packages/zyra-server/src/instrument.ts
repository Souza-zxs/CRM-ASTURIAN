...
if (process.env.EXCEPTION_HANDLER_DRIVER === ExceptionHandlerDriver.SENTRY) {
  Sentry.init({
    ...
    beforeSendSpan: (span) => {
      const zyraContext = Sentry.getIsolationScope().getScopeData().contexts
        ?.zyra as
        | {
            workspace_id?: string;
      ...
      if (!zyraContext?.workspace_id) {
        return span;
      }

      span.data = {
        ...span.data,
        'zyra.workspace.id': zyraContext.workspace_id,
        ...(zyraContext.user_workspace_id && {
          'zyra.user_workspace.id': zyraContext.user_workspace_id,
        }),
      };
