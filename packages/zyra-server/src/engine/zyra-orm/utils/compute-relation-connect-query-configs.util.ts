...
import { type ConnectObject } from 'src/engine/zyra-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/zyra-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/zyra-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  ZyraORMException,
  ZyraORMExceptionCode,
} from 'src/engine/zyra-orm/exceptions/zyra-orm.exception';
import { formatCompositeField } from 'src/engine/zyra-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new ZyraORMException(
      ...
      ZyraORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new ZyraORMException(
      ...
      ZyraORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
