import { type ObjectRecordUpdateEvent } from 'zyra-shared/database-events';
import { FieldMetadataType, RelationType } from 'zyra-shared/types';
...
describe('formatZyraOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw ZyraORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ZyraORMException);
        expect((error as ZyraORMException).code).toBe(
          ZyraORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as ZyraORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatZyraOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
