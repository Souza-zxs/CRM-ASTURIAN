...
describe('computeZyraORMException', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('should return an INVALID_INPUT exception with a missing-field message when error is a NOT_NULL_VIOLATION', async () => {
    ...
    const result = await computeZyraORMException(error);

    expect(result).toBeInstanceOf(ZyraORMException);
    expect((result as ZyraORMException).code).toBe(
      ZyraORMExceptionCode.INVALID_INPUT,
    );
    expect((result as ZyraORMException).userFriendlyMessage).toEqual(
  ...
  it('should preserve the original message when surfacing a constraint violation as INVALID_INPUT', async () => {
    ...
    expect((result as ZyraORMException).message).toBe(
      'null value in column "name" violates not-null constraint',
    );
  });
  ...
  it('should throw the generic PostgresException when error is a CHECK_VIOLATION (handled by the catch-all)', async () => {
    ...
    await expect(computeZyraORMException(error)).rejects.toThrow(
      PostgresException,
    );
    await expect(computeZyraORMException(error)).rejects.toMatchObject({
  ...
  it('should return a QUERY_READ_TIMEOUT exception when error message mentions a query read timeout', async () => {
    ...
    expect(result).toBeInstanceOf(ZyraORMException);
    expect((result as ZyraORMException).code).toBe(
      ZyraORMExceptionCode.QUERY_READ_TIMEOUT,
    );
  });
  ...
  it('should delegate to handleDuplicateKeyError when error is a UNIQUE_VIOLATION and metadata context is provided', async () => {
    ...
    const duplicateException = new ZyraORMException(
      'A duplicate entry was detected',
      ZyraORMExceptionCode.DUPLICATE_ENTRY_DETECTED,
    );

    handleDuplicateKeyErrorMock.mockResolvedValue(duplicateException);

    const result = await computeZyraORMException(
      error,
      objectMetadata,
      entityManager,
      internalContext,
  ...
  it('should rethrow the original error when error is a QueryFailedError with an unknown code', async () => {
    ...
    await expect(computeZyraORMException(error)).rejects.toBe(error);
  });
