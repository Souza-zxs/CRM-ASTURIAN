...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'zyra-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
} from 'src/engine/zyra-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  ZyraORMException,
  ZyraORMExceptionCode,
} from 'src/engine/zyra-orm/exceptions/zyra-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/zyra-orm/field-operations/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/zyra-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/zyra-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/zyra-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/zyra-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/zyra-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/zyra-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/zyra-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new ZyraORMException(
              errorMessage,
              ZyraORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
