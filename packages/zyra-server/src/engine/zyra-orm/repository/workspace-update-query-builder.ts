...
import { type QueryDeepPartialEntityWithNestedRelationFields } from 'src/engine/zyra-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import { type RelationConnectQueryConfig } from 'src/engine/zyra-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationDisconnectQueryFieldsByEntityIndex } from 'src/engine/zyra-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import { type WorkspaceEntityManager } from 'src/engine/zyra-orm/entity-manager/workspace-entity-manager';
import { computeZyraORMException } from 'src/engine/zyra-orm/error-handling/compute-zyra-orm-exception';
import {
  ZyraORMException,
  ZyraORMExceptionCode,
} from 'src/engine/zyra-orm/exceptions/zyra-orm.exception';
import { FilesFieldSync } from 'src/engine/zyra-orm/field-operations/files-field-sync/files-field-sync';
import { RelationNestedQueries } from 'src/engine/zyra-orm/field-operations/relation-nested-queries/relation-nested-queries';
import { validateQueryIsPermittedOrThrow } from 'src/engine/zyra-orm/repository/permissions.utils';
import { type WorkspaceDeleteQueryBuilder } from 'src/engine/zyra-orm/repository/workspace-delete-query-builder';
import { WorkspaceSelectQueryBuilder } from 'src/engine/zyra-orm/repository/workspace-select-query-builder';
import { type WorkspaceSoftDeleteQueryBuilder } from 'src/engine/zyra-orm/repository/workspace-soft-delete-query-builder';
import { applyRowLevelPermissionPredicates } from 'src/engine/zyra-orm/utils/apply-row-level-permission-predicates.util';
import { applyTableAliasOnWhereCondition } from 'src/engine/zyra-orm/utils/apply-table-alias-on-where-condition';
import { computeEventSelectQueryBuilder } from 'src/engine/zyra-orm/utils/compute-event-select-query-builder.util';
import { formatData } from 'src/engine/zyra-orm/utils/format-data.util';
import { formatResult } from 'src/engine/zyra-orm/utils/format-result.util';
import { formatZyraOrmEventToDatabaseBatchEvent } from 'src/engine/zyra-orm/utils/format-zyra-orm-event-to-database-batch-event.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/zyra-orm/utils/get-object-metadata-from-entity-target.util';
import { validateRLSPredicatesForRecords } from 'src/engine/zyra-orm/utils/validate-rls-predicates-for-records.util';
...
> extends UpdateQueryBuilder<T> {
    ...
    try {
      ...
      if (before.length > QUERY_MAX_RECORDS) {
        throw new ZyraORMException(
          ...
          ZyraORMExceptionCode.TOO_MANY_RECORDS_TO_UPDATE,
          {
      ...
      this.internalContext.eventEmitterService.emitDatabaseBatchEvent(
        formatZyraOrmEventToDatabaseBatchEvent({
      ...
      throw await computeZyraORMException(
        error,
        objectMetadata,
  ...
  override select(): WorkspaceSelectQueryBuilder<T> {
    throw new ZyraORMException(
      ...
      ZyraORMExceptionCode.METHOD_NOT_ALLOWED,
    );
  }
  ...
  private getMainAliasTarget(): EntityTarget<T> {
    ...
    if (!mainAliasTarget) {
      throw new ZyraORMException(
        ...
        ZyraORMExceptionCode.MISSING_MAIN_ALIAS_TARGET,
      );
    }
