...
import { GlobalWorkspaceDataSource } from 'src/engine/zyra-orm/global-workspace-datasource/global-workspace-datasource';
  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  async onModuleInit(): Promise<void> {
    this.globalWorkspaceDataSource = new GlobalWorkspaceDataSource(
      {
        url: this.zyraConfigService.get('PG_DATABASE_URL'),
        type: 'postgres',
        logging: this.zyraConfigService.getLoggingConfig(),
        entities: [],
        ssl: this.zyraConfigService.get('PG_SSL_ALLOW_SELF_SIGNED')
          ? {
              rejectUnauthorized: false,
            }
          : undefined,
        poolSize: this.zyraConfigService.get('PG_POOL_MAX_CONNECTIONS'),
        extra: {
          query_timeout: this.zyraConfigService.get(
            'PG_DATABASE_PRIMARY_TIMEOUT_MS',
          ),
          idleTimeoutMillis: this.zyraConfigService.get(
            'PG_POOL_IDLE_TIMEOUT_MS',
          ),
          allowExitOnIdle: this.zyraConfigService.get(
    ...
    const shouldInitializeReplicaDataSource = isDefined(
      this.zyraConfigService.get('PG_DATABASE_REPLICA_URL'),
    );

    if (shouldInitializeReplicaDataSource) {
      this.globalWorkspaceDataSourceReplica = new GlobalWorkspaceDataSource(
        {
          url: this.zyraConfigService.get('PG_DATABASE_REPLICA_URL'),
          type: 'postgres',
          logging: this.zyraConfigService.getLoggingConfig(),
          entities: [],
          ssl: this.zyraConfigService.get('PG_SSL_ALLOW_SELF_SIGNED')
