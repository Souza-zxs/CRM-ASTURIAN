...
import { buildEntitySchemaMetadataMaps } from 'src/engine/zyra-orm/global-workspace-datasource/types/entity-schema-metadata.type';
...
> {
  ...
  async computeForCache(workspaceId: string): Promise<EntityMetadata[]> {
    const [objectMetadatas, fieldMetadatas, zyraStandardApplication] =
      await Promise.all([
