...
import { ZyraStandardApplicationService } from 'src/engine/workspace-manager/zyra-standard-application/services/zyra-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly zyraStandardApplicationService: ZyraStandardApplicationService,
  ...
  public async seedDev(
    ...
    const isBillingEnabled = this.zyraConfigService.get('IS_BILLING_ENABLED');
    const appVersion = this.zyraConfigService.get('APP_VERSION') ?? 'unknown';
    ...
    const { workspaceCustomFlatApplication, zyraStandardFlatApplication } =
    ...
    await this.zyraStandardApplicationService.synchronizeZyraStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
    ...
    await this.sdkClientGenerationService.generateSdkClientForApplication({
      workspaceId,
      applicationId: zyraStandardFlatApplication.id,
    ...
    await seedPageLayouts({
      workspaceId,
      flatApplication: zyraStandardFlatApplication,
      objectMetadataItems,
  ...
  private async seedCoreSchema({
    ...
    try {
      ...
      await this.applicationService.createZyraStandardApplication(
        {
          workspaceId,
