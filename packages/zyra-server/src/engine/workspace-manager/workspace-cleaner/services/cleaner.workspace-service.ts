...
export class CleanerWorkspaceService {
  ...
  constructor(
    ...
    this.inactiveDaysBeforeSoftDelete = this.zyraConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_SOFT_DELETION',
    );
    this.inactiveDaysBeforeDelete = this.zyraConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_DELETION',
    );
    this.inactiveDaysBeforeWarn = this.zyraConfigService.get(
  ...
  async sendWarningEmail(
    ...
    const emailData = {
      ...
      appUrl: this.domainServerConfigService
        .buildBaseUrl({ pathname: 'settings/billing' })
        .toString(),
    };
    ...
    await this.emailService.send({
      ...
      from: `${this.zyraConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.zyraConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      subject,
      html,
      text,
    });
  ...
  async sendCleaningEmail(
    ...
    const emailData = {
      ...
      locale: workspaceMember.locale,
      appUrl: this.domainServerConfigService.buildBaseUrl({}).toString(),
    };
