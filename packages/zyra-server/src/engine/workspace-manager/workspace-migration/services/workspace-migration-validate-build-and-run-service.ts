...
export class WorkspaceMigrationValidateBuildAndRunService {
  ...
  > & {
    ...
    if (!isDefined(zyraStandardApplicationId) || !isDefined(applicationId)) {
    ...
    const isBuildingZyraStandardApplication =
      applicationUniversalIdentifier ===
      ZYRA_STANDARD_APPLICATION.universalIdentifier;

    if (!isBuildingZyraStandardApplication) {
      applicationIds.add(zyraStandardApplicationId);
    }
