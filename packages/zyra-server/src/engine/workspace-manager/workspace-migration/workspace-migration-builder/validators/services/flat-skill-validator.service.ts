...
import { ALL_METADATA_NAME } from 'zyra-shared/metadata';
...
export class FlatSkillValidatorService {
  ...
  >): FailedFlatEntityValidation<'skill', 'delete'> {
    ...
    if (
      !isCallerZyraStandardApp(buildOptions) &&
      belongsToZyraStandardApp({
  ...
  >): FailedFlatEntityValidation<'skill', 'update'> {
    ...
    const isZyraStandardSkill = belongsToZyraStandardApp({
