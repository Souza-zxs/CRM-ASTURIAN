...
import { resolveRolePermissionConfig } from 'src/engine/zyra-orm/utils/resolve-role-permission-config.util';
...
> {
  ...
  @Inject()
  protected readonly zyraConfigService: ZyraConfigService;
  @Inject()
  ...
  private async throttleQueryExecution(authContext: WorkspaceAuthContext) {
    try {
      ...
      const shortConfig = {
        ...
        maxTokens: this.zyraConfigService.get(
          'API_RATE_LIMITING_SHORT_LIMIT',
        ),
        timeWindow: this.zyraConfigService.get(
      ...
      const longConfig = {
        ...
        maxTokens: this.zyraConfigService.get('API_RATE_LIMITING_LONG_LIMIT'),
        timeWindow: this.zyraConfigService.get(
  ...
  private validateQueryComplexity(
    ...
    const maximumComplexity = this.zyraConfigService.get(
