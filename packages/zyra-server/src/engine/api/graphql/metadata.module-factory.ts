...
export const metadataModuleFactory = async (
  ...
  const config: YogaDriverConfig = {
    ...
    plugins: [
      ...
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        zyraConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields: zyraConfigService.get('GRAPHQL_MAX_FIELDS'),
  ...
  if (zyraConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT) {
