...
> {
  ...
  createGqlOptions(): YogaDriverConfig {
    const isDebugMode =
      this.zyraConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT;
    const plugins = [
      useDirectExecution({
      ...
      useGraphQLErrorHandlerHook({
        ...
        zyraConfigService: this.zyraConfigService,
      }),
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        this.zyraConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields:
          this.zyraConfigService.get('GRAPHQL_MAX_FIELDS'),
        maximumAllowedRootResolvers: this.zyraConfigService.get(
