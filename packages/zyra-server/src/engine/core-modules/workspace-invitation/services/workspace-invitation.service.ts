...
import ms from 'ms';
import { SendInviteLinkEmail } from 'zyra-emails';
import { AppPath, FileFolder } from 'zyra-shared/types';
...
export class WorkspaceInvitationService {
    ...
    for (const invitation of invitationResults) {
      if (invitation.status === 'fulfilled') {
        ...
        const emailData = {
          ...
          serverUrl: this.zyraConfigService.get('SERVER_URL'),
        ...
        const productName = this.zyraConfigService.get('ZYRA_PRODUCT_NAME');
        const joinTeamMsg = msg`Join your team on ${productName}`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via ${productName}) <${this.zyraConfigService.get('EMAIL_FROM_ADDRESS')}>`,
  ...
  async generateInvitationToken(
    ...
    const expiresIn = this.zyraConfigService.get(
