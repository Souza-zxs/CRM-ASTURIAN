...
export class FileStorageService {
  ...
  private buildStoragePathWithinWorkspaceOrThrow({
    ...
    // Storage keys are POSIX-style. Normalize Windows backslashes and use posix.join
    // so path segments are never joined with a backslash on Windows.
    const relativePathPosix = relativePath.replace(/\\/g, '/');

    const resourcePath = posix
      .join(fileFolder, relativePathPosix)
      .replace(/\/+/g, '/');
  ...
  private validateAndBuildFileStoragePathOrThrow(params: ResourceIdentifier): {
    ...
    const normalizedResourcePath = params.resourcePath.replace(/\\/g, '/');
    ...
    const { onStoragePath, resourcePath } =
      this.buildStoragePathWithinWorkspaceOrThrow({
        ...params,
        relativePath: normalizedResourcePath,
      });
  ...
  private validateAndBuildFolderStoragePathOrThrow(
    ...
    const normalizedFolderPath = params.folderPath.replace(/\\/g, '/');
