...
describe('FileStorageDriverFactory', () => {
  ...
  describe('buildConfigKey', () => {
    it('should build config key for local storage', () => {
      ...
      expect(zyraConfigService.get).toHaveBeenCalledWith('STORAGE_TYPE');
      expect(zyraConfigService.get).toHaveBeenCalledWith(
        'STORAGE_LOCAL_PATH',
      );
    });
  ...
  describe('createDriver', () => {
    ...
    it('should throw error for invalid storage driver type', () => {
      jest.spyOn(zyraConfigService, 'get').mockReturnValue('invalid-type');
