...
describe('BillingUsageCapService', () => {
  ...
  let zyraConfigService: jest.Mocked<ZyraConfigService>;
  ...
  beforeEach(async () => {
    ...
    zyraConfigService = module.get(ZyraConfigService);
  });
  ...
  describe('isClickHouseEnabled', () => {
    it('returns true when CLICKHOUSE_URL is configured', () => {
      zyraConfigService.get.mockReturnValue('http://clickhouse:8123');

      expect(service.isClickHouseEnabled()).toBe(true);
    });

    it('returns false when CLICKHOUSE_URL is empty', () => {
      zyraConfigService.get.mockReturnValue('');
