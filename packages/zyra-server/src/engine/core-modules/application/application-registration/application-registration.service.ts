...
import { type Manifest } from 'zyra-shared/application';
...
import { ZYRA_CLI_APPLICATION_REGISTRATION } from 'src/engine/workspace-manager/zyra-standard-application/constants/zyra-cli-application-registration.constant';
import {
...
export class ApplicationRegistrationService {
  ...
  async createCliRegistrationIfNotExists(): Promise<ApplicationRegistrationEntity | null> {
    const existing = await this.findOneByUniversalIdentifier(
      ZYRA_CLI_APPLICATION_REGISTRATION.universalIdentifier,
    );
    ...
    const registration = this.applicationRegistrationRepository.create({
      universalIdentifier:
        ZYRA_CLI_APPLICATION_REGISTRATION.universalIdentifier,
      name: ZYRA_CLI_APPLICATION_REGISTRATION.name,
      oAuthClientId: v4(),
      oAuthClientSecretHash: null,
      oAuthRedirectUris: [],
      oAuthScopes: ZYRA_CLI_APPLICATION_REGISTRATION.oAuthScopes,
      ownerWorkspaceId: null,
