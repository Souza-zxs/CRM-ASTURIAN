...
const APP_FETCHER_TMPDIR = join(tmpdir(), 'zyra-app-fetcher');
...
export class ApplicationPackageFetcherService implements OnModuleInit {
  ...
  private async resolveFromNpm(
    ...
    try {
      const registryUrl = this.zyraConfigService.get('APP_REGISTRY_URL');
      const authToken = this.zyraConfigService.get('APP_REGISTRY_TOKEN');
