...
import { FileFolder } from 'zyra-shared/types';
...
export class ApplicationService {
  ...
  async findWorkspaceZyraStandardAndCustomApplicationOrThrow({
    ...
    const zyraStandardFlatApplication =
      flatApplicationMaps.byId[zyraStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(zyraStandardFlatApplication) ||
  ...
  async findZyraStandardApplicationOrThrow(workspaceId: string): Promise<{
    ...
    const { zyraStandardFlatApplication } =
      await this.findWorkspaceZyraStandardAndCustomApplicationOrThrow({
        workspace,
      });

    const application = await this.applicationRepository.findOne({
      where: {
        id: zyraStandardFlatApplication.id,
  ...
  async createZyraStandardApplication(
    {
      workspaceId,
    ...
    const existingApplication = await this.findByUniversalIdentifier({
      universalIdentifier: ZYRA_STANDARD_APPLICATION.universalIdentifier,
      workspaceId,
    });
    ...
    const zyraStandardApplication = await this.create(
      {
        ...ZYRA_STANDARD_APPLICATION,
    ...
    return zyraStandardApplication;
  }
