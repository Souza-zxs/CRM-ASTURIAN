...
export const ZYRA_MCP_HELPER = `# Auto-injected Zyra MCP helper - provides access to Zyra tools
import os
import json
...
class ZyraMCP:
    """Helper for calling Zyra tools from sandboxed code.
    ...
    def __init__(self):
        self.url = os.environ.get('ZYRA_SERVER_URL', '')
        self.token = os.environ.get('ZYRA_API_TOKEN', '')
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = zyra.call_tool('find_many_companies', {'limit': 5})
            # companies == {'records': [...], 'count': '5'}
        """
        if not self._available:
            raise RuntimeError('Zyra MCP bridge not available. Missing requests library or credentials.')
...
# --------------------------------------------------------------------------
# \`zyra\` is a pre-built instance of the ZyraMCP class above. It is
# already bound in this module scope — DO NOT \`import zyra\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = zyra.call_tool('find_many_companies', {'limit': 10})
# --------------------------------------------------------------------------
zyra = ZyraMCP()
`;
