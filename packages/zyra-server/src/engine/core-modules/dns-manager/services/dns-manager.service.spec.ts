...
describe('DnsManagerService', () => {
  ...
  it('should initialize cloudflareClient when CLOUDFLARE_API_KEY is defined', () => {
    ...
    jest.spyOn(zyraConfigService, 'get').mockReturnValue(mockApiKey);

    const instance = new DnsManagerService(zyraConfigService, {} as any);

    expect(zyraConfigService.get).toHaveBeenCalledWith('CLOUDFLARE_API_KEY');
  ...
  describe('registerHostname', () => {
    ...
    it('should register a custom domain successfully', async () => {
      ...
      jest.spyOn(zyraConfigService, 'get').mockReturnValue('test-zone-id');
