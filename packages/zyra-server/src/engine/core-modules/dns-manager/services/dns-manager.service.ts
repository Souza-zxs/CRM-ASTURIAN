...
export class DnsManagerService {
  ...
  constructor(
    ...
    if (this.zyraConfigService.get('CLOUDFLARE_API_KEY')) {
      this.cloudflareClient = new Cloudflare({
        apiToken: this.zyraConfigService.get('CLOUDFLARE_API_KEY'),
      });
    }
  }
      ...
      records: [
        ...
        },
        {
          ...
          value:
            ...
            `${hostname}.${this.zyraConfigService.get('CLOUDFLARE_DCV_DELEGATION_ID')}.dcv.cloudflare.com`,
        },
      ],
    };
  }
  ...
  private getZoneId(options?: DnsManagerOptions): string {
    return options?.isPublicDomain
      ? this.zyraConfigService.get('CLOUDFLARE_PUBLIC_DOMAIN_ZONE_ID')
