...
export class EnterprisePlanService implements OnModuleInit {
  ...
  private refreshKeyPayload(): void {
    const enterpriseKey = this.zyraConfigService.get('ENTERPRISE_KEY');
  ...
  private async loadValidityToken(): Promise<void> {
    try {
      ...
      const tokenValue =
        dbToken?.value ??
        this.zyraConfigService.get('ENTERPRISE_VALIDITY_TOKEN');
  ...
  async setEnterpriseKey(enterpriseKey: string): Promise<void> {
    try {
      await this.zyraConfigService.set('ENTERPRISE_KEY', enterpriseKey);
    } catch (error) {
      if (
  ...
  async refreshValidityToken(): Promise<boolean> {
    ...
    const apiUrl = this.zyraConfigService.get('ENTERPRISE_API_URL');
  ...
  private async gatherInstanceMetadata(): Promise<EnterpriseInstanceMetadata> {
    return {
      serverId: this.zyraConfigService.get('SERVER_ID') ?? null,
      serverUrl: this.zyraConfigService.get('SERVER_URL') ?? null,
      appVersion: this.zyraConfigService.get('APP_VERSION') ?? null,
      nodeEnv: this.zyraConfigService.get('NODE_ENV') ?? null,
      telemetryEnabled:
        this.zyraConfigService.get('TELEMETRY_ENABLED') ?? null,
  ...
  private getPublicKeysToTry(): string[] {
    const nodeEnv = this.zyraConfigService.get('NODE_ENV');
