...
const eventSinksProvider = {
  ...
  useFactory: (
    ...
    const configuredSinkNames = zyraConfigService.get('EVENT_SINKS');
    ...
    return getAvailableSinkNames(configuredSinkNames, {
      hasClickhouseUrl: Boolean(zyraConfigService.get('CLICKHOUSE_URL')),
    })
      .map((name) => sinkByName[name.toLowerCase()])
      .filter(isDefined);
  },
  inject: [ZyraConfigService, ClickHouseEventSink, ConsoleEventSink],
};
