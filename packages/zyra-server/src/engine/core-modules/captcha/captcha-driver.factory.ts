...
export class CaptchaDriverFactory extends DriverFactoryBase<CaptchaDriver | null> {
  ...
  protected buildConfigKey(): string {
    const driver = this.zyraConfigService.get('CAPTCHA_DRIVER');
  ...
  protected createDriver(): CaptchaDriver | null {
    const driver = this.zyraConfigService.get('CAPTCHA_DRIVER');
    const siteKey = this.zyraConfigService.get('CAPTCHA_SITE_KEY');
    const secretKey = this.zyraConfigService.get('CAPTCHA_SECRET_KEY');
