...
export class LogicFunctionDriverFactory extends DriverFactoryBase<LogicFunctionDriver> {
  ...
  protected buildConfigKey(): string {
    const driverType = this.zyraConfigService.get('LOGIC_FUNCTION_TYPE');
  ...
  protected createDriver(): LogicFunctionDriver {
    ...
    switch (driverType) {
      ...
      case LogicFunctionDriverType.LAMBDA: {
        const region = this.zyraConfigService.get(
        ...
        const lambdaRole = this.zyraConfigService.get(
          'LOGIC_FUNCTION_LAMBDA_ROLE',
        );
        const subhostingRole = this.zyraConfigService.get(
          'LOGIC_FUNCTION_LAMBDA_SUBHOSTING_ROLE',
        );
        const s3BucketName = this.zyraConfigService.get('STORAGE_S3_NAME');
        const layerBucket =
          this.zyraConfigService.get('LOGIC_FUNCTION_LAMBDA_LAYER_BUCKET') ??
          s3BucketName ??
          'zyra-lambda-layer';
