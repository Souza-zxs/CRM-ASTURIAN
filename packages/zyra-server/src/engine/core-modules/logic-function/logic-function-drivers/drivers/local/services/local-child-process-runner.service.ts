...
export class LocalChildProcessRunnerService {
  // Symlinks everything from the deps layer except zyra-client-sdk,
  ...
  async assembleNodeModules({
    ...
    const symlinkPromises = entries
      .filter((entry) => entry.name !== 'zyra-client-sdk')
      .map((entry) =>
        fs.symlink(
    ...
    await fs.symlink(
      join(sdkNodeModules, 'zyra-client-sdk'),
      join(execNodeModules, 'zyra-client-sdk'),
      'dir',
    );
  }
