...
import { FeatureFlagKey } from 'zyra-shared/types';
...
export class LogicFunctionExecutorService {
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.zyraConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.zyraConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
  ...
  private async getExecutionEnvVariables({
    ...
    const baseUrl = cleanServerUrl(this.zyraConfigService.get('SERVER_URL'));
