...
import { type WorkspaceScopedRepository } from 'src/engine/zyra-orm/workspace-scoped-repository/workspace-scoped-repository';
  ...
  describe('sendApprovedAccessDomainValidationEmail', () => {
    ...
    it('should send a validation email if all conditions are met', async () => {
      ...
      jest
        ...
        .mockReturnValue(new URL('https://sub.zyra.com'));
      ...
      expect(emailService.send).toHaveBeenCalledWith({
        from: 'John Doe (via Zyra) <no-reply@example.com>',
        to: email,
