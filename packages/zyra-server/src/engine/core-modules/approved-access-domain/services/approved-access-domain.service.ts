...
import { SendApprovedAccessDomainValidation } from 'zyra-emails';
import { FileFolder, SettingsPath } from 'zyra-shared/types';
...
export class ApprovedAccessDomainService {
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via ${this.zyraConfigService.get('ZYRA_PRODUCT_NAME')}) <${this.zyraConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
