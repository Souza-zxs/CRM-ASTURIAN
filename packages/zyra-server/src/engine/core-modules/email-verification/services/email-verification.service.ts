...
import ms from 'ms';
import { SendEmailVerificationLinkEmail } from 'zyra-emails';
import { type APP_LOCALES } from 'zyra-shared/translations';
import { AppPath } from 'zyra-shared/types';
import { assertIsDefinedOrThrow, isDefined } from 'zyra-shared/utils';
...
export class EmailVerificationService {
  ...
  async sendVerificationEmail({
    ...
    if (!this.zyraConfigService.get('IS_EMAIL_VERIFICATION_REQUIRED')) {
    ...
    const emailVerificationMsg =
      verificationTrigger === EmailVerificationTrigger.EMAIL_UPDATE
        ...
        : msg`Welcome to ${productName}: Please Confirm Your Email`;
