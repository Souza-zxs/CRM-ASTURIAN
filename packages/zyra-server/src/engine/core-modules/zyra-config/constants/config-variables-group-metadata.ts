...
> = {
  ...
  [ConfigVariablesGroup.WHATSAPP_CONFIG]: {
    position: 2200,
    description:
      'Configure the WhatsApp Cloud API integration (Meta Tech Provider app used for Embedded Signup, so each workspace can connect its own WhatsApp Business number).',
    isHiddenOnLoad: true,
    isHiddenInAdminPanel: false,
  },
  [ConfigVariablesGroup.INSTAGRAM_CONFIG]: {
    position: 2300,
    description:
      'Configure the Instagram comment-to-DM automation integration (Meta App used for Instagram Login, so each workspace can connect its own Instagram professional account).',
    isHiddenOnLoad: true,
    isHiddenInAdminPanel: false,
  },
  [ConfigVariablesGroup.VOICE_AGENT_CONFIG]: {
    position: 2400,
    description:
      'Configure the AI voice agent integration (Twilio phone numbers + OpenAI Realtime API, so each workspace can run an AI agent that answers or makes phone calls).',
    isHiddenOnLoad: true,
    isHiddenInAdminPanel: false,
  },
  [ConfigVariablesGroup.BRANDING]: {
    position: 2500,
    description:
      'Configure white-label branding (product name, logo, support email) shown across the app and transactional emails.',
    isHiddenOnLoad: true,
    isHiddenInAdminPanel: false,
  },
  [ConfigVariablesGroup.WHATSAPP_AI_AGENT_CONFIG]: {
    position: 2600,
    description:
      'Configure the AI agent that auto-replies to inbound WhatsApp messages (reuses the existing WhatsApp channel connection and OPENAI_API_KEY).',
    isHiddenOnLoad: true,
    isHiddenInAdminPanel: false,
  },
};
