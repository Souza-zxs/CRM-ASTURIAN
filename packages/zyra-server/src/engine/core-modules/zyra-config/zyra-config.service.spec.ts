...
import { type ConfigVariables } from 'src/engine/core-modules/zyra-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/zyra-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/zyra-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type ZyraConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<ZyraConfigService>(ZyraConfigService),
...
const setPrivateProps = (
  service: ZyraConfigService,
  props: Partial<ZyraConfigServicePrivateProps>,
) => {
...
describe('ZyraConfigService', () => {
  let service: ZyraConfigService;
