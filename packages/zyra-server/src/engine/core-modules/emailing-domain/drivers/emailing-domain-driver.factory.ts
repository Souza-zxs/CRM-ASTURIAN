...
export class EmailingDomainDriverFactory extends DriverFactoryBase<EmailingDomainDriverInterface> {
  ...
  protected buildConfigKey(): string {
    const driver = this.zyraConfigService.get('EMAILING_DOMAIN_DRIVER');
  ...
  protected createDriver(): EmailingDomainDriverInterface {
    ...
    switch (driver) {
      case EmailingDomainDriver.AWS_SES: {
        const region = this.zyraConfigService.get('AWS_SES_REGION');
        const accountId = this.zyraConfigService.get('AWS_SES_ACCOUNT_ID');
        ...
        const sessionToken = this.zyraConfigService.get(
