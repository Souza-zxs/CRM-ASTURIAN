...
import { replaceCoreClient } from 'zyra-client-sdk/generate';
...
const SDK_CLIENT_ARCHIVE_NAME = 'zyra-client-sdk.zip';
...
export class SdkClientGenerationService {
  ...
  async enqueueSdkClientGenerationForWorkspace(
    ...
    await Promise.all(
      [zyraStandardFlatApplication, workspaceCustomFlatApplication].map(
        (application) =>
  ...
  private async generateAndStore({
    ...
    try {
      ...
      const tempPackageRoot = join(sourceTemporaryDir, 'zyra-client-sdk');
