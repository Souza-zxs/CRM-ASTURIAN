...
describe('SdkClientGenerationService', () => {
  ...
  let applicationService: jest.Mocked<
    Pick<
      ApplicationService,
      'findWorkspaceZyraStandardAndCustomApplicationOrThrow'
    >
  >;
  ...
  beforeEach(async () => {
    applicationService = {
      findWorkspaceZyraStandardAndCustomApplicationOrThrow: jest.fn(),
    };
    messageQueueService = {
  ...
  describe('enqueueSdkClientGenerationForWorkspace', () => {
    ...
    const apps = {
      zyraStandardFlatApplication: {
        id: 'std-app-id',
        universalIdentifier: 'zyra-standard',
      },
    ...
    it('enqueues one job per application with dedup id and retry limit', async () => {
      applicationService.findWorkspaceZyraStandardAndCustomApplicationOrThrow.mockResolvedValue(
        apps as never,
      );
      ...
      expect(messageQueueService.add).toHaveBeenNthCalledWith(
        ...
        GENERATE_SDK_CLIENT_JOB_NAME,
        {
          ...
          applicationUniversalIdentifier: 'zyra-standard',
        },
        {
