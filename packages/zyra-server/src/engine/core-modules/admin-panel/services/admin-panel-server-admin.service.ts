...
import { ServerAdminAccessChangedEmail } from 'zyra-emails';
...
export class AdminPanelServerAdminService {
  ...
  private async notifyAdministrators({
    ...
    try {
      ...
      const from = `${this.zyraConfigService.get('EMAIL_FROM_NAME')} <${this.zyraConfigService.get('EMAIL_FROM_ADDRESS')}>`;
