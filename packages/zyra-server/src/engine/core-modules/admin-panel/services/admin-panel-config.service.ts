...
import { CONFIG_VARIABLES_GROUP_METADATA } from 'src/engine/core-modules/zyra-config/constants/config-variables-group-metadata';
...
export class AdminPanelConfigService {
  ...
  getConfigVariablesGrouped(): ConfigVariablesDTO {
    const rawEnvVars = this.zyraConfigService.getAll();
  ...
  getConfigVariable(key: string): ConfigVariableDTO {
    const variableWithMetadata =
      this.zyraConfigService.getVariableWithMetadata(
