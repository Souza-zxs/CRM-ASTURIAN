...
export class WorkspaceDomainsService {
  ...
  private async getDefaultWorkspace() {
    if (this.zyraConfigService.get('IS_MULTIWORKSPACE_ENABLED')) {
      throw new Error(
  ...
  private getZyraWorkspaceUrl(subdomain: string) {
    const url = this.domainServerConfigService.getFrontUrl();

    url.hostname = this.zyraConfigService.get('IS_MULTIWORKSPACE_ENABLED')
  ...
  getSubdomainAndCustomDomainFromWorkspaceFallbackOnDefaultSubdomain(
    ...
    if (!workspace) {
      return {
        subdomain: this.zyraConfigService.get('DEFAULT_SUBDOMAIN'),
        customDomain: null,
    ...
    return {
      customUrl:
        isCustomDomainEnabled && customDomain
          ...
          : undefined,
      subdomainUrl: this.getZyraWorkspaceUrl(subdomain),
    };
  }
