...
describe('resolveWorkspaceIdFromRequest', () => {
  it('resolves a nested value from the body', () => {
    const request = buildRequest({
      body: { metadata: { zyraWorkspaceId: 'ws-1' } },
    });

    expect(
      resolveWorkspaceIdFromRequest({
        resolver: { source: 'body', path: 'metadata.zyraWorkspaceId' },
        request,
      }),
    ).toBe('ws-1');
  });
  ...
  it('resolves from a query parameter', () => {
    const request = buildRequest({ query: { zyraWorkspaceId: 'ws-2' } });

    expect(
      resolveWorkspaceIdFromRequest({
        resolver: { source: 'query', path: 'zyraWorkspaceId' },
        request,
      }),
    ).toBe('ws-2');
  });
