...
import { ZYRA_MIME_POLICY } from 'src/engine/core-modules/file/constants/zyra-mime-policy.constant';
import { buildFileInfo } from 'src/engine/core-modules/file/utils/build-file-info.utils';
import { getFileTypeParser } from 'src/utils/get-file-type-parser.util';
...
export const extractFileInfoOrThrow = async ({
  ...
  const { parser: fileTypeParser, supportedMimeTypes } =
    await getFileTypeParser();
  ...
  if (isNonEmptyString(ext)) {
    // Zyra policy wins over the ext-based fallback for the (small) set of
    ...
    // source upload.
    const policyMime = ZYRA_MIME_POLICY[ext];
