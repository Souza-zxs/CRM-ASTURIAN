...
import { ensureAbsoluteUrl } from 'zyra-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-Zyra-Webhook-Timestamp'] = Date.now().toString();
        headers['X-Zyra-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-Zyra-Webhook-Timestamp'],
        );
        headers['X-Zyra-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
