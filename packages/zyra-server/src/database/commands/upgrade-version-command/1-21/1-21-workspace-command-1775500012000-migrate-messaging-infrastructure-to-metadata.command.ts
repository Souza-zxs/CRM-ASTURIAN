...
export class MigrateMessagingInfrastructureToMetadataCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  constructor(
    private readonly zyraORMGlobalManager: GlobalWorkspaceOrmManager,
  ...
  override async runOnWorkspace({
    ...
    const connectedAccountWorkspaceRepository =
      await this.zyraORMGlobalManager.getRepository<LegacyConnectedAccountWorkspaceEntity>(
        workspaceId,
        'connectedAccount',
      );

    const messageChannelWorkspaceRepository =
      await this.zyraORMGlobalManager.getRepository<MessageChannelEntity>(
        workspaceId,
        'messageChannel',
      );

    const calendarChannelWorkspaceRepository =
      await this.zyraORMGlobalManager.getRepository<CalendarChannelEntity>(
        workspaceId,
        'calendarChannel',
      );

    const messageFolderWorkspaceRepository =
      await this.zyraORMGlobalManager.getRepository<MessageFolderWorkspaceEntity>(
        workspaceId,
        'messageFolder',
      );
  ...
  private async buildWorkspaceMemberIdToUserWorkspaceIdMap(
    ...
    const workspaceMemberRepository =
      await this.zyraORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
        workspaceId,
        'workspaceMember',
