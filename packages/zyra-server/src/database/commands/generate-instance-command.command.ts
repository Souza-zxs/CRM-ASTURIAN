...
export class GenerateInstanceCommandCommand extends CommandRunner {
  ...
  parseVersion(value: string): ZyraAllVersion {
    if (
      !ZYRA_ALL_VERSIONS.includes(
        value as (typeof ZYRA_ALL_VERSIONS)[number],
      )
    ) {
      throw new Error(
        `Invalid version "${value}". Must be one of: ${ZYRA_ALL_VERSIONS.join(', ')}`,
      );
    }

    return value as ZyraAllVersion;
  }
  ...
  async run(
    ...
    const version = options.version ?? ZYRA_CURRENT_VERSION;
