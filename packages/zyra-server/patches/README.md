...
yarn patch-commit <temp-folder> > packages/zyra-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/zyra-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/zyra-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
