...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const zyraStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        ZYRA_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(zyraStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardZyraApplication
    } = ZYRA_STANDARD_APPLICATION;

    expect(zyraStandardApp).toMatchObject(expectedStandardZyraApplication);
