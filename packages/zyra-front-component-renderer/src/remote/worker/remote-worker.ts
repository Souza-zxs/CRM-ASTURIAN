...
import { type FrontComponentExecutionContext } from 'zyra-sdk/front-component';
...
const SDK_IMPORT_SPECIFIERS = [
  'zyra-client-sdk/core',
  'zyra-client-sdk/metadata',
] as const;
...
const rewriteSdkImports = (
  ...
  const specifierToBlobUrl: Record<string, string> = {
    'zyra-client-sdk/core': sdkClientUrls.core,
    'zyra-client-sdk/metadata': sdkClientUrls.metadata,
  };
...
const render: WorkerExports['render'] = async (
  ...
  if (isDefined(renderContext.apiUrl)) {
    setWorkerEnv({
      ZYRA_API_URL: renderContext.apiUrl,
    });
  }

  if (isDefined(renderContext.applicationAccessToken)) {
    setWorkerEnv({
      ZYRA_APP_ACCESS_TOKEN: renderContext.applicationAccessToken,
    });
  }
