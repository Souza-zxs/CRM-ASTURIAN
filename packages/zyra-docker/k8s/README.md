# Kubernetes for Zyra CRM (Helm)
DISCLAIMER: The k8s and podman deployments are not maintained by the core team.
These files are provided and maintained by the community. Zyra core team
maintains support for docker deployment.

## Overview

This repository contains a world-class Helm chart and (legacy) raw Kubernetes manifests plus Terraform files to help you deploy and manage the Zyra CRM application. The chart is located at `packages/zyra-docker/helm/zyra` and supersedes the manifests in `packages/zyra-docker/k8s/manifests`.
...
helm install my-zyra packages/zyra-docker/helm/zyra \
  --namespace zyracrm --create-namespace --wait \
...
See [QUICKSTART](../helm/zyra/QUICKSTART.md) and [chart README](../helm/zyra/README.md) for details.
...
2. Create Server Secret
    ...
    ``` bash
    kubectl create secret generic -n zyracrm tokens --from-literal accessToken=changeme --from-literal loginToken="changeme" --from-literal refreshToken="changeme" --from-literal fileToken="changeme"
    ```
