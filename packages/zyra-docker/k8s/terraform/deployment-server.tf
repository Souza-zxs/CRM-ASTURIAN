resource "kubernetes_deployment" "zyracrm_server" {
  metadata {
  ...
  spec {
    replicas = var.zyracrm_server_replicas
    selector {
      match_labels = {
        app = "${var.zyracrm_app_name}-server"
      }
    ...
    template {
      ...
      spec {
        container {
          ...
          volume_mount {
            name       = "server-data"
            mount_path = var.zyracrm_server_data_mount_path
          }

          volume_mount {
            name       = "docker-data"
            mount_path = var.zyracrm_docker_data_mount_path
          }
        }
