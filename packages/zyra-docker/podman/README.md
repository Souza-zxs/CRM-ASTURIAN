...
2. Start zyra by running `podman-compose up -d`.

If you need to stop zyra, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run zyra, you can use the provided systemd service. 

Edit `zyracrm.service` and change these two variables:


	WorkingDirectory=/opt/apps/zyra
	EnvironmentFile=/opt/apps/zyra/.env
...
Note: this script will enable the service and also start it. So it will assume that zyra is not currently running.
