...
- Create the PostgreSQL database "zyra" and schema "core" automatically
...
```bash
kubectl get secret tokens -n zyracrm -o jsonpath='{.data.accessToken}' | base64 --decode && echo
```
...
```bash
helm uninstall my-zyra -n zyracrm
kubectl delete namespace zyracrm
```
