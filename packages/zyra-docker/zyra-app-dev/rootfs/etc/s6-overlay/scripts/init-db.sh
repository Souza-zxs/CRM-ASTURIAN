...
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_roles WHERE rolname='zyra'" | grep -q 1 \
  || su-exec postgres psql -h localhost -c "CREATE ROLE zyra WITH LOGIN PASSWORD 'zyra' SUPERUSER"

# Create database if it doesn't exist
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_database WHERE datname='default'" | grep -q 1 \
  || su-exec postgres createdb -h localhost -O zyra default

# Run Zyra database setup and migrations
cd /app/packages/zyra-server

has_schema=$(PGPASSWORD=zyra psql -h localhost -U zyra -d default -tAc \
...
has_workspace=$(PGPASSWORD=zyra psql -h localhost -U zyra -d default -tAc \
