...
version=${VERSION:-$(curl -s "https://hub.docker.com/v2/repositories/zyracrm/zyra/tags" | grep -o '"name":"[^"]*"' | grep -v 'latest' | cut -d'"' -f4 | sort -V | tail -n1)}
branch=${BRANCH:-$(curl -s https://api.github.com/repos/zyrahq/zyra/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
# Create a directory named zyra
echo "📁 Creating directory '$dir_name'"
mkdir -p "$dir_name" && cd "$dir_name" || { echo "❌ Failed to create/access directory '$dir_name'"; exit 1; }

# Copy zyra/packages/zyra-docker/docker-compose.yml in it
echo -e "\t• Copying docker-compose.yml"
curl -sLo docker-compose.yml https://raw.githubusercontent.com/zyrahq/zyra/$branch/packages/zyra-docker/docker-compose.yml

# Copy zyra/packages/zyra-docker/.env.example to .env
echo -e "\t• Setting up .env file"
curl -sLo .env https://raw.githubusercontent.com/zyrahq/zyra/$branch/packages/zyra-docker/.env.example
...
if [ "$answer" = "n" ]; then
  ...
  pid=$!
  while [ ! $(docker inspect --format='{{.State.Health.Status}}' zyra-server-1) = "healthy" ]; do
    sleep 1
  done
  kill $pid
  echo ""
