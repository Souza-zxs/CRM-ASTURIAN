...
const zyraClientTemplateSource = readFileSync(
  join(__dirname, '..', 'zyra-client-template.ts'),
  'utf-8',
);

type ZyraClassType = new (options?: {
  url?: string;
...
describe('Generated client wrapper auth behavior', () => {
  ...
  let ZyraClass: ZyraClassType;

  beforeAll(async () => {
    temporaryDir = await mkdtemp(join(tmpdir(), 'zyra-generated-client-'));

    const wrapperSource = buildClientWrapperSource(zyraClientTemplateSource, {
      apiClientName: 'MetadataApiClient',
      defaultUrl: '`${process.env.ZYRA_API_URL}/metadata`',
    ...
    ZyraClass = generatedModule.MetadataApiClient as ZyraClassType;
  });

  beforeEach(() => {
    delete process.env.ZYRA_APP_ACCESS_TOKEN;
    delete process.env.ZYRA_API_KEY;
  ...
  it('uses ZYRA_APP_ACCESS_TOKEN before ZYRA_API_KEY', async () => {
    process.env.ZYRA_APP_ACCESS_TOKEN = 'application-token';
    process.env.ZYRA_API_KEY = 'api-key-token';
    ...
    const zyraClient = new ZyraClass({
      url: 'https://example.com/graphql',
      fetch: fetchMock as unknown as typeof globalThis.fetch,
    });

    await zyraClient.query({ record: { id: true } });

    expect(capturedAuthorizationHeaders).toEqual(['Bearer application-token']);
  });

  it('falls back to ZYRA_API_KEY when ZYRA_APP_ACCESS_TOKEN is absent', async () => {
    process.env.ZYRA_API_KEY = 'legacy-api-key-token';
  ...
  it('refreshes and retries once after auth error when refresh callback is available', async () => {
    process.env.ZYRA_APP_ACCESS_TOKEN = 'stale-token';
    process.env.ZYRA_API_KEY = 'legacy-api-key-token';
    ...
    const queryResult = await zyraClient.query({ record: { id: true } });

    expect(queryResult).toEqual({ data: { record: { id: 'record-id' } } });
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(process.env.ZYRA_APP_ACCESS_TOKEN).toBe('fresh-token');
    expect(process.env.ZYRA_API_KEY).toBe('legacy-api-key-token');
  });
  ...
  it('deduplicates concurrent token refresh requests', async () => {
    ...
    await Promise.all([
      zyraClient.query({ record: { id: true } }),
      zyraClient.query({ record: { id: true } }),
    ]);
  ...
  it('retries uploadFile once after 401 when refresh callback is available', async () => {
    ...
    const uploadResult = await zyraClient.uploadFile(
      Buffer.from('content'),
      'test.txt',
      'text/plain',
      'field-uuid',
  ...
  it('bubbles auth error when refresh callback throws', async () => {
    ...
    await expect(zyraClient.query({ record: { id: true } })).rejects.toThrow(
      'Unauthorized',
    );
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'Zyra client: token refresh failed',
      expect.any(Error),
    );
