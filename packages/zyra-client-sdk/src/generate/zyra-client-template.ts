...
const APP_ACCESS_TOKEN_ENV_KEY = 'ZYRA_APP_ACCESS_TOKEN';
const API_KEY_ENV_KEY = 'ZYRA_API_KEY';

type ZyraGeneratedClientOptions = ClientOptions;
...
const defaultOptions: ZyraGeneratedClientOptions = {
  url: '__ZYRA_DEFAULT_URL__',
  headers: {
    'Content-Type': 'application/json',
  },
};

export class ZyraGeneratedClient {
  ...
  constructor(options?: ZyraGeneratedClientOptions) {
    const merged: ZyraGeneratedClientOptions = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (!this.fetchImplementation) {
      throw new Error(
        'Global `fetch` function is not available, ' +
          'pass a fetch implementation to the Zyra client',
      );
    }
    ...
    if (!this.refreshAccessTokenPromise) {
      this.refreshAccessTokenPromise = refreshAccessTokenFunction()
        ...
        .catch((refreshError: unknown) => {
          console.error('Zyra client: token refresh failed', refreshError);
