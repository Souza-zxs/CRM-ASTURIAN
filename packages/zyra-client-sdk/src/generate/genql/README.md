...
`listr`, etc.) that Zyra never executed.
...
- **Dropped the live-endpoint introspection path** (`schema/fetchSchema.ts`),
  which was the only consumer of `undici` / `native-fetch` / `qs`. Zyra always
...
- **Replaced `@graphql-tools/load`** with graphql's own `buildSchema` — Zyra
...
- **`Config` was narrowed** to the schema-string inputs Zyra actually passes
  ...
  generated client still defaults its url/fetch to `undefined` (Zyra's wrapper
