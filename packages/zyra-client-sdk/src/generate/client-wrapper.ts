...
export const buildClientWrapperSource = (
  ...
  source = source.replace("'__ZYRA_DEFAULT_URL__'", options.defaultUrl);

  source = source.replace(/ZyraGeneratedClient/g, options.apiClientName);
  ...
  return `\n// ${options.apiClientName} (auto-injected by zyra-client-sdk)\n${source}`;
};
