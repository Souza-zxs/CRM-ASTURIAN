...
const stopAction = (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    console.log(chalk.yellow('No Zyra server container found.'));

    return;
  }

  execSync(`docker stop ${containerName}`, { stdio: 'ignore' });
  console.log(chalk.green('Zyra server stopped.'));
};
...
const statusAction = async (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    ...
    console.log(
      chalk.gray(
        `  Run 'yarn zyra docker:start${options.test ? ' --test' : ''}' to create one.`,
      ),
    );
...
const resetAction = (options: { test?: boolean }) => {
  ...
  const volumeData = options.test
    ? 'zyra-app-dev-test-data'
    : 'zyra-app-dev-data';
  const volumeStorage = options.test
    ? 'zyra-app-dev-test-storage'
    : 'zyra-app-dev-storage';
  ...
  console.log(chalk.green('Zyra server data reset.'));
  console.log(
    chalk.gray(
      `Run 'yarn zyra docker:start${options.test ? ' --test' : ''}' to start a fresh instance.`,
    ),
  );
};
...
const upgradeAction = async (
  ...
  if (data.containerRecreated) {
    console.log(
      chalk.gray(
        `  Run 'yarn zyra docker:start${options.test ? ' --test' : ''}' to wait for the server to be ready.`,
      ),
    );
  }
};
...
export const registerServerCommands = (program: Command): void => {
  program
    ...
    .description(
      'Start the local Zyra container (version defaults to the app `engines.zyra` range, then `latest`)',
    )
  ...
  program
    .command('docker:stop')
    .description('Stop the local Zyra container')
  ...
  const server = program
    ...
    .description(
      'Manage a Zyra server (local instance and server-side actions)',
    );

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`zyra server ${oldCmd}\` is deprecated. Use \`zyra ${newCmd}\` instead.`,
      ),
    );
