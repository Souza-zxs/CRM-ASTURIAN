...
import { normalizeUrl } from 'zyra-shared/utils';
  ...
  if (serverUrl) {
    ...
    if (options.local) {
      if (!detectedUrl) {
        console.error(
          chalk.red(
            'No local Zyra server found.\n' +
              'Start one with: yarn zyra docker:start',
          ),
        );
        process.exit(1);
      }
...
const listAction = async () => {
  ...
  if (remotes.length === 0) {
    ...
    console.log("Use 'zyra remote:add' to add one.");
  ...
  for (const remoteName of remotes) {
    ...
    const authMethod = config.zyraCLIAccessToken
      ? 'oauth'
      : config.apiKey
        ? 'api-key'
        : 'none';
  ...
  console.log(
    '\n',
    chalk.gray("Use 'zyra remote:use <name>' to change default"),
  );
};
...
export const registerRemoteCommands = (program: Command): void => {
  program
    ...
    .option('--local', 'Connect to a local Zyra server (auto-detect)')
  ...
  const remote = program
    ...
    .description('Manage remote Zyra servers');

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`zyra remote ${oldCmd}\` is deprecated. Use \`zyra ${newCmd}\` instead.`,
      ),
    );
