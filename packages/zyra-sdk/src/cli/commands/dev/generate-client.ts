...
export class AppGenerateClientCommand {
  async execute(options: AppGenerateClientCommandOptions): Promise<void> {
    ...
    const clientSdkPath = join(appPath, 'node_modules', 'zyra-client-sdk');

    if (!(await pathExists(clientSdkPath))) {
      console.error(
        chalk.red(
          `Cannot find zyra-client-sdk in ${appPath}.\n\n` +
            '  Install it first:\n' +
            '    yarn add zyra-client-sdk',
        ),
      );
      process.exit(1);
    }
    ...
    if (!validateAuth.serverUp) {
      console.error(
        chalk.red(
          'Cannot reach Zyra server.\n\n' +
            '  Check your remotes:\n' +
            '    yarn zyra remote:status',
        ),
      );
      process.exit(1);
    }
    ...
    if (!validateAuth.authValid) {
      console.error(
        chalk.red(
          'Authentication failed. Run `yarn zyra remote:add` to authenticate.',
        ),
      );
      process.exit(1);
    }
