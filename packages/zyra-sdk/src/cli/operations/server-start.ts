...
const innerServerStart = async (
  ...
  if (existingUrl) {
    ...
    onProgress?.(`Zyra server detected on ${existingUrl}`);
    ...
    if (!healthy) {
      return {
        ...
        error: {
          ...
          message:
            'Zyra server did not become healthy in time.\n' +
            "Check: 'yarn zyra docker:logs'",
        },
      };
    }
  ...
  if (containerExists(containerName)) {
    ...
    if (existingPort !== port) {
      onProgress?.(
        `Existing container uses port ${existingPort}. Run 'yarn zyra docker:reset${isTest ? ' --test' : ''}' first to change ports.`,
      );
    }
    ...
    onProgress?.(
      `Pulling Docker image (${image}) and starting Zyra container...`,
    );
    ...
    const runResult = spawnSync(
      ...
      [
        ...
        '-v',
        `${volumeStorage}:/app/packages/zyra-server/.local-storage`,
        image,
      ],
      { stdio: 'inherit' },
    );
    ...
    if (runResult.status !== 0) {
      return {
        ...
        error: {
          ...
          message: 'Failed to start Zyra container.',
        },
      };
    }
  }

  onProgress?.('Waiting for Zyra to be ready...');
