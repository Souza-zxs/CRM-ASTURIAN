...
const getAppDeclaredSdkVersion = async (
  ...
  try {
    ...
    return (
      pkg.dependencies?.['zyra-sdk'] ??
      pkg.devDependencies?.['zyra-sdk'] ??
      undefined
    );
  } catch {
    return undefined;
...
export const checkSdkVersionCompatibility = async (
  ...
  if (cliMajor !== appMajor) {
    console.warn(
      chalk.yellow(
        `⚠ Version mismatch: your app requires zyra-sdk@${appMajor}.x ` +
          `but the CLI running is v${cliVersion}. ` +
          `Major version mismatches may cause unexpected behavior.\n` +
          `  Update with: npm install -g zyra-sdk@${appMajor}`,
      ),
    );
    console.warn('');
  }
