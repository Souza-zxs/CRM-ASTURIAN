...
export const checkServerVersionCompatibility = async (
  ...
  console.warn(
    chalk.yellow(
      `⚠ Local Zyra server is v${info.localServerVersion} (${info.daysBehind} days behind v${info.latestServerVersion}).`,
    ),
  );
  console.warn(chalk.dim('  Update with: yarn zyra docker:upgrade'));
  console.warn('');
};
