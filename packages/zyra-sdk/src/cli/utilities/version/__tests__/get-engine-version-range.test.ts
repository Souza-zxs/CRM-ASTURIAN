...
describe('getEngineVersionRange', () => {
  ...
  const seed = (contents: unknown): string => {
    const dir = mkdtempSync(join(tmpdir(), 'zyra-app-server-range-'));
  ...
  it('returns the engines.zyra range', () => {
    const dir = seed({ engines: { zyra: '>=2.2.0' } });
  ...
  it('returns null when engines.zyra is missing', () => {
    const dir = seed({ engines: { node: '^24.0.0' } });

    expect(getEngineVersionRange(dir)).toBeNull();
  });

  it('returns null when engines.zyra is empty', () => {
    const dir = seed({ engines: { zyra: '   ' } });

    expect(getEngineVersionRange(dir)).toBeNull();
  });

  it('returns null when engines.zyra is not a string', () => {
    const dir = seed({ engines: { zyra: 2 } });

    expect(getEngineVersionRange(dir)).toBeNull();
  });

  it('returns null when no package.json exists', () => {
    const dir = mkdtempSync(join(tmpdir(), 'zyra-app-server-range-empty-'));
