...
// TODO: ZYRA_LOGO_MARK_PATH still draws the upstream Twenty CRM logo mark
// (see assets/zyra-logo-mark-path.ts) as a placeholder on the right-hand
// tile of the generated cover image. Swap it for the real Zyra/Horizon logo
// mark before this ships to production.
import { ZYRA_LOGO_MARK_PATH } from '@/cli/utilities/build/cover/assets/zyra-logo-mark-path';
...
const ZYRA_MARK_VIEWBOX = 40;
const ZYRA_MARK_SCALE = TILE_SIZE / ZYRA_MARK_VIEWBOX;
...
export const generateCoverImage = async ({
  ...
  const svg = [
    ...
    `<g transform="translate(${RIGHT_TILE_X} ${TILE_Y}) scale(${ZYRA_MARK_SCALE})"><path d="${ZYRA_LOGO_MARK_PATH}" fill="#ffffff" /></g>`,
