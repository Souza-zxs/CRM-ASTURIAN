...
const writeTempPackageJson = async (
  ...
  const appPath = await mkdtemp(join(tmpdir(), 'zyra-sdk-deps-'));
...
describe('validatePackageJsonDependencies', () => {
  ...
  it('should warn when zyra-sdk is listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'zyra-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('zyra-sdk');
    expect(warnings[0]).toContain('devDependencies');
  });

  it('should warn when zyra-client-sdk is listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'zyra-client-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('zyra-client-sdk');
    expect(warnings[0]).toContain('devDependencies');
  });

  it('should warn for both SDK packages when both are listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'zyra-sdk': '^2.8.0', 'zyra-client-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(2);
    expect(warnings.some((warning) => warning.includes('zyra-sdk'))).toBe(
      true,
    );
    expect(
      warnings.some((warning) => warning.includes('zyra-client-sdk')),
    ).toBe(true);
  });
