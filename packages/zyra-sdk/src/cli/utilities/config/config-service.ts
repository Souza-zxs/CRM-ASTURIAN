...
export type RemoteConfig = {
  apiUrl: string;
  apiKey?: string;
  // CLI OAuth app credentials (from `yarn zyra remote:add`)
  zyraCLIRegistrationId?: string;
  zyraCLIRegistrationClientId?: string;
  zyraCLIAccessToken?: string;
  zyraCLIRefreshToken?: string;
...
export class ConfigService {
  ...
  // to the current format (remotes, zyraCLIAccessToken/zyraCLIRefreshToken)
  private async migrateConfigIfNeeded(
    ...
    const migrateRemoteFields = (
      ...
      zyraCLIRegistrationClientId:
        str(source.zyraCLIRegistrationClientId) ?? str(source.oauthClientId),
      zyraCLIAccessToken:
        str(source.zyraCLIAccessToken) ??
        str(source.accessToken) ??
        str(source.applicationAccessToken),
      zyraCLIRefreshToken:
        str(source.zyraCLIRefreshToken) ??
  ...
  async getConfigForRemote(remoteName: string): Promise<RemoteConfig> {
    ...
    try {
      ...
      return {
        ...
        zyraCLIRegistrationId: remoteConfig.zyraCLIRegistrationId,
        zyraCLIRegistrationClientId:
          remoteConfig.zyraCLIRegistrationClientId,
        zyraCLIAccessToken: remoteConfig.zyraCLIAccessToken,
        zyraCLIRefreshToken: remoteConfig.zyraCLIRefreshToken,
