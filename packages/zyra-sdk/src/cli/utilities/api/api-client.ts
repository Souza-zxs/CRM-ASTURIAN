...
export class ApiClient {
  ...
  constructor(options?: {
    ...
    this.client.interceptors.request.use(async (config) => {
      const zyraConfig = await this.configService.getConfig();

      config.baseURL = this.serverUrlOverride ?? zyraConfig.apiUrl;
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        if (error.response?.status === 401 && error.config) {
          ...
          console.error(
            chalk.red('Cannot connect to Zyra server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async refreshToken(): Promise<string | null> {
    ...
    if (
      !config.zyraCLIRefreshToken ||
      !config.zyraCLIRegistrationClientId
    ) {
      return null;
    }
    ...
    try {
      const tokenResponse = await axios.post(`${config.apiUrl}/oauth/token`, {
        ...
        refresh_token: config.zyraCLIRefreshToken,
        client_id: config.zyraCLIRegistrationClientId,
      });
      ...
      await this.configService.setConfig({
        zyraCLIAccessToken: newAccessToken,
        ...(newRefreshToken ? { zyraCLIRefreshToken: newRefreshToken } : {}),
      });
  ...
  async resolveAuthToken(): Promise<string | undefined> {
    ...
    const config = await this.configService.getConfig();
    const cliToken = config.zyraCLIAccessToken;
