import { type CommandMenuContextApi } from 'zyra-shared/types';
...
const createExpressionOnlyPlaceholder = (name: string): unknown => {
  const fail = (): never => {
    throw new Error(
      `"${name}" from zyra-sdk can only be used inside a command's conditionalAvailabilityExpression, not at runtime.`,
    );
  };
