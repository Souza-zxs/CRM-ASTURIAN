...
import { ComponentDecorator } from 'zyra-ui/testing';
  ...
  args: {
    value: {
      ...
      secondaryLinks: [
        ...
        { url: 'https://www.zyra.com', label: 'Valid Link' },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    expect(validLink).toHaveAttribute('href', 'https://www.zyra.com');
...
export const SingleLink: Story = {
  args: {
    value: {
      primaryLinkUrl: 'https://www.zyra.com',
      primaryLinkLabel: 'Zyra Website',
      secondaryLinks: null,
    },
  },
  play: async ({ canvasElement }) => {
    ...
    expect(link).toHaveAttribute('href', 'https://www.zyra.com');
    expect(link).toHaveTextContent('Zyra Website');
...
export const MultipleLinks: Story = {
  args: {
    value: {
      ...
      primaryLinkLabel: 'Zyra Website',
      secondaryLinks: [
        { url: 'https://docs.zyra.com', label: 'Documentation' },
        { url: 'https://blog.zyra.com', label: 'Blog' },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const primaryLink = await canvas.findByText('Zyra Website');
    expect(primaryLink).toBeVisible();
    expect(primaryLink).toHaveAttribute('href', 'https://www.zyra.com');

    const docsLink = await canvas.findByText('Documentation');
    expect(docsLink).toBeVisible();
    expect(docsLink).toHaveAttribute('href', 'https://docs.zyra.com');

    const blogLink = await canvas.findByText('Blog');
    expect(blogLink).toBeVisible();
    expect(blogLink).toHaveAttribute('href', 'https://blog.zyra.com');
  },
};
...
export const SocialMediaLinks: Story = {
  args: {
    value: {
      primaryLinkUrl: 'https://www.linkedin.com/company/zyra',
      primaryLinkLabel: 'Zyra on LinkedIn',
      secondaryLinks: [
        { url: 'https://twitter.com/zyracrm', label: 'Zyra on Twitter' },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const linkedinLink = await canvas.findByText('zyra');
    expect(linkedinLink).toBeVisible();
    expect(linkedinLink).toHaveAttribute(
      'href',
      'https://www.linkedin.com/company/zyra',
    );

    const twitterLink = await canvas.findByText('@zyracrm');
    expect(twitterLink).toBeVisible();
    expect(twitterLink).toHaveAttribute(
      'href',
      'https://twitter.com/zyracrm',
    );
  },
};
