...
import { ModalContent } from 'zyra-ui/surfaces';
import { useLingui } from '@lingui/react/macro';
import { useLocation, useSearchParams } from 'react-router-dom';
import { AppPath } from 'zyra-shared/types';
import { isDefined } from 'zyra-shared/utils';
import { Loader } from 'zyra-ui/feedback';
import { themeCssVariables } from 'zyra-ui/theme-constants';
import { AnimatedEaseIn } from 'zyra-ui/layout';
...
const StandardContent = ({
  ...
  const useSplitLayout =
    isMatchingLocation(location, AppPath.Login) ||
    isMatchingLocation(location, AppPath.SignUp);
...
export const SignInUp = () => {
  ...
  const location = useLocation();
  const isOnLoginRoute = isMatchingLocation(location, AppPath.Login);
  const isOnSignUpRoute = isMatchingLocation(location, AppPath.SignUp);
  ...
  const title = useMemo(() => {
    ...
    if (isGlobalScope) {
      return t`Welcome to ${brand?.productName || 'Zyra'}`;
    }
  ...
  if (signInUpStep === SignInUpStep.EmailVerification) {
    ...
    if (isOnLoginRoute || isOnSignUpRoute) {
      return <AuthSplitLayout>{emailVerificationContent}</AuthSplitLayout>;
    }
