...
All notable changes to the Zyra Codex plugin are documented in this file.
...
Entries reference the canonical skills (`create-app`, `develop-app`, `manage-app`, `publish-app`, `use-zyra-mcp`) and the validation script (`scripts/validate.js`).

## [0.1.0] - Unreleased

The first version of the Zyra Codex plugin.
...
- `references/develop-app/tests.md` and `references/manage-app/cli-and-sync.md`: documented that integration tests must run against the isolated test instance (`yarn zyra docker:start --test`, port `2021`) instead of the dev instance, since the test harness installs and uninstalls the app on its target server.
- `skills/manage-app/SKILL.md`: added direct test-run routing so "run tests" requests load the tests reference and run full suites with `ZYRA_API_URL=http://localhost:2021`.
...
- `.mcp.json` declaring only the public `zyra-docs` MCP server at `https://docs.zyra.com/mcp`.
- `assets/zyra-logo.png` and `assets/zyra-logo.svg` for marketplace branding.

### Skills (`skills/`)
- `create-app` — scaffold a new Zyra app via `create-zyra-app`, with prompts for the Zyra instance URL and Docker vs existing-instance choice.
- `develop-app` — add or modify Zyra app entities (objects, fields, logic functions, roles, views, layouts, skills, agents, connection providers, front components); enforces `yarn zyra dev:add` for entity generation and one-shot `yarn zyra dev --once` sync.
...
- `use-zyra-mcp` — workspace MCP connection, record retrieval, and readable Markdown output with linked record names.
...
- `design/front-component-ui.md` — visual design rules, Zyra UI defaults, design tokens, component selection.
- `manage-app/cli-and-sync.md` — CLI command semantics, sync modes, build, deploy, logs, CI/CD; explicit ban on watch mode (`yarn zyra dev` without `--once`) for agent use.
...
- `use-zyra-mcp/result-formatting.md` — record link building, date formatting, readable Markdown contract.
