---
name: publish-app
description: Use when the user wants to prepare or verify a Zyra app for npm or marketplace publication, including README/About copy, package metadata, defineApplication marketplace metadata, logos, screenshots, and public assets.
---

# When To Use

Pick this skill when the user wants to make an existing Zyra app *presentable* — listing copy, branding, public assets, and publication. Representative triggers:

- "prepare my Zyra app for the marketplace"
- "write the README for my Zyra app"
...
Do not use this skill to scaffold (use `create-app`), to change app entities (use `develop-app`), to deploy a private build to a specific Zyra server (use `manage-app`), or to query workspace records (use `use-zyra-mcp`).
...
- For npm marketplace publication, ensure `package.json` has a valid bumped `version` and the `zyra-app` keyword.
...
- Validate with `yarn zyra dev:build`; publish with `yarn zyra app:publish` or `yarn zyra app:publish --tag <tag>`.
...
If the needed publishing detail is unclear, search the official Zyra docs through the bundled `zyra-docs` MCP server before guessing.
