...
const assertZyraMcpFormattingContract = (fail) => {
  const skillPath = path.join(PLUGIN_ROOT, 'skills/use-zyra-mcp/SKILL.md');
  const resultFormattingPath = path.join(PLUGIN_ROOT, 'references/use-zyra-mcp/result-formatting.md');
  ...
  const requiredSkillFragments = [
    ...
    'MUST link each display name back to Zyra',
  ...
  for (const fragment of requiredSkillFragments) {
    if (!skill.includes(fragment)) {
      fail(`use-zyra-mcp/SKILL.md is missing formatting contract fragment: ${fragment}`);
    }
  }
...
const assertFrontComponentGuidance = (fail) => {
  ...
  const requiredDevelopSkillFragments = [
    ...
    'Zyra UI imports',
    'Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and Zyra UI component selection',
  ];
  ...
  const requiredFrontComponentFragments = [
    ...
    'Use `zyra-sdk/front-component`',
    'Use `zyra-client-sdk/core` or `zyra-client-sdk/metadata`',
    'Use `zyra-sdk/ui` for Zyra UI components',
    'Do not import from `zyra-ui` directly',
    'ThemeProvider',
    'example-sources/zyra-ui-example.front-component.tsx',
    'themeCssVariables',
    'mocks `zyra-sdk/ui` during manifest extraction',
...
const assertCliGuidanceSplit = (fail) => {
  ...
  const forbiddenAppStructureFragments = [
    ...
    'Do not run `yarn zyra dev:typecheck`',
  ...
  const requiredCliFragments = [
    ...
    'Do not use bare `yarn zyra dev` (watch mode)',
    'yarn zyra dev --once --verbose',
    'yarn zyra remote:list',
    'yarn zyra dev:build',
...
module.exports = {
  assertZyraMcpFormattingContract,
