...
const assertJsonMetadata = (fail) => {
  ...
  const packageJson = readJson('packages/zyra-codex-plugin/package.json');
  const pluginJson = readJson('packages/zyra-codex-plugin/.codex-plugin/plugin.json');
  const mcpJson = readJson('packages/zyra-codex-plugin/.mcp.json');
  ...
  const marketplaceEntry = marketplaceJson?.plugins?.find((entry) => entry.name === 'zyra');

  if (marketplaceJson && !marketplaceEntry) {
    fail('.agents/plugins/marketplace.json includes plugins but not the zyra plugin entry');
  } else if (marketplaceEntry && marketplaceEntry.source?.path !== './packages/zyra-codex-plugin') {
    fail('marketplace zyra source path must be ./packages/zyra-codex-plugin');
  }

  if (fs.existsSync(path.join(REPO_ROOT, 'plugins', 'zyra'))) {
    fail('legacy plugins/zyra path must not exist; use packages/zyra-codex-plugin directly');
  }
};
...
const assertNoBundledMcpConfig = (fail) => {
  ...
  if (fs.existsSync(gitignorePath) && readText(gitignorePath).split(/\r?\n/).includes('.mcp.json')) {
    fail('packages/zyra-codex-plugin/.gitignore must not ignore the public .mcp.json');
  }
...
const assertMarketplaceTemplate = (fail) => {
  const { readJson, readOptionalJson } = createJsonReaders(fail);
  const templatePath = 'packages/zyra-codex-plugin/templates/marketplace.example.json';
  ...
  const zyraEntry = entries.find((entry) => entry?.name === 'zyra');

  if (!zyraEntry) {
    fail(`${templatePath} must include a plugin entry named "zyra"`);
    return;
  }

  if (zyraEntry.version !== pluginJson?.version) {
    fail(`${templatePath} zyra.version must match plugin.json version`);
  }

  if (zyraEntry.source?.path !== './packages/zyra-codex-plugin') {
    fail(`${templatePath} zyra.source.path must be ./packages/zyra-codex-plugin`);
  }

  if (!zyraEntry.policy?.installation) {
    fail(`${templatePath} zyra.policy.installation is required`);
  }

  if (!zyraEntry.policy?.authentication) {
    fail(`${templatePath} zyra.policy.authentication is required`);
  }

  if (zyraEntry.category !== pluginJson?.interface?.category) {
    fail(`${templatePath} zyra.category must match plugin.json interface.category`);
  }
};
