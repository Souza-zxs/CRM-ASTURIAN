...
description: Step-by-step guide to migrate your data from any CRM to Zyra.
---

## Overview

This guide walks you through migrating your data from any CRM to Zyra. The process involves auditing your data, preparing your Zyra workspace, exporting from your current system, and importing into Zyra.
...
Create a mapping document between your current CRM and Zyra:
...
## Step 3: Set Up Your Zyra Workspace

Before importing data, prepare your Zyra workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Zyra before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Zyra.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Zyra already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Zyra's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Zyra
...
- Zyra will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Zyra.
  </Accordion>
...
Contact [contact@zyra.com](mailto:contact@zyra.com) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
