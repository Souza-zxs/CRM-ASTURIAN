...
When you import a file containing a **unique identifier** that matches an existing record, Zyra updates that record instead of creating a duplicate.

| If unique identifier... | Zyra will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or Zyra will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, Zyra will create a new record instead of updating the existing one.
</Warning>
...
Zyra matches records by the unique identifier and updates them with new values.
...
3. Zyra will match on that field
...
| **Case mismatch** | File has `John@acme.com` but Zyra has `john@acme.com` | Creates new record | Export from Zyra to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, Zyra shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
