...
description: Connect your email and calendar accounts to Zyra.
---
...
4. Authorize Zyra to access your Gmail and Google Calendar
...
4. Authorize Zyra to access your Outlook and Microsoft Calendar
5. Configure email sync settings (visibility, auto-creation) → click **Next**
6. Configure calendar sync settings (visibility, auto-creation) → click **Add Account**
7. Your emails and calendar events will start syncing automatically
...
<Note>
**Connecting to a mail or calendar server on a private network**: By default, Zyra refuses outbound connections to private/internal IP addresses (SSRF protection). If your mail or calendar server runs on a local/private IP (e.g. an on-premise server on a LAN), connections to it will be blocked. To allow them, contact our support team, who can set the following environment variable on the server:
...
<Warning>
Only true mailboxes can be connected (e.g., support@domain.com with its own inbox). Email aliases that forward to another mailbox cannot be connected to Zyra.
</Warning>
...
<Note>When enabled, contacts are automatically linked to their Company records based on their email domain. If the company doesn't exist yet, Zyra creates it for you.</Note>
...
Control which email folders sync with Zyra:
...
**Note**: We don't provide a CC email address for selective syncing. Instead, use the Message Folder feature above to achieve the same level of control over which emails sync with Zyra.
