---
title: MCP Server
description: Connect AI assistants to your Zyra workspace using the Model Context Protocol.
---
...
Zyra exposes an [MCP](https://modelcontextprotocol.io/) server so that AI assistants — Claude Desktop, Claude Code, Cursor, ChatGPT, and others — can read and write your CRM data through natural language.

Use your **workspace URL** (the URL you use to access Zyra) as the MCP endpoint. On Zyra Cloud, your workspace URL might be `https://{mycompany}.zyra.com` or a custom domain. The server is available at:

| Environment | MCP Endpoint |
|-------------|-------------|
| **Cloud** | `https://{your-workspace-url}/mcp` (e.g. `https://mycompany.zyra.com/mcp`) |
...
Add this to your MCP client configuration, replacing `{your-workspace-url}` with your workspace host (e.g. `mycompany.zyra.com`):
...
1. Discover Zyra's OAuth metadata via `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`
...
Go to **Settings > AI > More > MCP Server** in Zyra. Choose your authentication method (OAuth or API Key), copy the JSON snippet (it will already use your workspace URL), and paste it into your MCP client's config file.
...
Restart your MCP client (or reload the config). If using OAuth you will be redirected to Zyra to authorize access. If using an API key the connection is immediate.
...
Once connected, the MCP server exposes tools that mirror the Zyra API. The recommended workflow is:
...
**Connection timeout**
- Confirm the MCP endpoint URL is reachable from your machine.
