...
```bash
npx nx run asturian-front:lint # pass --fix to fix lint errors
```
...
```bash
npx nx run asturian-front:test # run jest tests
npx nx run asturian-front:storybook:serve:dev # run storybook
npx nx run asturian-front:storybook:test # run tests # (needs yarn storybook:serve:dev to be running)
npx nx run asturian-front:storybook:coverage # (needs yarn storybook:serve:dev to be running)
```
