...
2. Spawns an isolated Zyra test instance using the `./.github/actions/spawn-zyra-app-dev-test` composite action (the CI equivalent of `yarn zyra docker:start --test`).
...
2. Runs `./.github/actions/deploy-zyra-app` — the CI equivalent of `yarn zyra app:publish --private`.
3. Runs `./.github/actions/install-zyra-app` so the newly deployed version is installed into the target workspace.
...
Both workflows reference reusable actions at `@main`, so action updates are picked up automatically. If you want deterministic builds, replace `@main` with a commit SHA or release tag on each `uses:` line.
