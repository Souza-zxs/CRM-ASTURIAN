---
title: 프런트 컴포넌트
description: Zyra의 UI 내부에서 샌드박스 환경으로 격리된 채 렌더링되는 React 컴포넌트를 빌드하세요.
icon: window-maximize
---

프런트 컴포넌트는 Zyra의 UI 내부에서 직접 렌더링되는 React 컴포넌트입니다. 이들은 Remote DOM을 사용하는 격리된 Web Worker에서 실행됩니다 — 코드는 샌드박스 처리되지만 iframe이 아닌 페이지 내에서 네이티브로 렌더링됩니다.

## 프런트 컴포넌트를 사용할 수 있는 위치

프런트 컴포넌트는 Zyra 내에서 두 위치에 렌더링될 수 있습니다:
...
`yarn zyra dev`로 동기화한 후(또는 일회성으로 `yarn zyra dev --once`를 실행한 경우), 페이지 우측 상단에 빠른 작업이 표시됩니다:
...
컴포넌트가 `null`을 반환하기 때문에, Zyra는 해당 컴포넌트에 대한 컨테이너 렌더링을 생략합니다 — 레이아웃에 빈 공간이 생기지 않습니다. 컴포넌트는 여전히 모든 훅과 호스트 통신 API에 접근할 수 있습니다.

## SDK Command 컴포넌트

`zyra-sdk` 패키지는 헤드리스 프런트 컴포넌트를 위해 설계된 네 가지 Command 헬퍼 컴포넌트를 제공합니다. 각 컴포넌트는 마운트 시 동작을 실행하고, 스낵바 알림을 표시하여 오류를 처리하며, 완료되면 프런트 컴포넌트를 자동으로 언마운트합니다.

`zyra-sdk/command`에서 임포트하세요:
...
`httpRouteTriggerSettings`로 선언된 로직 함수는 `${ZYRA_API_URL}/s\<path>`의 `/s/` 엔드포인트 아래에 노출됩니다. 여러분의 Front 컴포넌트는 `zyra-client-sdk/rest`의 `RestApiClient`를 사용해 해당 라우트를 호출하며, Zyra가 워커에 주입하는 `ZYRA_APP_ACCESS_TOKEN`으로 인증합니다.

`RestApiClient`는 바로 이런 용도로 설계되었습니다. 이는 워커 환경에서 `ZYRA_API_URL`과 `ZYRA_APP_ACCESS_TOKEN`을 읽어 `Authorization: Bearer` 헤더를 추가하고, JSON을 직렬화 및 파싱하며, 토큰이나 URL이 없거나 응답이 2xx가 아닐 경우 `RestApiClientError`를 발생시켜, 여러분이 각 컴포넌트마다 이러한 보일러플레이트를 다시 구현하지 않아도 되도록 해 줍니다.
...
클라이언트에 전달되는 경로는 라우트의 public 경로이며, 로직 함수의 `httpRouteTriggerSettings.path` 앞에 `/s`가 접두사로 붙습니다. `isAuthRequired: true`를 유지하세요. 클라이언트가 컴포넌트용으로 Zyra가 발행한 앱 액세스 토큰을 제공합니다:
...
<Note>
`ZYRA_API_URL`과 `ZYRA_APP_ACCESS_TOKEN`은 자동으로 주입됩니다. 자세한 내용은 [Application variables](#application-variables)을 참고하세요. 비밀 애플리케이션 변수는 Front 컴포넌트에 절대 노출되지 않으므로, API 키 및 기타 민감한 로직은 Front 컴포넌트가 아니라 로직 함수 안에 유지해야 합니다.
</Note>

### RestApiClient 레퍼런스

`RestApiClient`를 `zyra-client-sdk/rest`에서 import하세요. 이는 `CoreApiClient` 및 `MetadataApiClient`와 동일한 클라이언트 패밀리에 속하지만, GraphQL API 대신 앱의 HTTP 라우트를 대상으로 합니다.
...
| `ZYRA_APP_ACCESS_TOKEN` | 앱의 역할 범위로 제한된 단기간 유효한 토큰 |

## 호스트 통신 API

프런트 컴포넌트는 `zyra-sdk`의 함수를 사용해 내비게이션, 모달, 알림을 트리거할 수 있습니다:
...
* **Zyra UI 컴포넌트** — `zyra-sdk/ui`에서 임포트(버튼, 태그, 상태, 칩, 아바타 등)
