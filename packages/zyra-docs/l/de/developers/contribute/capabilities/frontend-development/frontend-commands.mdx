...
```bash
npx nx run asturian-front:lint # Füge --fix hinzu, um Lint-Fehler zu beheben
```
...
```bash
npx nx run asturian-front:test # führe JEST-Tests aus
npx nx run asturian-front:storybook:serve:dev # führe Storybook aus
npx nx run asturian-front:storybook:test # führe Tests aus # benötigt yarn storybook:serve:dev als laufenden Prozess
npx nx run asturian-front:storybook:coverage # benötigt yarn storybook:serve:dev als laufenden Prozess
```
