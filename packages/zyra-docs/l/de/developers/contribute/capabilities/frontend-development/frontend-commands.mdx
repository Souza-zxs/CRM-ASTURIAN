...
```bash
npx nx run zyra-front:lint # Füge --fix hinzu, um Lint-Fehler zu beheben
```
...
```bash
npx nx run zyra-front:test # führe JEST-Tests aus
npx nx run zyra-front:storybook:serve:dev # führe Storybook aus
npx nx run zyra-front:storybook:test # führe Tests aus # benötigt yarn storybook:serve:dev als laufenden Prozess
npx nx run zyra-front:storybook:coverage # benötigt yarn storybook:serve:dev als laufenden Prozess
```
