...
2. Startet eine isolierte Zyra-Testinstanz mithilfe der Composite-Action `./.github/actions/spawn-zyra-app-dev-test` (das CI-Äquivalent zu `yarn zyra docker:start --test`).
...
2. Führt `./.github/actions/deploy-zyra-app` aus — das CI-Äquivalent zu `yarn zyra app:publish --private`.
3. Führt `./.github/actions/install-zyra-app` aus, damit die neu bereitgestellte Version in den Ziel-Workspace installiert wird.
...
Beide Workflows verweisen auf wiederverwendbare Actions mit `@main`, sodass Aktualisierungen der Actions automatisch übernommen werden. Wenn Sie deterministische Builds möchten, ersetzen Sie `@main` in jeder `uses:`-Zeile durch eine Commit-SHA oder einen Release-Tag.
