...
* Verwenden Sie `serverVariables` (nicht `applicationVariables`) — OAuth-Anmeldedaten gelten serverweit und es gibt eine OAuth-App pro Zyra-Server.
...
4. Öffnen Sie die installierte App in Zyra als Serveradministrator → setzen Sie die Werte in den entsprechenden `serverVariables`.
