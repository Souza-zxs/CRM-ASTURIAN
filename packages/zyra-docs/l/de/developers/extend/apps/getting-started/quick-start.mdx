...
<Note>
Wenn Docker nicht installiert ist oder nicht läuft, zeigt das Scaffolding-Tool den richtigen Startbefehl für Ihr Betriebssystem an. Sobald Docker läuft, können Sie mit `yarn zyra docker:start` fortfahren — ein erneutes Scaffolding ist nicht nötig.
</Note>
...
| `yarn zyra dev --once --dry-run` | Erstellt und gibt die Metadatenänderungen aus **ohne sie anzuwenden**.             | Prüfen Sie, was eine Synchronisierung ändern würde, bevor Sie sie ausführen. |
