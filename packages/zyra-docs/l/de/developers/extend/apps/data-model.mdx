...
* Das zur Laufzeit als `ZYRA_APP_ACCESS_TOKEN` injizierte Token wird aus dieser Rolle abgeleitet.
...
* Ein funktionierendes Beispiel finden Sie unter: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Sie können mit `yarn zyra add` neue Objekte erzeugen; der Assistent führt Sie durch Benennung, Felder und Beziehungen.

<Note>
**Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt Zyra automatisch Standardfelder hinzu
...
Verwenden Sie `defineField()`, um Objekten, die Ihnen nicht gehören — etwa Standardobjekten von Zyra (Person, Company usw.) — Felder hinzuzufügen oder Objekten aus anderen Apps. Im Gegensatz zu Inline-Feldern in `defineObject()` benötigen eigenständige Felder einen `objectUniversalIdentifier`, um anzugeben, welches Objekt sie erweitern:
...
Hauptpunkte:
* Der `objectUniversalIdentifier` identifiziert das Zielobjekt. Für Standardobjekte verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, die aus `zyra-sdk` exportiert werden.
...
Relationen verbinden Objekte miteinander. In Zyra sind Relationen stets **bidirektional** — Sie definieren beide Seiten, und jede Seite referenziert die andere.
...
Um eine Relation mit einem integrierten Zyra-Objekt (Person, Company usw.) zu erstellen, verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Entitäten mit `yarn zyra add` erstellen
...
| Feld                   | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Logikfunktion          | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Frontend-Komponente    | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rolle                  | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Skill                  | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Agent                  | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Ansicht                | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Navigationsmenüeintrag | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Seitenlayout           | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Was der Scaffolder generiert

Jeder Entitätstyp hat seine eigene Vorlage. Zum Beispiel fragt `yarn zyra add object` nach:
