...
Dabei werden TypeScript-Quelltexte kompiliert, Logikfunktionen und Frontend-Komponenten transpiliert und alles in `.zyra/output/` geschrieben. Fügen Sie `--tarball` hinzu, um zusätzlich ein `.tgz`-Paket für die manuelle Verteilung oder den Deploy-Befehl zu erzeugen.
...
```bash filename="Terminal"
yarn zyra remote add --api-url https://your-zyra-server.com --as production
```
...
# yarn zyra deploy --remote production
```
...
2. Führen Sie `yarn zyra deploy` aus (oder `yarn zyra deploy --remote production`)
...
<Note>
Der Server ist die maßgebliche Prüfinstanz — er validiert `engines.zyra` sowohl beim Tarball-Upload als auch bei der Workspace-Installation. Auch wenn Sie ein Tarball außerhalb des regulären Prozesses bereitstellen oder aus dem Marketplace installieren, erzwingt der Server weiterhin die Kompatibilität.
</Note>
...
2. Startet eine isolierte Zyra-Testinstanz mithilfe der Composite-Action `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (das CI-Äquivalent zu `yarn zyra server start --test`).
...
2. Führt `zyrahq/zyra/.github/actions/deploy-zyra-app@main` aus — das CI-Äquivalent zu `yarn zyra deploy`.
...
```bash filename="Terminal"
yarn zyra server catalog-sync
# To target a specific remote:
# yarn zyra server catalog-sync --remote production
```
...
Für andere CI-Systeme (GitLab CI, CircleCI usw.) gelten die gleichen drei Befehle: `yarn install`, `yarn zyra build` und anschließend `npm publish` aus `.zyra/output`.
...
Um eine neuere Version zu installieren, stellen Sie sie zuerst bereit oder veröffentlichen Sie sie und führen Sie dann `yarn zyra install` erneut aus.
</Note>
