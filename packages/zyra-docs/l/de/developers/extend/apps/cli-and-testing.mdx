...
* **In Builds enthalten** — `yarn zyra build` bündelt alle öffentlichen Assets in der Distributionsausgabe.

### Zugriff auf öffentliche Assets mit `getPublicAssetUrl`

Verwenden Sie den Helper `getPublicAssetUrl` aus `zyra-sdk`, um die vollständige URL einer Datei in Ihrem `public/`-Verzeichnis zu erhalten. Dies funktioniert sowohl in Logikfunktionen als auch in Frontend-Komponenten.
...
In beiden Umgebungen stehen `zyra-client-sdk/core` und `zyra-client-sdk/metadata` als vorab bereitgestellte Module zur Verfügung — sie werden nicht gebündelt, sondern zur Laufzeit vom Server aufgelöst.

## Ihre App testen

Das SDK stellt programmgesteuerte APIs bereit, mit denen Sie Ihre App aus Testcode heraus bauen, bereitstellen, installieren und deinstallieren können. In Kombination mit [Vitest](https://vitest.dev/) und den typisierten API-Clients können Sie Integrationstests schreiben, die prüfen, dass Ihre App End-to-End gegen einen echten Zyra-Server funktioniert.
...
Der Subpfad `zyra-sdk/cli` exportiert Funktionen, die Sie direkt aus Testcode aufrufen können:
...
Stellen Sie sicher, dass Ihr lokaler Zyra-Server läuft, und führen Sie dann Folgendes aus:
...
### Funktionen ausführen (`yarn zyra exec`)
...
### Funktionsprotokolle ansehen (`yarn zyra logs`)
...
<Note>
Dies unterscheidet sich von `yarn zyra server logs`, das die Docker-Container-Logs anzeigt. `yarn zyra logs` zeigt die Funktionsausführungsprotokolle Ihrer App vom Zyra-Server.
</Note>

### Eine App deinstallieren (`yarn zyra uninstall`)
...
2. Startet einen temporären Zyra-Server mit der Aktion `zyrahq/zyra/.github/actions/spawn-zyra-docker-image`
3. Installiert Abhängigkeiten mit `yarn install --immutable`
4. Führt `yarn test` aus, wobei `ZYRA_API_URL` und `ZYRA_API_KEY` aus den Aktionsausgaben injiziert werden.
...
Sie müssen keine Secrets konfigurieren — die Aktion `spawn-zyra-docker-image` startet einen flüchtigen Zyra-Server direkt im Runner und gibt die Verbindungsdetails aus. Das Secret `GITHUB_TOKEN` wird automatisch von GitHub bereitgestellt.

Um eine bestimmte Zyra-Version statt `latest` festzulegen, ändern Sie die Umgebungsvariable `ZYRA_VERSION` oben im Workflow.
