...
description: Erstellen Sie React-Komponenten, die innerhalb der Zyra-UI gerendert werden und durch eine Sandbox isoliert sind.
icon: window-maximize
---

Front-Komponenten sind React-Komponenten, die direkt innerhalb der Zyra-UI gerendert werden. Sie laufen in einem **isolierten Web Worker** unter Verwendung von Remote DOM — Ihr Code wird in einer Sandbox ausgeführt, rendert jedoch nativ auf der Seite, nicht in einem iframe.

## Wo Front-Komponenten verwendet werden können

Front-Komponenten können an zwei Stellen innerhalb von Zyra gerendert werden:
...
Nach dem Synchronisieren mit `yarn zyra dev` (oder durch einmaliges Ausführen von `yarn zyra dev --once`) erscheint die Schnellaktion oben rechts auf der Seite:
...
Da die Komponente `null` zurückgibt, überspringt Zyra das Rendern eines Containers dafür — im Layout entsteht kein Leerraum. Die Komponente hat dennoch Zugriff auf alle Hooks und die Host-Kommunikations-API.

## SDK-Command-Komponenten

Das Paket `zyra-sdk` stellt vier Command-Hilfskomponenten bereit, die für Headless-Front-Komponenten ausgelegt sind. Jede Komponente führt beim Mounten eine Aktion aus, behandelt Fehler durch Anzeige einer Snackbar-Benachrichtigung und unmountet die Front-Komponente nach Abschluss automatisch.

Importieren Sie sie aus `zyra-sdk/command`:
...
Eine mit `httpRouteTriggerSettings` deklarierte Logikfunktion wird unter dem `/s/`-Endpunkt unter `${ZYRA_API_URL}/s\<path>` bereitgestellt. Ihre Front-Komponente ruft diese Route mit dem `RestApiClient` aus `zyra-client-sdk/rest` auf, der sich mit dem `ZYRA_APP_ACCESS_TOKEN` authentifiziert, das Zyra in den Worker injiziert.

Der `RestApiClient` ist genau dafür gemacht. Er liest `ZYRA_API_URL` und `ZYRA_APP_ACCESS_TOKEN` aus der Worker-Umgebung, hängt den Header `Authorization: Bearer` an, serialisiert und parst JSON und löst einen `RestApiClientError` aus, wenn das Token oder die URL fehlt oder die Antwort kein 2xx-Status ist – sodass Sie diesen Boilerplate-Code nicht in jeder Komponente neu implementieren müssen.
...
Der an den Client übergebene Pfad ist der öffentliche Pfad der Route – der `httpRouteTriggerSettings.path` der Logikfunktion, der mit `/s` präfixiert ist. Belasse `isAuthRequired: true`; der Client stellt das App-Zugriffstoken bereit, das Zyra für deine Komponente ausstellt:
...
<Note>
`ZYRA_API_URL` und `ZYRA_APP_ACCESS_TOKEN` werden automatisch injiziert – siehe [Anwendungsvariablen](#application-variables). Da geheime Anwendungsvariablen niemals in Front-Komponenten offengelegt werden, sollten API-Schlüssel und andere sensible Logik in der Logikfunktion verbleiben und nicht in der Front-Komponente.
</Note>

### RestApiClient-Referenz

Importiere `RestApiClient` aus `zyra-client-sdk/rest`. Er gehört zur gleichen Client-Familie wie `CoreApiClient` und `MetadataApiClient`, zielt jedoch auf die HTTP-Routen deiner App statt auf die GraphQL-API.
...
| `ZYRA_APP_ACCESS_TOKEN` | Kurzlebiges Token mit dem Geltungsbereich der Rolle Ihrer App |

## Host-Kommunikations-API

Front-Komponenten können Navigation, Modals und Benachrichtigungen mittels Funktionen aus `zyra-sdk` auslösen:
...
* **Zyra-UI-Komponenten** — Import aus `zyra-sdk/ui` (Button, Tag, Status, Chip, Avatar und mehr)
