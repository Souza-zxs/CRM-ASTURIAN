---
title: Zyra-Apps
description: Zyra-Anpassungen als Code erstellen und verwalten.
---
...
* **Frontend-Komponenten** — React-Komponenten, die innerhalb der UI von Zyra gerendert werden (Datensatzseiten, Befehlsmenü, Seitenpanels)
* **KI-Fähigkeiten und -Agenten** — erweitern Sie die KI von Zyra mit benutzerdefinierten Fähigkeiten
...
Dies erstellt das Grundgerüst für eine neue App, startet optional einen lokalen Zyra-Server und beginnt, Ihre Dateien auf Änderungen zu überwachen. Den vollständigen Ablauf finden Sie im Leitfaden [Erste Schritte](/l/de/developers/extend/apps/getting-started).
...
| `defineFrontComponent()`           | React-Komponenten in der UI von Zyra           |
...
1. **`yarn zyra dev`** — überwacht Quelldateien, baut bei Änderungen neu, synchronisiert mit dem Server, generiert typisierte API-Clients
2. **`yarn zyra build`** — erzeugt ein auslieferbares Build
3. **`yarn zyra deploy`** — stellt auf einem entfernten Zyra-Server bereit
4. **`yarn zyra add`** — erstellt interaktiv das Gerüst für eine neue Entität

### CLI-Referenz

```bash filename="Terminal"
yarn zyra help          # Alle Befehle auflisten
yarn zyra server start  # Lokalen Dev-Server starten
yarn zyra remote add    # Mit einem Zyra-Server verbinden
yarn zyra exec -n fn    # Eine Logikfunktion ausführen
yarn zyra logs -n fn    # Funktionsprotokolle streamen
```
