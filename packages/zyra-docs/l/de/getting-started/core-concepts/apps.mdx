...
title: Apps
icon: cube
description: Erweitern Sie Zyra mit Code – benutzerdefinierte Objekte, serverseitige Logik, UI-Komponenten und KI-Agenten, alles als TypeScript-Pakete.
---

Die meisten CRMs bieten Ihnen ein Konfigurationspanel. Zyra bietet Ihnen eine Plattform. Apps sind die Art und Weise, wie Entwickler Zyra über das hinaus erweitern, was die UI bietet – sie definieren Datenmodelle, serverseitige Logik, UI-Komponenten und KI-Funktionen als Code und stellen diese dann in einem oder mehreren Workspaces bereit.
...
Eine App ist ein TypeScript-Paket, das **Entitäten** mit dem `zyra-sdk` deklariert:
...
| **Frontend-Komponenten**   | Sandboxed React-Komponenten, die in der UI von Zyra gerendert werden (Seitenleiste, Widgets, Befehlsmenü) |
...
Sie schreiben Ihre App als TypeScript-Projekt auf Ihrem Rechner. Die CLI beobachtet Ihre Quelldateien und synchronisiert sie in Echtzeit mit einem laufenden Zyra-Server – bearbeiten Sie eine Datei und sehen Sie die Änderung innerhalb einer Sekunde in der UI. Der typisierte API-Client wird automatisch neu generiert, wenn sich das Schema ändert. Wenn Sie fertig sind, pusht `yarn zyra app:publish --private` auf einen Produktionsserver, oder `yarn zyra app:publish` listet Ihre App auf npm und im Zyra-Marktplatz.
