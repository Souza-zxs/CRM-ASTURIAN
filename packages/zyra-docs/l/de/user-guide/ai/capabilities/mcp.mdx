---
title: MCP-Server
description: Verbinden Sie KI-Assistenten mit Ihrem Zyra-Workspace über das Model Context Protocol.
---
...
Zyra stellt einen [MCP](https://modelcontextprotocol.io/)-Server bereit, damit KI-Assistenten — Claude Desktop, Claude Code, Cursor, ChatGPT und andere — Ihre CRM-Daten in natürlicher Sprache lesen und schreiben können.

Verwenden Sie Ihre **Workspace-URL** (die URL, mit der Sie auf Zyra zugreifen) als MCP-Endpunkt. In Zyra Cloud kann Ihre Workspace-URL `https://{mycompany}.zyra.com` oder eine benutzerdefinierte Domain sein. Der Server ist verfügbar unter:

| Umgebung          | MCP-Endpunkt                                                                  |
| ----------------- | ----------------------------------------------------------------------------- |
| **Cloud**         | `https://{your-workspace-url}/mcp` (z. B. `https://mycompany.zyra.com/mcp`) |
...
Fügen Sie dies zu Ihrer MCP-Client-Konfiguration hinzu und ersetzen Sie `{your-workspace-url}` durch den Host Ihrer Workspace-URL (z. B. `mycompany.zyra.com`):
...
1. Die OAuth-Metadaten von Zyra über `/.well-known/oauth-protected-resource` und `/.well-known/oauth-authorization-server` ermitteln
...
Gehen Sie in Zyra zu **Settings > AI > More > MCP Server**. Wählen Sie Ihre Authentifizierungsmethode (OAuth oder API-Schlüssel), kopieren Sie das JSON-Snippet (es verwendet bereits Ihre Workspace-URL) und fügen Sie es in die Konfigurationsdatei Ihres MCP-Clients ein.
...
Starten Sie Ihren MCP-Client neu (oder laden Sie die Konfiguration neu). Bei Verwendung von OAuth werden Sie zu Zyra weitergeleitet, um den Zugriff zu autorisieren. Bei Verwendung eines API-Schlüssels wird die Verbindung sofort hergestellt.
...
Nach der Verbindung stellt der MCP-Server Tools bereit, die die Zyra-API widerspiegeln. Der empfohlene Workflow ist:
...
Für selbstgehostete Instanzen ersetzen Sie `{your-workspace-url}` durch die URL Ihres Servers. Stellen Sie sicher, dass `SERVER_URL` in Ihrer Umgebung der öffentlichen URL Ihrer Zyra-Instanz entspricht — dieser Wert wird verwendet, um die OAuth-Discovery-Metadaten zu generieren.

```bash
SERVER_URL=https://zyra.yourcompany.com
```
