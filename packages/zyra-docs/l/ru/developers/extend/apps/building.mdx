---
title: Архитектура
description: Как работают приложения Zyra — изоляция в песочнице, жизненный цикл и базовые элементы.
icon: sitemap
---
...
Приложение — это набор **сущностей**, объявленных с помощью функций `defineEntity()` из пакета `zyra-sdk`. SDK обнаруживает эти объявления посредством анализа AST на этапе сборки и формирует **манифест** — полное описание того, что ваше приложение добавляет в рабочее пространство.
...
| **Компонент фронтенда**  | Изолированный в песочнице интерфейс React на странице Zyra | [Компоненты фронтенда](/l/ru/developers/extend/apps/front-components) |
...
* **`yarn zyra build`** — компилирует TypeScript, упаковывает логические функции и фронтенд-компоненты с помощью esbuild и формирует манифест.
...
<CardGroup cols={2}>
  ...
  <Card title="Компоненты фронтенда" icon="window-maximize" href="/l/ru/developers/extend/apps/front-components">
    Изолированные в песочнице компоненты React внутри интерфейса Zyra.
  </Card>
