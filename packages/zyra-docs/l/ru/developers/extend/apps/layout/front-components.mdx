...
description: Создавайте компоненты React, которые отображаются внутри интерфейса Zyra в изолированной песочнице.
icon: window-maximize
---

Фронтенд-компоненты — это компоненты React, которые отображаются непосредственно внутри интерфейса Zyra. Они выполняются в изолированном Web Worker с использованием Remote DOM — ваш код изолирован (sandboxed), но рендерится нативно на странице, а не в iframe.

## Где можно использовать фронт-компоненты

Фронт-компоненты могут отображаться в двух местах внутри Zyra:
...
const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This component renders inside Zyra.</p>
    </div>
  );
};
...
После синхронизации с помощью `yarn zyra dev` (или однократного запуска `yarn zyra dev --once`) быстрое действие появится в правом верхнем углу страницы:
...
import { defineFrontComponent } from 'zyra-sdk/define';
import { useRecordId, enqueueSnackbar } from 'zyra-sdk/front-component';
...
Поскольку компонент возвращает `null`, Zyra пропускает рендеринг контейнера для него — в макете не появляется пустое место. Компонент по-прежнему имеет доступ ко всем хукам и API взаимодействия с хостом.

## Компоненты SDK Command

Пакет `zyra-sdk` предоставляет четыре вспомогательных компонента Command, предназначенных для headless фронт-компонентов. Каждый компонент выполняет действие при монтировании, обрабатывает ошибки, показывая уведомление snackbar, и автоматически размонтирует фронт-компонент по завершении.

Импортируйте их из `zyra-sdk/command`:
...
import { defineFrontComponent } from 'zyra-sdk/define';
import { Command } from 'zyra-sdk/command';
import { CoreApiClient } from 'zyra-sdk/clients';
...
import { defineFrontComponent } from 'zyra-sdk/define';
import { CommandModal } from 'zyra-sdk/command';
...
Логическая функция, объявленная с `httpRouteTriggerSettings`, доступна по эндпоинту `/s/` по адресу `${ZYRA_API_URL}/s\<path>`. Ваш front-компонент вызывает этот маршрут с помощью `RestApiClient` из `zyra-client-sdk/rest`, который аутентифицируется с использованием `ZYRA_APP_ACCESS_TOKEN`, который Zyra внедряет в worker.

`RestApiClient` создан именно для этого. Он считывает `ZYRA_API_URL` и `ZYRA_APP_ACCESS_TOKEN` из окружения worker, добавляет заголовок `Authorization: Bearer`, сериализует и парсит JSON и выбрасывает `RestApiClientError`, когда токен или URL отсутствуют или ответ не является 2xx — чтобы вам не приходилось реализовывать этот шаблонный код в каждом компоненте.
...
import { defineFrontComponent } from 'zyra-sdk/define';
import { Command } from 'zyra-sdk/command';
import { RestApiClient } from 'zyra-client-sdk/rest';
...
Путь, передаваемый клиенту, — это общедоступный путь маршрута: значение `httpRouteTriggerSettings.path` для логической функции с префиксом `/s`. Сохраните `isAuthRequired: true`; клиент передает токен доступа к приложению, который Zyra выпускает для вашего компонента:
...
<Note>
`ZYRA_API_URL` и `ZYRA_APP_ACCESS_TOKEN` внедряются автоматически — см. [переменные приложения](#application-variables). Поскольку секретные переменные приложения никогда не раскрываются front-компонентам, храните ключи API и другую конфиденциальную логику в логической функции, а не во front-компоненте.
</Note>

### Справочник по RestApiClient

Импортируйте `RestApiClient` из `zyra-client-sdk/rest`. Он принадлежит к тому же семейству клиентов, что и `CoreApiClient` и `MetadataApiClient`, но предназначен для HTTP-маршрутов вашего приложения вместо GraphQL API.
...
```tsx
import { RestApiClient, RestApiClientError } from 'zyra-client-sdk/rest';
...
| `ZYRA_APP_ACCESS_TOKEN` | Краткоживущий токен с областью действия, ограниченной ролью вашего приложения |

## API взаимодействия с хостом

Компоненты фронтенда могут вызывать навигацию, модальные окна и уведомления с помощью функций из `zyra-sdk`:
...
import { enqueueSnackbar, closeSidePanel } from 'zyra-sdk/front-component';
import { CoreApiClient } from 'zyra-sdk/clients';
...
import { defineFrontComponent, numberOfSelectedRecords } from 'zyra-sdk/define';
...
```tsx
import { defineFrontComponent } from 'zyra-sdk/define';
import { getPublicAssetUrl } from 'zyra-sdk/utils';
...
* **Компоненты Zyra UI** — импорт из `zyra-sdk/ui` (Button, Tag, Status, Chip, Avatar и другие)
...
```tsx
import { defineFrontComponent } from 'zyra-sdk/define';
import { Button, Tag, Status } from 'zyra-sdk/ui';
