...
```bash filename="Terminal"
yarn zyra dev:function:exec --preInstall
```
...
* **Не выполняется в режиме разработки**: как и post-install, процесс установки полностью пропускается для локально зарегистрированных приложений, поэтому pre-install никогда не запускается при `yarn zyra dev`. Используйте `yarn zyra dev:function:exec --preInstall`, чтобы запустить это вручную.
