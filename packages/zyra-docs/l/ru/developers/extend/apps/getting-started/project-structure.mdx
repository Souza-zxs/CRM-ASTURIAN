...
description: Что находится внутри сгенерированного приложения Zyra — файлы, папки и назначение каждого из них.
icon: folder-tree
---

Новое приложение, сгенерированное с помощью `npx create-zyra-app`, выглядит так:
...
Оба пакета Zyra SDK должны находиться в `devDependencies`, а не в `dependencies`:
...
* **`zyra-sdk`** поставляет `zyra` CLI и инструменты сборки/скэффолдинга. Он выполняется только на этапах разработки и сборки и никогда не импортируется во время выполнения опубликованного приложения.
* **`zyra-client-sdk`** *действительно* импортируется кодом вашего приложения (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`), но Zyra предоставляет его во время выполнения — функции логики получают его из сгенерированного слоя SDK, а фронтенд‑компоненты получают его из модулей, отдаваемых сервером. Установленная вами копия используется только для проверки типов и сборки на этапе деплоя, поэтому её не нужно включать в развертываемый бандл.

Если любой из пакетов находится в `dependencies`, он попадает в бандл среды выполнения установленного приложения, где он является лишним грузом. `zyra build` выдаёт предупреждение, если любой из них всё ещё указан в `dependencies`.
