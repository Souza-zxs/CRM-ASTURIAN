...
export default defineApplication({
  universalIdentifier: '...',
  displayName: 'Linear',
  description: 'Connect Linear to Zyra.',
  defaultRoleUniversalIdentifier: '...',
  // OAuth client credentials live on the app registration (one OAuth app per
  // Zyra server, configured by the admin) — not per-workspace. Declare them
...
* Используйте `serverVariables` (не `applicationVariables`) — учетные данные OAuth являются общими для сервера, и на каждом сервере Zyra используется одно приложение OAuth.
...
```
https://<your-zyra-server>/apps/oauth/callback
```
...
4. Откройте установленное приложение в Zyra под учетной записью администратора сервера → задайте значения в соответствующих `serverVariables`.
