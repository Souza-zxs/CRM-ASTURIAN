---
title: Модель данных
description: Определяйте объекты, поля, роли и метаданные приложения с помощью Zyra SDK.
icon: database
---

Пакет `zyra-sdk` предоставляет функции `defineEntity` для определения модели данных вашего приложения. Вы должны использовать `export default defineEntity({...})`, чтобы SDK обнаруживал ваши сущности. Эти функции проверяют вашу конфигурацию на этапе сборки и обеспечивают автодополнение в IDE и безопасность типов.
...
* Токен времени выполнения, подставляемый как `ZYRA_APP_ACCESS_TOKEN`, формируется из этой роли.
...
import { defineRole, PermissionFlag } from 'zyra-sdk/define';
...
* См. рабочий пример: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Вы можете сгенерировать новые объекты с помощью `yarn zyra add`, который проведёт вас через выбор именования, полей и связей.

<Note>
**Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, Zyra автоматически добавляет стандартные поля,
...
Используйте `defineField()` для добавления полей к объектам, которые вам не принадлежат — например, к стандартным объектам Zyra (Person, Company и т. д.). или к объектам из других приложений. В отличие от встроенных полей в `defineObject()`, отдельные поля требуют `objectUniversalIdentifier`, чтобы указать, какой объект они расширяют:
...
Основные моменты:
* `objectUniversalIdentifier` определяет целевой объект. Для стандартных объектов используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, экспортируемые из `zyra-sdk`.
...
Отношения связывают объекты между собой. В Zyra отношения всегда двунаправленные — вы определяете обе стороны, и каждая сторона ссылается на другую.
...
Чтобы создать отношение со встроенным объектом Zyra (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Создание заготовок сущностей с помощью `yarn zyra add`
...
```bash filename="Terminal"
yarn zyra add object
yarn zyra add logicFunction
yarn zyra add frontComponent
```
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Объект               | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Поле                 | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Логическая функция   | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Компонент фронтенда  | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Роль                 | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Навык                | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Агент                | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Представление        | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Пункт меню навигации | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Макет страницы       | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Что генерирует скэффолдер

У каждого типа сущности есть свой шаблон. Например, `yarn zyra add object` запрашивает:
...
```bash filename="Terminal"
yarn zyra add logicFunction --path src/custom-folder
```
