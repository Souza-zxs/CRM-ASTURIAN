...
```bash
npx nx run zyra-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run zyra-front:test # запуск тестов jest
npx nx run zyra-front:storybook:serve:dev # запуск storybook
npx nx run zyra-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run zyra-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
