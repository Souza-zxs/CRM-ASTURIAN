...
```bash
npx nx run asturian-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run asturian-front:test # запуск тестов jest
npx nx run asturian-front:storybook:serve:dev # запуск storybook
npx nx run asturian-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run asturian-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
