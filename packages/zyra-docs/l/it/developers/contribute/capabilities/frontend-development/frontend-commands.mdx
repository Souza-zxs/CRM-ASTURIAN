...
```bash
npx nx run asturian-front:lint # aggiungere --fix per correggere gli errori di lint
```

## Traduzioni

```bash
npx nx run asturian-front:lingui:extract
npx nx run asturian-front:lingui:compile
```

### Test

```bash
npx nx run asturian-front:test # eseguire i test jest
npx nx run asturian-front:storybook:serve:dev # eseguire storybook
npx nx run asturian-front:storybook:test # eseguire i test # (è necessario che yarn storybook:serve:dev sia in esecuzione)
npx nx run asturian-front:storybook:coverage # (è necessario che yarn storybook:serve:dev sia in esecuzione)
```
