...
```bash
npx nx run zyra-front:lint # aggiungere --fix per correggere gli errori di lint
```

## Traduzioni

```bash
npx nx run zyra-front:lingui:extract
...
```bash
npx nx run zyra-front:test # eseguire i test jest
npx nx run zyra-front:storybook:serve:dev # eseguire storybook
npx nx run zyra-front:storybook:test # eseguire i test # (è necessario che yarn storybook:serve:dev sia in esecuzione)
npx nx run zyra-front:storybook:coverage # (è necessario che yarn storybook:serve:dev sia in esecuzione)
```
