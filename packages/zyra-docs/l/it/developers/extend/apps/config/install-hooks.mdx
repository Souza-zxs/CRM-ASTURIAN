...
* **Non eseguito in modalità dev**: come per il post-install — il flusso di installazione viene completamente saltato per le app registrate localmente, quindi il pre-install non viene mai eseguito con `yarn zyra dev`. Usa `yarn zyra dev:function:exec --preInstall` per attivarlo manualmente.
