...
2. Avvia un'istanza di test isolata di Zyra utilizzando l'azione composita `./.github/actions/spawn-zyra-app-dev-test` (l'equivalente per la CI di `yarn zyra docker:start --test`).
...
2. Esegue `./.github/actions/deploy-zyra-app` — l'equivalente per la CI di `yarn zyra app:publish --private`.
3. Esegue `./.github/actions/install-zyra-app` in modo che la versione appena distribuita venga installata nello spazio di lavoro di destinazione.
...
Entrambi i workflow fanno riferimento ad azioni riutilizzabili a `@main`, quindi gli aggiornamenti delle azioni vengono recepiti automaticamente. Se desideri build deterministiche, sostituisci `@main` con uno SHA di commit o un tag di release in ciascuna riga `uses:`.
