---
title: Modello dati
description: Definisci oggetti, campi, ruoli e metadati dell'applicazione con Zyra SDK.
icon: database
---

Il pacchetto `zyra-sdk` fornisce le funzioni `defineEntity` per dichiarare il modello di dati della tua applicazione. Devi usare `export default defineEntity({...})` affinché l'SDK rilevi le tue entità. Queste funzioni convalidano la configurazione in fase di build e offrono il completamento automatico nell'IDE e la sicurezza dei tipi.
...
* Il token di runtime iniettato come `ZYRA_APP_ACCESS_TOKEN` è derivato da questo ruolo.
...
* Vedi un esempio funzionante: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Puoi generare nuovi oggetti con `yarn zyra add`, che ti guida nella denominazione, nei campi e nelle relazioni.

<Note>
**I campi base vengono creati automaticamente.** Quando definisci un oggetto personalizzato, Zyra aggiunge automaticamente i campi standard
...
Usa `defineField()` per aggiungere campi a oggetti che non possiedi — come gli oggetti standard di Zyra (Person, Company, ecc.) o oggetti di altre app. A differenza dei campi inline in `defineObject()`, i campi autonomi richiedono un `objectUniversalIdentifier` per specificare quale oggetto estendono:
...
Punti chiave:
* `objectUniversalIdentifier` identifica l'oggetto di destinazione. Per gli oggetti standard, usa `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` esportati da `zyra-sdk`.
...
Le relazioni collegano gli oggetti tra loro. In Zyra, le relazioni sono sempre **bidirezionali** — definisci entrambi i lati e ciascun lato fa riferimento all'altro.
...
Per creare una relazione con un oggetto Zyra integrato (Person, Company, ecc.), usa `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Creazione di entità con lo scaffolding tramite `yarn zyra add`
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Oggetto                      | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Campo                        | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Funzione logica              | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente front-end         | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Ruolo                        | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Abilità                      | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Agente                       | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Vista                        | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Voce del menu di navigazione | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout di pagina             | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Cosa genera lo scaffolder

Ogni tipo di entità ha il proprio template. Ad esempio, `yarn zyra add object` richiede:
