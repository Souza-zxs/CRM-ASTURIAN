...
Questo compila i sorgenti TypeScript, transpila le funzioni di logica e i componenti front-end e scrive tutto in `.zyra/output/`. Aggiungi `--tarball` per produrre anche un pacchetto `.tgz` per la distribuzione manuale o il comando di deploy.
...
2. Esegui `yarn zyra deploy` (oppure `yarn zyra deploy --remote production`)
...
2. Avvia un'istanza di test isolata di Zyra utilizzando l'azione composita `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (l'equivalente per la CI di `yarn zyra server start --test`).
...
2. Esegue `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — l'equivalente per la CI di `yarn zyra deploy`.
...
```bash filename="Terminal"
yarn zyra catalog-sync
# To target a specific remote:
# yarn zyra catalog-sync --remote production
```
...
Per altri sistemi CI (GitLab CI, CircleCI, ecc.), si applicano gli stessi tre comandi: `yarn install`, `yarn zyra build`, quindi `npm publish` da `.zyra/output`.
...
Per installare una versione più recente, effettua prima il deploy o la pubblicazione, quindi riesegui `yarn zyra install`.
</Note>
