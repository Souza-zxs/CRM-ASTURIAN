...
* **Inclusi nelle build** — `yarn zyra build` raggruppa tutti gli asset pubblici nell'output di distribuzione.

### Accedere agli asset pubblici con `getPublicAssetUrl`

Usa l'helper `getPublicAssetUrl` da `zyra-sdk` per ottenere l'URL completo di un file nella tua directory `public/`. Funziona sia nelle funzioni logiche che nei componenti front-end.

**In una funzione logica:**

```ts src/logic-functions/send-invoice.ts
import { defineLogicFunction, getPublicAssetUrl } from 'zyra-sdk/define';
...
### Esecuzione delle funzioni (`yarn zyra exec`)

Esegui manualmente una funzione logica senza attivarla tramite HTTP, cron o evento del database:

```bash filename="Terminal"
# Execute by function name
yarn zyra exec -n create-new-post-card

# Execute by universalIdentifier
yarn zyra exec -u e56d363b-0bdc-4d8a-a393-6f0d1c75bdcf

# Pass a JSON payload
yarn zyra exec -n create-new-post-card -p '{"name": "Hello"}'

# Execute the post-install function
yarn zyra exec --postInstall
```

### Visualizzazione dei log delle funzioni (`yarn zyra logs`)
...
yarn zyra logs -n create-new-post-card

# Filter by universalIdentifier
yarn zyra logs -u e56d363b-0bdc-4d8a-a393-6f0d1c75bdcf
```

<Note>
Questo è diverso da `yarn zyra server logs`, che mostra i log del container Docker. `yarn zyra logs` mostra i log di esecuzione delle funzioni della tua app dal server Zyra.
</Note>

### Disinstallazione di un'app (`yarn zyra uninstall`)
...
yarn zyra remote add --api-url https://your-zyra-server.com --api-key $ZYRA_API_KEY --as my-remote

# List all configured remotes
yarn zyra remote list

# Switch the active remote
yarn zyra remote switch <name>
```
...
2. Avvia un server Zyra temporaneo utilizzando l'azione `zyrahq/zyra/.github/actions/spawn-zyra-docker-image`
