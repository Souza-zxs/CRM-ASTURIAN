...
Le funzioni logiche sono funzioni TypeScript lato server che vengono eseguite sulla piattaforma Zyra. Possono essere attivate da richieste HTTP, pianificazioni cron o eventi del database — e possono anche essere esposte come strumenti per agenti di IA.
...
> ad es. `path: '/post-card/create'` è invocabile su `https://your-zyra-server.com/s/post-card/create`
...
yarn zyra dev:function:exec -n create-new-post-card -p '{"key": "value"}'
```
...
Importa il tipo `RoutePayload` da `zyra-sdk/logic-function`:
...
Per impostazione predefinita, restituire un valore semplice dal tuo handler lo invia come risposta `200` (JSON per gli oggetti, `text/plain` per le stringhe). Per controllare il codice di stato e le intestazioni della risposta, restituisci un oggetto `Response` da `zyra-sdk/logic-function`:
...
export default defineLogicFunction({
  ...
  serverWebhookTriggerSettings: {
    workspaceIdResolver: { source: 'body', path: 'metadata.zyraWorkspaceId' },
...
```
POST https://your-zyra-server.com/webhooks/server/:applicationRegistrationUniversalIdentifier/:logicFunctionUniversalIdentifier
```
...
| Campo      | Valori                                                  | Note                                                                                                                                                       |
| ---------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fonte`    | `body` \| `query` \| `header`                           | `body` legge il JSON analizzato. `query` è il più universale — di solito controlli l'URL di callback che registri, quindi aggiungi `?zyraWorkspaceId=…`. |
| `percorso` | percorso con punti, ad es. `metadata.zyraWorkspaceId` | Limitato a segmenti alfanumerici / `_` / `-`; le chiavi del prototipo vengono rifiutate.                                                                   |
...
* **`toolTriggerSettings`** — rende la funzione individuabile dalle funzionalità di IA di Zyra (chat, MCP, function calling). Usa lo standard JSON Schema, il formato che gli LLM comprendono nativamente.
* **`workflowActionTriggerSettings`** — fa apparire la funzione come un passaggio nel builder visivo dei flussi di lavoro. Usa il ricco `InputSchema` di Zyra affinché il builder possa visualizzare correttamente editor di campi, selettori di variabili ed etichette.
...
* `toolTriggerSettings.inputSchema` e `workflowActionTriggerSettings.inputSchema` sono entrambi opzionali. Se omessi, il builder del manifest li deduce dal codice sorgente dell'handler (JSON Schema per lo strumento di IA, `InputSchema` di Zyra per l'azione del flusso di lavoro). Forniscine uno esplicitamente quando desideri una tipizzazione più ricca — ad esempio, con campi compatibili con `FieldMetadataType` come `CURRENCY` o `RELATION` per il builder dei flussi di lavoro, oppure con campi `description` che l'agente di IA può leggere:
...
Per dichiarare i parametri **una sola volta** e servire entrambi i contesti, definisci un unico JSON Schema (`InputJsonSchema`) e convertilo per l'azione del flusso di lavoro con `jsonSchemaToInputSchema` da `zyra-sdk/logic-function`. `toolTriggerSettings.inputSchema` accetta direttamente il JSON Schema, mentre `workflowActionTriggerSettings.inputSchema` si aspetta l'`InputSchema` di Zyra:

```ts
import { defineLogicFunction } from 'zyra-sdk/define';
import { jsonSchemaToInputSchema, type InputJsonSchema } from 'zyra-sdk/logic-function';
...
<Note>
**Helper di runtime.** `zyra-sdk/utils` riesporta piccoli helper di runtime in modo che gli handler non importino mai direttamente da `zyra-shared`. Per esempio, `isDefined(value)` restituisce `false` sia per `null` che per `undefined` — usalo per restringere in modo sicuro gli input opzionali degli handler, che possono arrivare come `null` a runtime anche quando sono tipizzati come `T | undefined`:

```ts
import { isDefined } from 'zyra-sdk/utils';
...
## Client API tipizzati (zyra-client-sdk)

Il pacchetto `zyra-client-sdk` fornisce due client GraphQL tipizzati per interagire con l'API di Zyra dalle tue funzioni logiche e dai componenti front-end.

| Client              | Importa                      | Endpoint                                                                 | Generato?                  |
| ------------------- | ---------------------------- | ------------------------------------------------------------------------ | -------------------------- |
| `CoreApiClient`     | `zyra-client-sdk/core`     | `/graphql` — dati dello spazio di lavoro (record, oggetti)               | Sì, in fase di dev/build   |
| `MetadataApiClient` | `zyra-client-sdk/metadata` | `/metadata` — configurazione dello spazio di lavoro, caricamenti di file | No, fornito pronto all'uso |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Esegui query e modifica i dati dello spazio di lavoro (record, oggetti)">

`CoreApiClient` è il client principale per interrogare e modificare i dati dello spazio di lavoro. Viene **generato dallo schema del tuo spazio di lavoro** durante `yarn zyra dev` o `yarn zyra dev:build`, quindi è completamente tipizzato per corrispondere ai tuoi oggetti e campi.
...
<Note>
**CoreApiClient viene generato in fase di dev/build.** Se lo usi senza eseguire prima `yarn zyra dev` o `yarn zyra dev:build`, genera un errore. La generazione avviene automaticamente — la CLI esegue l'introspezione dello schema GraphQL del tuo spazio di lavoro e genera un client tipizzato usando `@genql/cli`.
</Note>
...
<Note>
  Quando il tuo codice viene eseguito su Zyra (funzioni logiche o componenti front-end), la piattaforma inietta le credenziali come variabili d'ambiente:

  * `ZYRA_API_URL` — URL di base dell'API di Zyra
  * `ZYRA_APP_ACCESS_TOKEN` — Chiave a breve durata con ambito al ruolo funzione predefinito della tua applicazione
