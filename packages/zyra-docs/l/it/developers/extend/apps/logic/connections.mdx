...
* Usa `serverVariables` (non `applicationVariables`) — le credenziali OAuth sono a livello di server e c'è una sola app OAuth per ogni server Zyra.
...
4. Apri l'app installata in Zyra come amministratore del server → imposta i valori nelle corrispondenti `serverVariables`.
