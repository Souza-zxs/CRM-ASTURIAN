...
description: Guida passo-passo per migrare i tuoi dati da qualsiasi CRM a Zyra.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da qualsiasi CRM a Zyra. Il processo prevede la revisione dei tuoi dati, la preparazione del tuo spazio di lavoro Zyra, l'esportazione dal sistema attuale e l'importazione in Zyra.
...
Crea un documento di mappatura tra il tuo CRM attuale e Zyra:
...
* Il campo di destinazione di Zyra
...
## Passaggio 3: Configura il tuo spazio di lavoro Zyra

Prima di importare i dati, prepara il tuo spazio di lavoro Zyra:
...
Se i tuoi dati includono riferimenti a utenti (Proprietario dell'account, Assegnatario, ecc.), tali utenti devono esistere in Zyra prima dell'importazione. In caso contrario, tali relazioni non possono essere mappate.
</Warning>
...
Apri ogni file CSV esportato in un'applicazione di fogli di calcolo e preparalo per Zyra.

### Rimuovi i duplicati

1. Ordina per il campo univoco (email per Persone, dominio per Aziende)
2. Rimuovi o unisci le righe duplicate
3. Verifica che non esistano già duplicati in Zyra
...
Usa `https://domain.com` (non `domain.com` o `www.domain.com`). Questo corrisponde al formato di Zyra ed evita duplicati quando colleghi la sincronizzazione di email e calendario.
</Warning>
...
2. Usa gli stessi indirizzi email che gli utenti hanno utilizzato per unirsi al tuo spazio di lavoro Zyra
...
* Zyra inizierà a sincronizzare le email con i record dei contatti
...
<AccordionGroup>
  <Accordion title="Posso migrare i flussi di lavoro automaticamente?">
    Al momento no. I flussi di lavoro devono essere ricreati manualmente in Zyra.
  </Accordion>
...
Contatta [contact@zyra.com](mailto:contact@zyra.com) oppure scopri i nostri [Servizi di implementazione](/l/it/user-guide/getting-started/capabilities/implementation-services).
