...
description: Créez des composants React qui s’affichent dans l’interface utilisateur (UI) de Zyra avec une isolation en bac à sable.
icon: window-maximize
---

Les composants frontaux sont des composants React qui s'affichent directement dans l'interface utilisateur de Zyra. Ils s'exécutent dans un **Web Worker isolé** en utilisant Remote DOM — votre code est isolé mais s'affiche nativement dans la page, pas dans un iframe.

## Où les composants frontaux peuvent être utilisés

Les composants frontaux peuvent s'afficher à deux emplacements au sein de Zyra :
...
Après la synchronisation avec `yarn zyra dev` (ou en exécutant une seule fois `yarn zyra dev --once`), l'action rapide apparaît dans le coin supérieur droit de la page :
...
Comme le composant retourne `null`, Zyra n'affiche pas de conteneur pour celui-ci — aucun espace vide n'apparaît dans la mise en page. Le composant a toujours accès à tous les hooks et à l'API de communication de l'hôte.

## Composants Command du SDK

Le package `zyra-sdk` fournit quatre composants utilitaires Command conçus pour les composants frontaux headless. Chaque composant exécute une action au montage, gère les erreurs en affichant une notification snackbar et démonte automatiquement le composant frontal une fois terminé.

Importez-les depuis `zyra-sdk/command` :
...
Une fonction logique déclarée avec `httpRouteTriggerSettings` est exposée sous le point de terminaison `/s/` à `${ZYRA_API_URL}/s\<path>`. Votre composant front appelle cette route avec le `RestApiClient` de `zyra-client-sdk/rest`, qui s’authentifie avec le `ZYRA_APP_ACCESS_TOKEN` que Zyra injecte dans le worker.

Le `RestApiClient` est conçu exactement pour cela. Il lit `ZYRA_API_URL` et `ZYRA_APP_ACCESS_TOKEN` depuis l’environnement du worker, ajoute l’en-tête `Authorization: Bearer`, sérialise et analyse le JSON, et déclenche une `RestApiClientError` lorsque le jeton ou l’URL est manquant ou que la réponse n’est pas de type 2xx — afin que vous n’ayez pas à réimplémenter ce boilerplate dans chaque composant.
...
Le chemin transmis au client est le chemin public de la route — la propriété `httpRouteTriggerSettings.path` de la fonction logique, préfixée par `/s`. Conservez `isAuthRequired: true` ; le client fournit le jeton d’accès de l’application que Zyra émet pour votre composant :
...
<Note>
`ZYRA_API_URL` et `ZYRA_APP_ACCESS_TOKEN` sont injectés automatiquement — voir [Variables d’application](#application-variables). Comme les variables d’application secrètes ne sont jamais exposées aux composants front, conservez les clés d’API et les autres éléments sensibles dans la fonction logique, et non dans le composant front.
</Note>

### Référence de `RestApiClient`

Importez `RestApiClient` depuis `zyra-client-sdk/rest`. Il appartient à la même famille de clients que `CoreApiClient` et `MetadataApiClient`, mais cible les routes HTTP de votre application au lieu de l’API GraphQL.
...
| `ZYRA_APP_ACCESS_TOKEN` | Jeton de courte durée limité au rôle de votre application |

## API de communication de l'hôte

Les composants frontaux peuvent déclencher la navigation, des modales et des notifications en utilisant des fonctions de `zyra-sdk` :
...
* **Composants Zyra UI** — à importer depuis `zyra-sdk/ui` (Button, Tag, Status, Chip, Avatar, etc.)
