...
* Utilisez `serverVariables` (pas `applicationVariables`) — les identifiants OAuth sont au niveau du serveur et il n'y a qu'une application OAuth par serveur Zyra.
...
4. Ouvrez l'application installée dans Zyra en tant qu'administrateur du serveur → définissez les valeurs sur les `serverVariables` correspondantes.
