---
title: Génération
description: Générez des fichiers d'entité de manière interactive avec `yarn zyra dev:add` — objets, champs, vues, fonctions logiques et plus encore.
...
| ----------------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Objet                         | `yarn zyra dev:add object`             | `src/objects/\<name>.ts`               |
| Champ                         | `yarn zyra dev:add field`              | `src/fields/\<name>.ts`                |
| Fonction logique              | `yarn zyra dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Composant frontal             | `yarn zyra dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rôle                          | `yarn zyra dev:add role`               | `src/roles/\<name>.ts`                 |
| Compétence                    | `yarn zyra dev:add skill`              | `src/skills/\<name>.ts`                |
| Agent                         | `yarn zyra dev:add agent`              | `src/agents/\<name>.ts`                |
| Vue                           | `yarn zyra dev:add view`               | `src/views/\<name>.ts`                 |
| Élément de menu de navigation | `yarn zyra dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Mise en page                  | `yarn zyra dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Ce que génère l'outil de génération

Chaque type d'entité a son propre modèle. Par exemple, `yarn zyra dev:add object` demande :
