---
title: Applications Zyra
description: Créez et gérez les personnalisations Zyra sous forme de code.
---
...
Les applications vous permettent de créer et de gérer des personnalisations Zyra **sous forme de code**. Au lieu de tout configurer via l’interface utilisateur, vous définissez votre modèle de données et vos fonctions logiques dans le code — ce qui accélère la création, la maintenance et le déploiement sur plusieurs espaces de travail.
...
* Un espace de travail Zyra et une clé API (créez-en une sur https://app.zyra.com/settings/api-webhooks)
...
Voir aussi : les pages de référence CLI pour [create-zyra-app](https://www.npmjs.com/package/create-zyra-app) et [zyra-sdk CLI](https://www.npmjs.com/package/zyra-sdk).

## Structure du projet (générée)

Lorsque vous exécutez `npx create-zyra-app@latest my-zyra-app`, l’outil de scaffolding :

* Copie une application de base minimale dans `my-zyra-app/`
* Ajoute une dépendance locale à `zyra-sdk` et la configuration Yarn 4
* Crée des fichiers de configuration et des scripts reliés à la CLI `zyra`
...
* **package.json** : Déclare le nom de l’application, la version, les moteurs (Node 24+, Yarn 4), et ajoute `zyra-sdk` ainsi que des scripts comme `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` et `auth:login` qui délèguent à la CLI locale `zyra`.
...
* `yarn app:dev` génère automatiquement le client Zyra typé dans `node_modules/zyra-sdk/generated`.
...
Vos identifiants sont stockés par utilisateur dans `~/.zyra/config.json`. Vous pouvez gérer plusieurs profils et basculer entre eux.
...
Le paquet zyra-sdk fournit des blocs de construction typés et des fonctions utilitaires que vous utilisez dans votre application. Voici les éléments clés que vous manipulerez le plus souvent.
...
<Note>
  **Les champs de base sont créés automatiquement.** Lorsque vous définissez un objet personnalisé, Zyra ajoute automatiquement des champs standard tels que `name`, `createdAt`, `updatedAt`, `createdBy`, `position` et `deletedAt`. Vous n'avez pas besoin de les définir dans votre tableau `fields` — ajoutez uniquement vos champs personnalisés.
</Note>
...
* La clé API d’exécution injectée sous `ZYRA_API_KEY` est dérivée de ce rôle de fonction par défaut.
...
* Voir un exemple fonctionnel dans l’application Hello World : [`packages/zyra-apps/hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
Lorsqu’un déclencheur de route appelle votre fonction logique, elle reçoit un objet `RoutePayload` qui suit le format AWS HTTP API v2. Importez le type depuis `zyra-sdk` :
...
`yarn app:dev` génère automatiquement le client Zyra typé dans `node_modules/zyra-sdk/generated`. Utilisez-le dans vos fonctions :
...
Lorsque votre fonction s’exécute sur Zyra, la plateforme injecte des identifiants sous forme de variables d’environnement avant l’exécution de votre code :

* `ZYRA_API_URL` : URL de base de l’API Zyra ciblée par votre application.
* `ZYRA_API_KEY` : Clé de courte durée limitée au rôle de fonction par défaut de votre application.

Notes:

* Vous n’avez pas besoin de passer l’URL ou la clé API au client généré. Il lit `ZYRA_API_URL` et `ZYRA_API_KEY` depuis process.env à l’exécution.
...
Découvrez un exemple minimal de bout en bout qui démontre des objets, des fonctions et plusieurs déclencheurs [ici](https://github.com/zyrahq/zyra/tree/main/packages/zyra-apps/hello-world) :

## Configuration manuelle (sans l’outil de scaffolding)

Même si nous recommandons d’utiliser `create-zyra-app` pour une expérience de démarrage optimale, vous pouvez également configurer un projet manuellement. N’installez pas la CLI globalement. Ajoutez plutôt `zyra-sdk` comme dépendance locale et reliez des scripts dans votre package.json :
...
* Impossible de se connecter au serveur : vérifiez l’URL de l’API et que le serveur Zyra est accessible.
