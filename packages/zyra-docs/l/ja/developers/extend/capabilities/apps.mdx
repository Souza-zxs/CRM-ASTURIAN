---
title: Zyra アプリ
description: Zyra のカスタマイズをコードとして構築・管理します。
---
...
Apps を使うと、Zyra のカスタマイズを**コードとして**構築・管理できます。 Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* Node.js 24+ と Yarn 4
* Zyra のワークスペースと API キー（https://app.zyra.com/settings/api-webhooks で作成）
...
参考: [create-zyra-app](https://www.npmjs.com/package/create-zyra-app) および [zyra-sdk CLI](https://www.npmjs.com/package/zyra-sdk) の CLI リファレンスページをご覧ください。

## プロジェクト構成（スキャフォルド作成）

`npx create-zyra-app@latest my-zyra-app` を実行すると、スキャフォルダーは次を行います：

* 最小限のベースアプリケーションを `my-zyra-app/` にコピーします
* ローカルの `zyra-sdk` 依存関係と Yarn 4 の設定を追加します
* `zyra` CLI と連携する設定ファイルとスクリプトを作成します
...
* **package.json**: Declares the app name, version, engines (Node 24+, Yarn 4), and adds `zyra-sdk` plus scripts like `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall`, and `auth:login` that delegate to the local `zyra` CLI.
...
* `yarn app:dev` は `node_modules/zyra-sdk/generated` に型付き Zyra クライアントを自動生成します。
...
認証情報はユーザーごとに `~/.zyra/config.json` に保存されます。 複数のプロファイルを管理し、相互に切り替えることができます。
...
zyra-sdk は、アプリ内で使用する型付きのビルディングブロックとヘルパー関数を提供します。 以下は、最も頻繁に扱う主要な構成要素です。
...
<Note>
  **ベースフィールドは自動作成されます。** カスタムオブジェクトを定義すると、Zyra は `name`、`createdAt`、`updatedAt`、`createdBy`、`position`、`deletedAt` などの標準フィールドを自動的に追加します。 これらを `fields` 配列で定義する必要はありません。カスタムフィールドのみを追加してください。
</Note>
...
* `ZYRA_API_KEY` として注入される実行時の API キーは、このデフォルトの関数ロールから派生します。
...
* 動作例は Hello World アプリにあります: [packages/zyra-apps/hello-world/src/roles/function-role.ts](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts)。
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. 型を `zyra-sdk` からインポートします:
...
`yarn app:dev` は `node_modules/zyra-sdk/generated` に型付き Zyra クライアントを自動生成します。 関数内で使用します：
...
関数が Zyra 上で実行されると、コードが実行される前に、プラットフォームが認証情報を環境変数として注入します：

* `ZYRA_API_URL`: アプリが対象とする Zyra API のベース URL。
* `ZYRA_API_KEY`: アプリケーションのデフォルト関数ロールにスコープされた短命のキー。

ノート:

* 生成されたクライアントに URL や API キーを渡す必要はありません。 実行時に process.env から `ZYRA_API_URL` と `ZYRA_API_KEY` を読み取ります。
...
オブジェクト、関数、複数のトリガーを示す最小のエンドツーエンド例は[こちら](https://github.com/zyrahq/zyra/tree/main/packages/zyra-apps/hello-world)をご覧ください。

## 手動セットアップ（スキャフォルダーなし）

最適な導入体験のために `create-zyra-app` の使用を推奨しますが、手動でプロジェクトをセットアップすることもできます。 CLI をグローバルにインストールしないでください。 代わりに、`zyra-sdk` をローカル依存関係として追加し、package.json にスクリプトを設定します：
...
* サーバーに接続できません: API URL と、Zyra サーバーに到達可能であることを確認してください。
