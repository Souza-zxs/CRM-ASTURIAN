---
title: 前端组件
description: 构建可在 Zyra 的 UI 中渲染并具备沙盒隔离的 React 组件。
icon: window-maximize
---

前端组件是直接在 Zyra 的 UI 内渲染的 React 组件。 它们在使用 Remote DOM 的**隔离 Web Worker**中运行——你的代码在沙盒中执行，但会原生渲染到页面中，而非在 iframe 里。

## 前端组件可用位置

在 Zyra 中，前端组件可在两个位置进行渲染：
...
使用 `yarn zyra dev` 同步后（或单次运行 `yarn zyra dev --once`），快速操作会出现在页面右上角：
...
由于该组件返回 `null`，Zyra 会跳过为其渲染容器——布局中不会出现空白区域。 该组件仍可访问所有 hooks 和宿主通信 API。

## SDK Command 组件

`zyra-sdk` 包提供了四个为无头前端组件设计的 Command 辅助组件。 每个组件都会在挂载时执行一个操作，通过显示 snackbar 通知来处理错误，并在完成后自动卸载该前端组件。

从 `zyra-sdk/command` 导入它们：
...
使用 `httpRouteTriggerSettings` 声明的逻辑函数会通过 `/s/` 端点暴露在 `${ZYRA_API_URL}/s\<path>` 下。 你的前端组件使用来自 `zyra-client-sdk/rest` 的 `RestApiClient` 调用该路由，该客户端会使用 Zyra 注入到 worker 中的 `ZYRA_APP_ACCESS_TOKEN` 进行身份验证。

`RestApiClient` 正是为这种场景而构建的。 它会从 worker 环境中读取 `ZYRA_API_URL` 和 `ZYRA_APP_ACCESS_TOKEN`，附加 `Authorization: Bearer` 请求头，对 JSON 进行序列化和解析，并在 token 或 URL 缺失或响应为非 2xx 时抛出 `RestApiClientError`——这样你就不必在每个组件中重复实现这些样板逻辑。
...
传递给客户端的路径是该路由的公共路径——逻辑函数的 `httpRouteTriggerSettings.path`，并以 `/s` 作为前缀。 保持 `isAuthRequired: true`；客户端会为你的组件提供由 Zyra 签发的应用访问令牌：
...
<Note>
`ZYRA_API_URL` 和 `ZYRA_APP_ACCESS_TOKEN` 会被自动注入——参见 [应用变量](#application-variables)。 由于机密应用变量永远不会暴露给前端组件，请将 API 密钥和其他敏感逻辑保留在逻辑函数中，而不是前端组件中。
</Note>

### RestApiClient 参考

从 `zyra-client-sdk/rest` 中导入 `RestApiClient`。 它与 `CoreApiClient` 和 `MetadataApiClient` 属于同一客户端家族，但目标是你应用的 HTTP 路由，而不是 GraphQL API。
...
| `ZYRA_APP_ACCESS_TOKEN` | 限定在你的应用角色范围内的短期令牌  |

## 宿主通信 API

前端组件可以使用来自 `zyra-sdk` 的函数触发导航、模态框和通知：
...
* **Zyra UI 组件** — 从 `zyra-sdk/ui` 导入（Button、Tag、Status、Chip、Avatar 等）
