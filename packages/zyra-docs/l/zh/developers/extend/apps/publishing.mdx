...
1. 检出你的应用源代码。
2. 使用 `./.github/actions/spawn-zyra-app-dev-test` 组合 action 启动一个隔离的 Zyra 测试实例（相当于 CI 中的 `yarn zyra server start --test`）。
...
2. 运行 `./.github/actions/deploy-zyra-app`——相当于 CI 中的 `yarn zyra deploy`。
3. 运行 `./.github/actions/install-zyra-app`，将新部署的版本安装到目标工作区。
...
两个工作流都引用了位于 `@main` 的可复用 actions，因此会自动获取 action 更新。 如果你希望构建具有确定性，请在每个 `uses:` 行中将 `@main` 替换为某个提交的 SHA 或发行标签。
