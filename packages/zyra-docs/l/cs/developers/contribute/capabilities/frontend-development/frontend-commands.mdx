...
```bash
npx nx run zyra-front:lint # přidejte --fix pro opravu chyb ve formátování
```
...
```bash
npx nx run zyra-front:test # spusťte jest testy
npx nx run zyra-front:storybook:serve:dev # spusťte storybook
npx nx run zyra-front:storybook:test # spusťte testy # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
npx nx run zyra-front:storybook:coverage # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
```
