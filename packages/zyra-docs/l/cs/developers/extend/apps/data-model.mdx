---
title: Datový model
description: Definujte objekty, pole, role a metadata aplikace pomocí Zyra SDK.
icon: database
---

Balíček `zyra-sdk` poskytuje funkce `defineEntity` pro deklaraci datového modelu vaší aplikace. Abyste umožnili SDK detekovat vaše entity, musíte použít `export default defineEntity({...})`. Tyto funkce validují vaši konfiguraci v době sestavení a poskytují automatické doplňování v IDE a typovou bezpečnost.
...
* Běhový token vložený jako `ZYRA_APP_ACCESS_TOKEN` je odvozen z této role.
...
* Podívejte se na funkční příklad: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Nové objekty můžete vygenerovat pomocí `yarn zyra add`, který vás provede pojmenováním, poli a vztahy.

<Note>
**Základní pole jsou vytvořena automaticky.** Když definujete vlastní objekt, Zyra automaticky přidá standardní pole
...
Pomocí `defineField()` přidejte pole k objektům, které nevlastníte — například ke standardním objektům Zyra (Person, Company atd.). nebo k objektům z jiných aplikací. Na rozdíl od inline polí v `defineObject()` vyžadují samostatná pole `objectUniversalIdentifier` k určení, který objekt rozšiřují:
...
Hlavní body:
* `objectUniversalIdentifier` identifikuje cílový objekt. Pro standardní objekty použijte `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportovaný z `zyra-sdk`.
...
Relace propojují objekty. Ve Zyra jsou relace vždy obousměrné — definujete obě strany a každá strana odkazuje na tu druhou.
...
Chcete-li vytvořit relaci s vestavěným objektem Zyra (Person, Company atd.), použijte `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Generování entit pomocí `yarn zyra add`
...
| ------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Objekt                    | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Pole                      | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Logická funkce            | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Frontendová komponenta    | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Role                      | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Dovednost                 | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Agent                     | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Pohled                    | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Položka navigační nabídky | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Rozvržení stránky         | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Co generátor vytváří

Každý typ entity má vlastní šablonu. Například `yarn zyra add object` se zeptá na:
