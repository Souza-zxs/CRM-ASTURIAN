...
2. Spustí izolovanou testovací instanci Zyra pomocí složené akce `./.github/actions/spawn-zyra-app-dev-test` (ekvivalent v CI k `yarn zyra docker:start --test`).
...
2. Spustí `./.github/actions/deploy-zyra-app` — ekvivalent v CI k `yarn zyra app:publish --private`.
3. Spustí `./.github/actions/install-zyra-app`, aby se nově nasazená verze nainstalovala do cílového pracovního prostoru.
...
Obě workflowy odkazují na znovupoužitelné akce na `@main`, takže aktualizace akcí se přeberou automaticky. Pokud chcete deterministická sestavení, nahraďte `@main` v každém řádku `uses:` za commit SHA nebo tag vydání.
