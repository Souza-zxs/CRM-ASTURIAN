...
* Použijte `serverVariables` (nikoli `applicationVariables`) — pověření OAuth jsou celoserverová a na jeden server Zyra je jedna aplikace OAuth.
...
4. Otevřete nainstalovanou aplikaci v Zyra jako správce serveru → nastavte hodnoty na odpovídajících `serverVariables`.
