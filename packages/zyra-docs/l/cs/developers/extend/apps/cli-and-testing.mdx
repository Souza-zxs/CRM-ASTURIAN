...
* **Zahrnuté do buildů** — `yarn zyra build` zabalí všechny veřejné prostředky do distribučního výstupu.

### Přístup k veřejným prostředkům pomocí `getPublicAssetUrl`

K získání plné URL souboru ve vaší složce `public/` použijte pomocnou funkci `getPublicAssetUrl` z `zyra-sdk`. Funguje jak v logických funkcích, tak ve frontendových komponentách.
...
### Spouštění funkcí (`yarn zyra exec`)
...
### Zobrazení logů funkcí (`yarn zyra logs`)
...
<Note>
To je jiné než `yarn zyra server logs`, které zobrazují logy kontejneru Docker. `yarn zyra logs` zobrazuje logy spuštění funkcí vaší aplikace ze serveru Zyra.
</Note>

### Odinstalace aplikace (`yarn zyra uninstall`)
