...
Tím se zkompilují zdrojové soubory TypeScriptu, transpilují logické funkce a frontendové komponenty a vše se zapíše do `.zyra/output/`. Přidejte `--tarball`, abyste také vytvořili balíček `.tgz` pro ruční distribuci nebo příkaz deploy.
...
2. Spusťte `yarn zyra deploy` (nebo `yarn zyra deploy --remote production`)
...
2. Spustí izolovanou testovací instanci Zyra pomocí složené akce `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (ekvivalent v CI k `yarn zyra server start --test`).
...
2. Spustí `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — ekvivalent v CI k `yarn zyra deploy`.
3. Spustí `zyrahq/zyra/.github/actions/install-zyra-app@main`, aby se nově nasazená verze nainstalovala do cílového workspace.
...
* Klíčové slovo `zyra-app` ve vašem poli `keywords` v souboru `package.json` (přidejte je ručně — ve výchozím nastavení není zahrnuto v šabloně `create-zyra-app`)
...
Pro jiné systémy CI (GitLab CI, CircleCI atd.) platí stejné tři příkazy: `yarn install`, `yarn zyra build` a poté `npm publish` z `.zyra/output`.
...
K instalaci novější verze ji nejprve nasaďte nebo publikujte, poté znovu spusťte `yarn zyra install`.
</Note>
