...
2. Spustí izolovanou testovací instanci Zyra pomocí složené akce `./.github/actions/spawn-zyra-app-dev-test` (ekvivalent v CI k `yarn zyra server start --test`).
...
2. Spustí `./.github/actions/deploy-zyra-app` — ekvivalent v CI k `yarn zyra deploy`.
3. Spustí `./.github/actions/install-zyra-app`, aby se nově nasazená verze nainstalovala do cílového workspace.
