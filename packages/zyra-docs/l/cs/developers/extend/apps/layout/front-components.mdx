...
description: Vytvářejte komponenty Reactu, které se vykreslují uvnitř uživatelského rozhraní Zyra se sandboxovou izolací.
icon: window-maximize
---

Frontendové komponenty jsou React komponenty, které se vykreslují přímo v uživatelském rozhraní Zyra. Běží v **izolovaném Web Workeru** s využitím Remote DOM — váš kód je sandboxovaný, ale vykresluje se nativně na stránce, nikoli v iframu.

## Kde lze použít frontendové komponenty

Frontendové komponenty se mohou vykreslovat na dvou místech v rámci Zyra:
...
Po synchronizaci pomocí `yarn zyra dev` (nebo po jednorázovém spuštění `yarn zyra dev --once`) se rychlá akce zobrazí v pravém horním rohu stránky:
...
Protože komponenta vrací `null`, Zyra přeskočí vykreslení kontejneru — v rozvržení se neobjeví žádné prázdné místo. Komponenta má však stále přístup ke všem hookům a API komunikace s hostitelem.

## Komponenty SDK Command

Balíček `zyra-sdk` poskytuje čtyři pomocné komponenty Command navržené pro headless front-endové komponenty. Každá komponenta při připojení provede akci, chyby zpracuje zobrazením oznámení ve snackbaru a po dokončení automaticky odpojí front-endovou komponentu.

Importujte je z `zyra-sdk/command`:
...
Logická funkce deklarovaná pomocí `httpRouteTriggerSettings` je vystavena pod endpointem `/s/` na `${ZYRA_API_URL}/s\<path>`. Vaše front komponenta volá tuto trasu pomocí `RestApiClient` z `zyra-client-sdk/rest`, který se autentizuje pomocí `ZYRA_APP_ACCESS_TOKEN`, který Zyra do workeru vkládá.

`RestApiClient` je přesně pro tento účel. Z worker prostředí čte `ZYRA_API_URL` a `ZYRA_APP_ACCESS_TOKEN`, přidává hlavičku `Authorization: Bearer`, serializuje a parsuje JSON a vyhazuje `RestApiClientError`, pokud token nebo URL chybí nebo je odpověď mimo rozsah 2xx — takže nemusíte tento boilerplate znovu implementovat v každé komponentě.
...
Cesta předaná klientovi je veřejná cesta trasy — `httpRouteTriggerSettings.path` logické funkce s předponou `/s`. Ponechte `isAuthRequired: true`; klient poskytuje pro vaši komponentu přístupový token aplikace vydaný Zyra:
...
<Note>
`ZYRA_API_URL` a `ZYRA_APP_ACCESS_TOKEN` jsou vloženy automaticky — viz [Proměnné aplikace](#application-variables). Protože tajné proměnné aplikace nejsou nikdy vystaveny front komponentám, ponechte API klíče a další citlivou logiku v logické funkci, ne ve front komponentě.
</Note>

### Reference `RestApiClient`

Importujte `RestApiClient` z `zyra-client-sdk/rest`. Patří do stejné rodiny klientů jako `CoreApiClient` a `MetadataApiClient`, ale cílí na HTTP trasy vaší aplikace místo na GraphQL API.
...
| `ZYRA_APP_ACCESS_TOKEN` | Krátkodobý token s oprávněními omezenými na roli vaší aplikace |

## API komunikace s hostitelem

Frontendové komponenty mohou pomocí funkcí z `zyra-sdk` vyvolávat navigaci, modály a oznámení:
...
* **Komponenty Zyra UI** — import z `zyra-sdk/ui` (Button, Tag, Status, Chip, Avatar a další)
