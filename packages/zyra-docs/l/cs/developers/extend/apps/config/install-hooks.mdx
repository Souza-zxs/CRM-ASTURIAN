...
* **Nespouští se v režimu dev**: stejně jako u post-install — u lokálně registrovaných aplikací je instalační tok zcela přeskočen, takže se pre-install pod `yarn zyra dev` nikdy nespustí. Použijte `yarn zyra dev:function:exec --preInstall` k ručnímu spuštění.
