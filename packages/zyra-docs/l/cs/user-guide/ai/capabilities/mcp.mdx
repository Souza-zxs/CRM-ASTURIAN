---
title: MCP Server
description: Připojte asistenty AI ke svému pracovnímu prostoru Zyra pomocí protokolu Model Context Protocol.
---
...
Zyra zpřístupňuje server [MCP](https://modelcontextprotocol.io/), aby AI asistenti — Claude Desktop, Claude Code, Cursor, ChatGPT a další — mohli číst a zapisovat data vašeho CRM pomocí přirozeného jazyka.

Použijte svou **adresu URL pracovní oblasti** (adresu URL, kterou používáte k přístupu k Zyra) jako koncový bod MCP. V Zyra Cloud může vaše adresa URL pracovní oblasti být `https://{mycompany}.zyra.com` nebo vlastní doména. Server je dostupný na:

| Prostředí           | Koncový bod MCP                                                               |
| ------------------- | ----------------------------------------------------------------------------- |
| **Cloud**           | `https://{your-workspace-url}/mcp` (např. `https://mycompany.zyra.com/mcp`) |
...
Přidejte toto do konfigurace svého klienta MCP a nahraďte `{your-workspace-url}` hostitelem vaší pracovní oblasti (např. `mycompany.zyra.com`):
...
1. Získá metadata OAuth služby Zyra pomocí `/.well-known/oauth-protected-resource` a `/.well-known/oauth-authorization-server`
...
V Zyra přejděte do **Settings > AI > More > MCP Server**. Vyberte metodu ověření (OAuth nebo klíč API), zkopírujte úryvek JSON (už bude používat adresu URL vaší pracovní oblasti) a vložte jej do konfiguračního souboru klienta MCP.
...
Restartujte svého klienta MCP (nebo znovu načtěte konfiguraci). Pokud používáte OAuth, budete přesměrováni do Zyra k autorizaci přístupu. Pokud používáte klíč API, připojení je okamžité.
...
Po připojení server MCP zpřístupní nástroje, které odpovídají rozhraní Zyra API. Doporučený pracovní postup je:
...
U samostatně hostovaných instancí nahraďte `{your-workspace-url}` adresou URL svého serveru. Ujistěte se, že `SERVER_URL` ve vašem prostředí odpovídá veřejné adrese URL vaší instance Zyra — tato hodnota se používá k vygenerování metadat pro zjišťování OAuth.
