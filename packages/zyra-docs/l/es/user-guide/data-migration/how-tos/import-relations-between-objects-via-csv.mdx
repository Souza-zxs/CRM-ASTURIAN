...
El padre (lado "uno") debe existir en Zyra antes de poder hacer referencia a él.
...
1. Exporte los registros padre desde Zyra
...
Use `https://domain.com` (no solo `domain.com`). Esto coincide con cómo Zyra almacena los dominios de las Empresas y evita errores de coincidencia.
</Warning>
...
Zyra vinculará automáticamente cada registro hijo con el padre correspondiente.
...
| **Registros inexistentes**                | El registro padre no existe en Zyra                | Importe primero los registros padre o verifique si hay errores tipográficos |
| **Diferencia de mayúsculas y minúsculas** | `Acme.com` en el archivo pero `acme.com` en Zyra   | Asegure una coincidencia exacta de mayúsculas y minúsculas                  |
...
<AccordionGroup>
  <Accordion title="¿Qué pasa si mi registro padre no tiene dominio ni correo electrónico?">
    Tiene dos opciones:
    1. Use el `id` de Zyra (exporte los registros padre para obtener sus IDs)
