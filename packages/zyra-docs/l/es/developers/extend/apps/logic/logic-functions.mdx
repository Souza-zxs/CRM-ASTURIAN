...
Las funciones lógicas son funciones de TypeScript del lado del servidor que se ejecutan en la plataforma Zyra. Pueden activarse mediante solicitudes HTTP, programaciones de cron o eventos de base de datos — y también pueden exponerse como herramientas para agentes de IA.
...
> p. ej., `path: '/post-card/create'` se puede invocar en `https://your-zyra-server.com/s/post-card/create`
...
Importa el tipo `RoutePayload` desde `zyra-sdk/logic-function`:
...
De forma predeterminada, devolver un valor sencillo desde tu controlador lo envía de vuelta como una respuesta `200` (JSON para objetos, `text/plain` para cadenas). Para controlar el código de estado y los encabezados de la respuesta, devuelve un `Response` desde `zyra-sdk/logic-function`:
...
| Campo    | Valores                                              | Notas                                                                                                                                                   |
| -------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `source` | `body` \| `query` \| `header`                        | `body` lee el JSON parseado. `query` es el más universal: normalmente controlas la URL de callback que registras, así que añade `?zyraWorkspaceId=…`. |
| `path`   | ruta con puntos, p. ej. `metadata.zyraWorkspaceId` | Restringido a segmentos alfanuméricos / `_` / `-`; las claves de prototipo se rechazan.                                                                 |
...
* **`toolTriggerSettings`** — hace que la función sea descubrible por las funciones de IA de Zyra (chat, MCP, llamadas a funciones). Usa el JSON Schema estándar, el formato que los LLM entienden de forma nativa.
* **`workflowActionTriggerSettings`** — hace que la función aparezca como un paso en el constructor visual de flujos de trabajo. Usa el `InputSchema` completo de Zyra para que el constructor pueda renderizar editores de campos adecuados, selectores de variables y etiquetas.
...
Para declarar tus parámetros **una sola vez** y atender ambas superficies, define un único JSON Schema (`InputJsonSchema`) y conviértelo para la acción de flujo de trabajo con `jsonSchemaToInputSchema` de `zyra-sdk/logic-function`. `toolTriggerSettings.inputSchema` usa directamente el JSON Schema, mientras que `workflowActionTriggerSettings.inputSchema` espera el `InputSchema` de Zyra:
...
## Clientes de API tipados (zyra-client-sdk)

El paquete `zyra-client-sdk` proporciona dos clientes GraphQL tipados para interactuar con la API de Zyra desde tus funciones de lógica y componentes de frontend.

| Cliente             | Importar                     | Endpoint                                                               | ¿Generado?                              |
| ------------------- | ---------------------------- | ---------------------------------------------------------------------- | --------------------------------------- |
| `CoreApiClient`     | `zyra-client-sdk/core`     | `/graphql` — datos del espacio de trabajo (registros, objetos)         | Sí, en tiempo de desarrollo/compilación |
| `MetadataApiClient` | `zyra-client-sdk/metadata` | `/metadata` — configuración del espacio de trabajo, cargas de archivos | No, viene preconstruido                 |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Consultar y modificar datos del espacio de trabajo (registros, objetos)">

`CoreApiClient` es el cliente principal para consultar y mutar datos del espacio de trabajo. Se **genera a partir del esquema de tu espacio de trabajo** durante `yarn zyra dev` o `yarn zyra dev:build`, por lo que está completamente tipado para coincidir con tus objetos y campos.
...
<Note>
**CoreApiClient se genera en tiempo de desarrollo/compilación.** Si intentas usarlo sin ejecutar primero `yarn zyra dev` o `yarn zyra dev:build`, lanzará un error. La generación ocurre automáticamente: la CLI inspecciona el esquema GraphQL de tu espacio de trabajo y genera un cliente tipado usando `@genql/cli`.
</Note>
...
<Note>
  Cuando tu código se ejecuta en Zyra (funciones de lógica o componentes de frontend), la plataforma inyecta credenciales como variables de entorno:

  * `ZYRA_API_URL` — URL base de la API de Zyra
  * `ZYRA_APP_ACCESS_TOKEN` — Token de corta duración con alcance al rol de función predeterminado de tu aplicación
