---
title: Andamiaje
description: Genera archivos de entidad de forma interactiva con yarn zyra dev:add — objetos, campos, vistas, funciones de lógica y más.
...
| ------------------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Objeto                          | `yarn zyra dev:add object`             | `src/objects/\<name>.ts`               |
| Campo                           | `yarn zyra dev:add field`              | `src/fields/\<name>.ts`                |
| Función de lógica               | `yarn zyra dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de frontend          | `yarn zyra dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                             | `yarn zyra dev:add role`               | `src/roles/\<name>.ts`                 |
| Habilidad                       | `yarn zyra dev:add skill`              | `src/skills/\<name>.ts`                |
| Agente                          | `yarn zyra dev:add agent`              | `src/agents/\<name>.ts`                |
| Vista                           | `yarn zyra dev:add view`               | `src/views/\<name>.ts`                 |
| Elemento del menú de navegación | `yarn zyra dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Diseño de página                | `yarn zyra dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Qué genera el generador

Cada tipo de entidad tiene su propia plantilla. Por ejemplo, `yarn zyra dev:add object` solicita:
