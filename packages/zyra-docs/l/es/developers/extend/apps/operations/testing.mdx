...
Ambos entornos tienen `zyra-client-sdk/core` y `zyra-client-sdk/metadata` disponibles como módulos preproporcionados — estos no se incluyen en el bundle sino que se resuelven en tiempo de ejecución por el servidor.
...
La subruta `zyra-sdk/cli` exporta funciones que puedes invocar directamente desde el código de pruebas:
...
Asegúrate de que tu servidor local de Zyra esté en ejecución y luego:
...
1. Obtiene tu código
2. Inicia un servidor temporal de Zyra usando la acción `zyrahq/zyra/.github/actions/spawn-zyra-docker-image`
3. Instala las dependencias con `yarn install --immutable`
4. Ejecuta `yarn test` con `ZYRA_API_URL` y `ZYRA_API_KEY` inyectados a partir de las salidas de la acción
...
No necesitas configurar secretos: la acción `spawn-zyra-docker-image` inicia un servidor efímero de Zyra directamente en el runner y devuelve los detalles de conexión. El secreto `GITHUB_TOKEN` lo proporciona GitHub automáticamente.

Para fijar una versión específica de Zyra en lugar de `latest`, cambia la variable de entorno `ZYRA_VERSION` al inicio del flujo de trabajo.
