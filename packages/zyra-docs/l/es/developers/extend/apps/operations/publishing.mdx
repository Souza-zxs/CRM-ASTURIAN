...
2. Inicia una instancia de prueba aislada de Zyra usando la acción compuesta `./.github/actions/spawn-zyra-app-dev-test` (el equivalente en CI de `yarn zyra docker:start --test`).
...
2. Ejecuta `./.github/actions/deploy-zyra-app` — el equivalente en CI de `yarn zyra app:publish --private`.
3. Ejecuta `./.github/actions/install-zyra-app` para que la versión recién desplegada se instale en el espacio de trabajo de destino.
...
Ambos flujos de trabajo hacen referencia a acciones reutilizables en `@main`, por lo que las actualizaciones de acciones se aplican automáticamente. Si quieres compilaciones deterministas, reemplaza `@main` por un SHA de commit o una etiqueta de versión en cada línea `uses:`.
