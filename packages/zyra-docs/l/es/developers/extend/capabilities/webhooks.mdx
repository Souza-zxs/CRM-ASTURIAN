...
Los webhooks envían datos a tus sistemas en tiempo real cuando ocurren eventos en Zyra — no se requiere sondeo. Úsalos para mantener sincronizados los sistemas externos, activar automatizaciones o enviar alertas.
...
Zyra envía webhooks para estos tipos de eventos:
...
Zyra firma cada solicitud de webhook por seguridad. Valida las firmas para garantizar que las solicitudes sean auténticas.

### Encabezados

| Encabezado                   | Descripción                     |
| ---------------------------- | ------------------------------- |
| `X-Zyra-Webhook-Signature` | Firma HMAC SHA256               |
| `X-Zyra-Webhook-Timestamp` | Marca de tiempo de la solicitud |

### Pasos de validación

1. Obtén la marca de tiempo de `X-Zyra-Webhook-Timestamp`
2. Crea la cadena: `{timestamp}:{JSON payload}`
3. Calcula HMAC SHA256 usando tu secreto de webhook
4. Compara con `X-Zyra-Webhook-Signature`
...
| **Disparador de webhook de flujo de trabajo** | ENTRADA   | Recibir datos en Zyra desde sistemas externos                                   |
