---
title: Aplicaciones de Zyra
description: Crea y gestiona personalizaciones de Zyra como código.
---
...
Las aplicaciones te permiten crear y administrar personalizaciones de Zyra **como código**. En lugar de configurar todo a través de la interfaz de usuario, defines tu modelo de datos y funciones de lógica en código, lo que hace más rápido crear, mantener y desplegar en múltiples espacios de trabajo.
...
* Node.js 24+ y Yarn 4
* Un espacio de trabajo de Zyra y una clave de API (créala en https://app.zyra.com/settings/api-webhooks)
...
Consulta también: las páginas de referencia de la CLI para [create-zyra-app](https://www.npmjs.com/package/create-zyra-app) y [zyra-sdk CLI](https://www.npmjs.com/package/zyra-sdk).

## Estructura del proyecto (generada)

Cuando ejecutas `npx create-zyra-app@latest my-zyra-app`, el generador:

* Copia una aplicación base mínima en `my-zyra-app/`
* Añade una dependencia local de `zyra-sdk` y la configuración de Yarn 4
* Crea archivos de configuración y scripts vinculados a la CLI `zyra`
...
* **package.json**: Declara el nombre de la aplicación, la versión, los entornos (Node 24+, Yarn 4) y agrega `zyra-sdk` además de scripts como `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` y `auth:login` que delegan en la CLI local `zyra`.
...
* `yarn app:dev` genera automáticamente el cliente Zyra tipado en `node_modules/zyra-sdk/generated`.
...
Tus credenciales se almacenan por usuario en `~/.zyra/config.json`. Puedes mantener varios perfiles y cambiar entre ellos.
...
El zyra-sdk proporciona bloques de construcción tipados y funciones auxiliares que utilizas dentro de tu aplicación. A continuación, las partes clave que usarás con más frecuencia.
...
import { defineObject, FieldType } from 'zyra-sdk';
...
<Note>
  **Los campos base se crean automáticamente.** Cuando defines un objeto personalizado, Zyra añade automáticamente campos estándar como `name`, `createdAt`, `updatedAt`, `createdBy`, `position` y `deletedAt`. No necesitas definir estos en tu matriz `fields` — solo agrega tus campos personalizados.
</Note>
...
import { defineApplication } from 'zyra-sdk';
import { DEFAULT_ROLE_UNIVERSAL_IDENTIFIER } from './default-function.role';

export default defineApplication({
  universalIdentifier: '4ec0391d-18d5-411c-b2f3-266ddc1c3ef7',
  displayName: 'My Zyra App',
  description: 'My first Zyra app',
  icon: 'IconWorld',
...
* La clave de API en tiempo de ejecución inyectada como `ZYRA_API_KEY` se deriva de este rol de función predeterminado.
...
import { defineRole, PermissionFlag } from 'zyra-sdk';
...
* Consulta un ejemplo funcional en la aplicación Hello World: [`packages/zyra-apps/hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
import { defineFunction } from 'zyra-sdk';
import type { DatabaseEventPayload, ObjectRecordCreateEvent, CronPayload, RoutePayload } from 'zyra-sdk';
import Zyra, { type Person } from '~/generated';

const handler = async (params: RoutePayload) => {
  const client = new Zyra(); // generated typed client
...
Cuando un disparador de ruta invoca tu función de lógica, esta recibe un objeto `RoutePayload` que sigue el formato de AWS HTTP API v2. Importa el tipo desde `zyra-sdk`:

```typescript
import { defineFunction, type RoutePayload } from 'zyra-sdk';
...
`yarn app:dev` genera automáticamente el cliente Zyra tipado en `node_modules/zyra-sdk/generated`. Úsalo en tus funciones:

```typescript
import Zyra from '~/generated';

const client = new Zyra();
...
Cuando tu función se ejecuta en Zyra, la plataforma inyecta credenciales como variables de entorno antes de que tu código se ejecute:

* `ZYRA_API_URL`: URL base de la API de Zyra a la que apunta tu aplicación.
* `ZYRA_API_KEY`: Clave de corta duración con alcance al rol de función predeterminado de tu aplicación.

Notas:

* No necesitas pasar la URL ni la clave de API al cliente generado. Lee `ZYRA_API_URL` y `ZYRA_API_KEY` de process.env en tiempo de ejecución.
...
Explora un ejemplo mínimo de extremo a extremo que demuestra objetos, funciones y múltiples desencadenadores [aquí](https://github.com/zyrahq/zyra/tree/main/packages/zyra-apps/hello-world):

## Configuración manual (sin el generador)

Aunque recomendamos usar `create-zyra-app` para la mejor experiencia de inicio, también puedes configurar un proyecto manualmente. No instales la CLI globalmente. En su lugar, agrega `zyra-sdk` como dependencia local y conecta scripts en tu package.json:
...
{
  "scripts": {
    "auth:login": "zyra auth:login",
    "auth:logout": "zyra auth:logout",
    "auth:status": "zyra auth:status",
    "auth:switch": "zyra auth:switch",
    "auth:list": "zyra auth:list",
    "app:dev": "zyra app:dev",
    "app:uninstall": "zyra app:uninstall",
    "entity:add": "zyra entity:add",
    "function:logs": "zyra function:logs",
    "function:execute": "zyra function:execute",
    "help": "zyra help"
  }
}
```
...
* No se puede conectar al servidor: verifica la URL de la API y que el servidor de Zyra sea accesible.
