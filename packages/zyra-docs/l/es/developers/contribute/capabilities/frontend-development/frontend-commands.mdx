...
```bash
npx nx run zyra-front:lint # pasar --fix para corregir errores de lint
```
...
```bash
npx nx run zyra-front:test # ejecutar pruebas con Jest
npx nx run zyra-front:storybook:serve:dev # ejecutar Storybook
npx nx run zyra-front:storybook:test # ejecutar pruebas # (requiere que yarn storybook:serve:dev esté en ejecución)
npx nx run zyra-front:storybook:coverage # (requiere que yarn storybook:serve:dev esté en ejecución)
```
