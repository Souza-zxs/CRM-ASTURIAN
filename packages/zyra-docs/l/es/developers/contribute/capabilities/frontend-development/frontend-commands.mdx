...
```bash
npx nx run asturian-front:lint # pasar --fix para corregir errores de lint
```
...
```bash
npx nx run asturian-front:test # ejecutar pruebas con Jest
npx nx run asturian-front:storybook:serve:dev # ejecutar Storybook
npx nx run asturian-front:storybook:test # ejecutar pruebas # (requiere que yarn storybook:serve:dev esté en ejecución)
npx nx run asturian-front:storybook:coverage # (requiere que yarn storybook:serve:dev esté en ejecución)
```
