...
```bash
npx nx run zyra-front:lint # utilizați --fix pentru a corecta erorile de lint
```
...
```bash
npx nx run zyra-front:test # rulați teste jest
npx nx run zyra-front:storybook:serve:dev # rulați storybook
npx nx run zyra-front:storybook:test # rulați testele # (are nevoie de yarn storybook:serve:dev pentru a rula)
npx nx run zyra-front:storybook:coverage # (are nevoie de yarn storybook:serve:dev pentru a rula)
```
