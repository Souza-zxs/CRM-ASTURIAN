...
2. Pornește o instanță de test Zyra izolată folosind acțiunea compozită `./.github/actions/spawn-zyra-app-dev-test` (echivalentul din CI al `yarn zyra docker:start --test`).
...
2. Rulează `./.github/actions/deploy-zyra-app` — echivalentul din CI al `yarn zyra app:publish --private`.
3. Rulează `./.github/actions/install-zyra-app` astfel încât versiunea nou implementată să fie instalată în spațiul de lucru țintă.
...
Ambele fluxuri de lucru fac referire la acțiuni reutilizabile la `@main`, astfel încât actualizările acțiunilor sunt preluate automat. Dacă dorești builduri deterministe, înlocuiește `@main` cu un SHA de commit sau cu un tag de release pe fiecare linie `uses:`.
