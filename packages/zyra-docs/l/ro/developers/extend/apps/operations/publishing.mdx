...
title: Publicare
icon: încarcă
description: Distribuie aplicația ta Zyra în marketplace sau implementeaz-o intern.
---
...
* **Implementează un tarball** — încarcă aplicația direct pe un server Zyra anume pentru uz intern sau privat.
* **Publică pe npm** — listează aplicația ta în marketplace-ul Zyra pentru ca orice spațiu de lucru să o poată descoperi și instala.
...
Aceasta compilează sursele TypeScript, transpilează funcțiile de logică și componentele de front-end și scrie totul în `.zyra/output/`. Adaugă `--tarball` pentru a produce și un pachet `.tgz` pentru distribuire manuală sau pentru comanda de publish.

## Implementare pe un server (tarball)

Pentru aplicațiile pe care nu le dorești disponibile public — instrumente proprietare, integrări doar pentru enterprise sau build-uri experimentale — poți implementa un tarball direct pe un server Zyra.

### Cerințe

Înainte de implementare, ai nevoie de un remote configurat care să indice serverul țintă. Remote-urile stochează local URL-ul serverului și credențialele de autentificare în `~/.zyra/config.json`.
...
2. Rulează `yarn zyra app:publish --private` (sau `yarn zyra app:publish --private --remote production`)
...
Dacă aplicația ta folosește o funcționalitate introdusă într-o anumită versiune de server Zyra (de exemplu, furnizori OAuth adăugați în v2.3.0), ar trebui să declari versiunea minimă de server necesară aplicației folosind câmpul `engines.zyra` din `package.json`:
...
* Dacă `engines.zyra` este setat și versiunea serverului țintă nu respectă intervalul, implementarea (încărcarea arhivei tarball) sau instalarea este respinsă cu eroarea `SERVER_VERSION_INCOMPATIBLE` și cu un mesaj care indică atât intervalul necesar, cât și versiunea efectivă a serverului.
* Dacă `engines.zyra` nu este setat, aplicația este acceptată pe orice versiune de server (retrocompatibilă cu aplicațiile existente).
* Dacă serverul nu are nicio `APP_VERSION` configurată, verificarea este omisă.

<Note>
Serverul este verificarea autoritativă — validează `engines.zyra` atât la încărcarea arhivei tarball, cât și la instalarea în spațiul de lucru. Dacă implementezi un tarball în afara fluxului standard sau instalezi din marketplace, serverul impune în continuare compatibilitatea.
</Note>

## CI/CD automatizat (fluxuri de lucru preconfigurate)

Aplicațiile generate cu `create-zyra-app` vin, gata de utilizare, cu două fluxuri de lucru GitHub Actions, în `.github/workflows/`. Acestea sunt gata să ruleze imediat ce faci push al repozitoriului pe GitHub — nu este necesară nicio configurare suplimentară pentru CI, iar CD necesită doar un singur secret.
...
2. Pornește o instanță de test Zyra izolată folosind acțiunea compozită `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (echivalentul din CI al `yarn zyra docker:start --test`).
3. Activează Corepack, configurează Node.js pe baza fișierului `.nvmrc` și instalează dependențele cu `yarn install --immutable`.
4. Rulează `yarn test`, transmitând `ZYRA_API_URL` și `ZYRA_API_KEY` din instanța pornită, astfel încât testele să poată comunica cu un server real.

**Opțiuni de configurare:**

* `ZYRA_VERSION` (variabilă de mediu, implicit `latest`) — fixează versiunea serverului Zyra folosită în CI editând acest parametru în `ci.yml`.
...
Implementează aplicația pe un server Zyra configurat la fiecare push pe `main` și, opțional, dintr-un pull request când se aplică eticheta `deploy`.

**Ce face:**

1. Preia head-ul PR-ului (pentru PR-urile etichetate) sau commitul împins.
2. Rulează `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — echivalentul din CI al `yarn zyra app:publish --private`.
3. Rulează `zyrahq/zyra/.github/actions/install-zyra-app@main` astfel încât versiunea nou implementată să fie instalată în spațiul de lucru țintă.

**Configurare necesară:**

| Setare                  | Unde                                                       | Scop                                                                                                               |
| ----------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `ZYRA_DEPLOY_URL`     | `env` în `cd.yml` (implicit `http://localhost:3000`)       | Serverul Zyra la care se face implementarea. Modifică-l la URL-ul real al serverului înainte de prima utilizare. |
| `ZYRA_DEPLOY_API_KEY` | GitHub repo **Settings → Secrets and variables → Actions** | Cheie API cu permisiune de implementare pe serverul țintă.                                                         |

<Note>
Valoarea implicită a `ZYRA_DEPLOY_URL`, `http://localhost:3000`, este un placeholder — nu va putea accesa nimic dintr-un runner găzduit de GitHub. Actualizează-l la URL-ul public al serverului tău (sau folosește un runner self-hosted cu acces la rețea) înainte de a activa CD.
</Note>
...
Ambele fluxuri de lucru fac referire la acțiuni reutilizabile la `@main`, astfel încât actualizările acțiunilor din repo-ul `zyrahq/zyra` sunt preluate automat. Dacă dorești builduri deterministe, înlocuiește `@main` cu un SHA de commit sau cu un tag de release pe fiecare linie `uses:`.

## Publicarea pe npm

Publicarea pe npm face ca aplicația ta să poată fi descoperită în marketplace-ul Zyra. Orice spațiu de lucru Zyra poate răsfoi, instala și actualiza aplicațiile din marketplace direct din interfață.

### Cerințe

* Un cont [npm](https://www.npmjs.com)
* Cuvântul cheie `zyra-app` din array-ul `keywords` al fișierului `package.json` (adaugă-l manual — nu este inclus în mod implicit în șablonul `create-zyra-app`)
...
Serverul Zyra sincronizează catalogul marketplace-ului din registrul npm **la fiecare oră**.
...
<Note>
Dacă aplicația ta nu definește un `aboutDescription` în `defineApplication()`, piața va folosi automat fișierul `README.md` al pachetului tău de pe npm drept conținut pentru pagina Despre. Acest lucru înseamnă că poți menține un singur README atât pentru npm, cât și pentru piața Zyra. Dacă vrei o descriere diferită în piață, setează explicit `aboutDescription`.
</Note>
...
Pentru alte sisteme CI (GitLab CI, CircleCI etc.), se aplică aceleași trei comenzi: `yarn install`, `yarn zyra dev:build`, apoi `npm publish` din `.zyra/output`.
...
Mergi la pagina **Setări > Aplicații** din Zyra, unde pot fi parcurse și instalate atât aplicațiile din marketplace, cât și cele implementate prin tarball.
...
Pentru a instala o versiune mai nouă, fă mai întâi deploy sau public-o, apoi rulează din nou `yarn zyra app:install`.
</Note>
