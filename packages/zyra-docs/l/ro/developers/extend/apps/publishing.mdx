...
* **Implementați o arhivă tar** — încărcați aplicația direct pe un server Zyra anume pentru uz intern sau privat.
...
Aceasta compilează sursele TypeScript, transpilează funcțiile de logică și componentele de front-end și scrie totul în `.zyra/output/`. Adaugă `--tarball` pentru a produce și un pachet `.tgz` pentru distribuire manuală sau pentru comanda de deploy.
...
2. Rulează `yarn zyra deploy` (sau `yarn zyra deploy --remote production`)
...
2. Pornește o instanță de test Zyra izolată folosind acțiunea compozită `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (echivalentul din CI al `yarn zyra server start --test`).
...
2. Rulează `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — echivalentul din CI al `yarn zyra deploy`.
...
Pentru alte sisteme CI (GitLab CI, CircleCI etc.), se aplică aceleași trei comenzi: `yarn install`, `yarn zyra build`, apoi `npm publish` din `.zyra/output`.
...
Pentru a instala o versiune mai nouă, fă mai întâi deploy sau public-o, apoi rulează din nou `yarn zyra install`.
</Note>
