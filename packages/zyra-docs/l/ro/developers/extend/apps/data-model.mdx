...
* Tokenul de runtime injectat ca `ZYRA_APP_ACCESS_TOKEN` este derivat din acest rol.
...
* Vedeți un exemplu funcțional: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Puteți genera obiecte noi folosind `yarn zyra add`, care vă ghidează prin denumire, câmpuri și relații.

<Note>
**Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, Zyra adaugă automat câmpuri standard
...
Utilizați `defineField()` pentru a adăuga câmpuri la obiecte pe care nu le dețineți — cum ar fi obiectele standard Zyra (Person, Company etc.). sau obiecte din alte aplicații. Spre deosebire de câmpurile inline din `defineObject()`, câmpurile independente necesită un `objectUniversalIdentifier` pentru a specifica obiectul pe care îl extind:
...
Puncte cheie:
* `objectUniversalIdentifier` identifică obiectul țintă. Pentru obiectele standard, utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportați din `zyra-sdk`.
...
Relațiile conectează obiectele între ele. În Zyra, relațiile sunt întotdeauna bidirecționale — definiți ambele părți, iar fiecare parte o referențiază pe cealaltă.
...
Pentru a crea o relație cu un obiect Zyra încorporat (Person, Company etc.), utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Generarea scheletului entităților cu `yarn zyra add`
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Obiect                       | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Câmp                         | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Funcție logică               | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componentă frontend          | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                          | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Abilitate                    | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Agent                        | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Vizualizare                  | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Element de meniu de navigare | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Machetă de pagină            | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Ce generează scaffolder-ul

Fiecare tip de entitate are propriul său șablon. De exemplu, `yarn zyra add object` solicită:
