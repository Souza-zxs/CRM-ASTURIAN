...
```bash
npx nx run asturian-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run asturian-front:test # jest testlerini çalıştır
npx nx run asturian-front:storybook:serve:dev # storybook'u çalıştır
npx nx run asturian-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run asturian-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
