...
```bash
npx nx run zyra-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run zyra-front:test # jest testlerini çalıştır
npx nx run zyra-front:storybook:serve:dev # storybook'u çalıştır
npx nx run zyra-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run zyra-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
