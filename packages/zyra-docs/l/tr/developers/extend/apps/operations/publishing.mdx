...
2. `./.github/actions/spawn-zyra-app-dev-test` bileşik eylemini kullanarak yalıtılmış bir Zyra test örneği başlatır (CI'daki `yarn zyra docker:start --test` eşdeğeridir).
...
2. `./.github/actions/deploy-zyra-app` çalıştırır — `yarn zyra app:publish --private` komutunun CI eşdeğeridir.
3. `./.github/actions/install-zyra-app` eylemini çalıştırır; böylece yeni dağıtılan sürüm hedef çalışma alanına kurulur.
...
Her iki iş akışı da `@main` üzerindeki yeniden kullanılabilir eylemlere başvurur; bu nedenle eylem güncellemeleri otomatik olarak alınır. Deterministik derlemeler istiyorsanız, her `uses:` satırında `@main` ifadesini bir commit SHA'sı veya sürüm etiketiyle değiştirin.
