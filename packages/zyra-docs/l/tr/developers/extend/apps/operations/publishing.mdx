...
title: Yayımlama
icon: yükle
description: Zyra uygulamanızı pazaryerine sunun ya da dahili olarak dağıtın.
---
...
* **Bir tar arşivi dağıtın** — uygulamanızı dahili veya özel kullanım için doğrudan belirli bir Zyra sunucusuna yükleyin.
* **npm’ye yayımlama** — uygulamanızı Zyra pazaryerinde listeleyin; böylece herhangi bir çalışma alanı keşfedip yükleyebilir.
...
Bu işlem TypeScript kaynaklarını derler, mantık işlevlerini ve ön uç bileşenlerini transpile eder ve her şeyi `.zyra/output/` konumuna yazar. El ile dağıtım veya yayımlama komutu için bir `.tgz` paketini de üretmek amacıyla `--tarball` ekleyin.

## Sunucuya dağıtım (tarball)

Genel kullanıma açık olmasını istemediğiniz uygulamalar — sahipli araçlar, yalnızca kurumsal entegrasyonlar veya deneysel derlemeler — için bir tarball’ı doğrudan bir Zyra sunucusuna dağıtabilirsiniz.

### Ön Gereksinimler

Dağıtmadan önce, hedef sunucuyu işaret eden yapılandırılmış bir remote’a ihtiyacınız vardır. Remote’lar sunucu URL’sini ve kimlik doğrulama bilgilerini yerel olarak `~/.zyra/config.json` içinde saklar.
...
2. `yarn zyra app:publish --private` (veya `yarn zyra app:publish --private --remote production`) komutunu çalıştırın
...
Uygulamanız belirli bir Zyra sunucu sürümünde sunulan bir özelliği kullanıyorsa (örneğin, v2.3.0'da eklenen OAuth sağlayıcıları), uygulamanızın gerektirdiği en düşük sunucu sürümünü `package.json` içindeki `engines.zyra` alanını kullanarak belirtmelisiniz:
...
* `engines.zyra` ayarlanmışsa ve hedef sunucunun sürümü aralığı karşılamıyorsa, dağıtım (tarball yüklemesi) veya yükleme, gerekli aralığı ve gerçek sunucu sürümünü belirten bir mesajla birlikte `SERVER_VERSION_INCOMPATIBLE` hatasıyla reddedilir.
* `engines.zyra` **ayarlı değilse**, uygulama herhangi bir sunucu sürümünde kabul edilir (mevcut uygulamalarla geriye dönük uyumludur).
* Sunucuda `APP_VERSION` yapılandırılmamışsa, denetim atlanır.

<Note>
Nihai denetim sunucudadır — hem tarball yüklemesinde hem de çalışma alanı (workspace) kurulumunda `engines.zyra`'yi doğrular. Bir tarball'ı bant dışı dağıtırsanız veya marketplace'ten kurarsanız, sunucu yine de uyumluluğu zorunlu kılar.
</Note>

## Otomatik CI/CD (hazır şablonlu iş akışları)

`create-zyra-app` ile oluşturulan uygulamalar, kutudan çıktığı gibi `.github/workflows/` altında iki GitHub Actions iş akışıyla gelir. Depoyu GitHub’a iter itmez çalışmaya hazırdır — CI için ek bir kurulum gerekmez ve CD yalnızca tek bir gizli anahtar gerektirir.
...
2. `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` bileşik eylemini kullanarak yalıtılmış bir Zyra test örneği başlatır (CI'daki `yarn zyra docker:start --test` eşdeğeridir).
3. Corepack’i etkinleştirir, `.nvmrc` dosyanızdan Node.js'i kurar ve bağımlılıkları `yarn install --immutable` ile yükler.
4. Oluşturulan örnekten `ZYRA_API_URL` ve `ZYRA_API_KEY` değerlerini aktararak `yarn test`i çalıştırır; böylece testleriniz gerçek bir sunucuyla haberleşebilir.

**Yapılandırma seçenekleri:**

* `ZYRA_VERSION` (ortam, varsayılanı `latest`) — CI’da kullanılan Zyra sunucu sürümünü `ci.yml` içinde bunu düzenleyerek sabitleyin.
...
`main` dalına yapılan her itmede uygulamanızı yapılandırılmış bir Zyra sunucusuna dağıtır ve isteğe bağlı olarak `deploy` etiketi uygulandığında bir çekme isteğinden de dağıtım yapar.

**Ne yapar:**

1. Etiketli PR'ler için PR'in head commit'ini ya da itilen commit'i alır.
2. `zyrahq/zyra/.github/actions/deploy-zyra-app@main` çalıştırır — `yarn zyra app:publish --private` komutunun CI eşdeğeridir.
3. `zyrahq/zyra/.github/actions/install-zyra-app@main` eylemini çalıştırır; böylece yeni dağıtılan sürüm hedef çalışma alanına kurulur.

**Gerekli yapılandırma:**

| Ayar                    | Nerede                                                       | Amaç                                                                                                |
| ----------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `ZYRA_DEPLOY_URL`     | `cd.yml` içinde `env` (varsayılan: `http://localhost:3000`)  | Dağıtımın yapılacağı Zyra sunucusu. İlk kullanımdan önce bunu gerçek sunucu URL'nizle değiştirin. |
| `ZYRA_DEPLOY_API_KEY` | GitHub deposu **Settings → Secrets and variables → Actions** | Hedef sunucuda dağıtım iznine sahip API anahtarı.                                                   |

<Note>
Varsayılan `ZYRA_DEPLOY_URL` olan `http://localhost:3000` bir yer tutucudur — GitHub barındırmalı bir çalıştırıcıdan hiçbir yere erişemez. CD'yi etkinleştirmeden önce bunu sunucunuzun genel URL'siyle güncelleyin (veya ağ erişimi olan öz barındırılan bir çalıştırıcı kullanın).
</Note>
...
Her iki iş akışı da `@main` üzerindeki yeniden kullanılabilir eylemlere başvurur; bu nedenle `zyrahq/zyra` deposundaki eylem güncellemeleri otomatik olarak alınır. Deterministik derlemeler istiyorsanız, her `uses:` satırında `@main` ifadesini bir commit SHA'sı veya sürüm etiketiyle değiştirin.

## npm’ye yayımlama

npm’ye yayımlamak, uygulamanızın Zyra pazaryerinde keşfedilebilir olmasını sağlar. Herhangi bir Zyra çalışma alanı, pazaryeri uygulamalarına doğrudan arayüzden göz atabilir, yükleyebilir ve güncelleyebilir.

### Gereksinimler

* Bir [npm](https://www.npmjs.com) hesabı
* `package.json` içindeki `keywords` dizinizdeki `zyra-app` anahtar sözcüğü (elle ekleyin — varsayılan olarak `create-zyra-app` şablonunda yer almaz)
...
Zyra sunucusu pazar yeri kataloğunu npm kayıt defterinden **her saat** eşitler.
...
Diğer CI sistemleri (GitLab CI, CircleCI, vb.) için de aynı üç komut geçerlidir: `yarn install`, `yarn zyra dev:build` ve ardından `.zyra/output` dizininden `npm publish`.
...
Zyra içinde **Ayarlar > Uygulamalar** sayfasına gidin; burada hem pazar yerindeki hem de tarball ile dağıtılmış uygulamalar görüntülenip yüklenebilir.
...
Daha yeni bir sürümü kurmak için önce onu dağıtın veya yayımlayın, ardından `yarn zyra app:install` komutunu yeniden çalıştırın.
</Note>
