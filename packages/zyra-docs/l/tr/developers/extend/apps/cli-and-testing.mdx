...
* **Derlemelere dahil edilir** — `yarn zyra build`, tüm genel varlıkları dağıtım çıktısına paketler.

### `getPublicAssetUrl` ile genel varlıklara erişme

`zyra-sdk` içindeki `getPublicAssetUrl` yardımcı işlevini kullanarak `public/` dizininizdeki bir dosyanın tam URL'sini alın. Hem **mantık işlevlerinde** hem de **ön uç bileşenlerinde** çalışır.
...
Her iki ortamda da `zyra-client-sdk/core` ve `zyra-client-sdk/metadata` önceden sağlanmış modüller olarak mevcuttur — bunlar paketlenmez, ancak çalışma zamanında sunucu tarafından çözülür.

## Uygulamanızı test etme

SDK, test kodundan uygulamanızı derlemenize, dağıtmanıza, yüklemenize ve kaldırmanıza olanak tanıyan programatik API'ler sağlar. Tiplenmiş API istemcileriyle birlikte [Vitest](https://vitest.dev/) kullanarak, uygulamanızın gerçek bir Zyra sunucusunda uçtan uca çalıştığını doğrulayan entegrasyon testleri yazabilirsiniz.
...
`zyra-sdk/cli` alt yolu, test kodundan doğrudan çağırabileceğiniz fonksiyonları dışa aktarır:
...
Yerel Zyra sunucunuzun çalıştığından emin olun, ardından:
...
### Fonksiyonları çalıştırma (`yarn zyra exec`)
...
### Fonksiyon günlüklerini görüntüleme (`yarn zyra logs`)
...
<Note>
Bu, Docker konteyner günlüklerini gösteren `yarn zyra server logs` komutundan farklıdır. `yarn zyra logs`, uygulamanızın fonksiyon yürütme günlüklerini Zyra sunucusundan gösterir.
</Note>

### Bir uygulamayı kaldırma (`yarn zyra uninstall`)
...
2. `zyrahq/zyra/.github/actions/spawn-zyra-docker-image` eylemini kullanarak geçici bir Zyra sunucusu başlatır
3. `yarn install --immutable` ile bağımlılıkları kurar
4. Eylem çıktılarından enjekte edilen `ZYRA_API_URL` ve `ZYRA_API_KEY` ile `yarn test` çalıştırır
...
Herhangi bir gizli değişken yapılandırmanız gerekmez — `spawn-zyra-docker-image` eylemi, koşucu içinde doğrudan geçici bir Zyra sunucusu başlatır ve bağlantı ayrıntılarını çıktı olarak verir. `GITHUB_TOKEN` gizli değişkeni GitHub tarafından otomatik olarak sağlanır.

`latest` yerine belirli bir Zyra sürümünü sabitlemek için iş akışının başındaki `ZYRA_VERSION` ortam değişkenini değiştirin.
