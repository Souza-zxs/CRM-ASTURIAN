---
title: Veri modeli
description: Zyra SDK ile nesneleri, alanları, rolleri ve uygulama meta verilerini tanımlayın.
icon: database
---

`zyra-sdk` paketi, uygulamanızın veri modelini tanımlamak için `defineEntity` işlevlerini sağlar. SDK'nin varlıklarınızı algılayabilmesi için `export default defineEntity({...})` kullanmanız gerekir. Bu fonksiyonlar, derleme zamanında yapılandırmanızı doğrular ve IDE otomatik tamamlama ile tür güvenliği sağlar.
...
* `ZYRA_APP_ACCESS_TOKEN` olarak enjekte edilen çalışma zamanı belirteci bu rolden türetilir.
...
* Çalışan bir örnek için bkz.: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* `yarn zyra add` kullanarak, adlandırma, alanlar ve ilişkiler konusunda sizi yönlendirerek yeni nesneler oluşturabilirsiniz.

<Note>
**Temel alanlar otomatik olarak oluşturulur.** Özel bir nesne tanımladığınızda Zyra, standart alanları otomatik olarak ekler
...
Önemli noktalar:
* `objectUniversalIdentifier` hedef nesneyi tanımlar. Standart nesneler için, `zyra-sdk`'den dışa aktarılan `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`'ı kullanın.
...
İlişkiler nesneleri birbirine bağlar. Zyra'de ilişkiler her zaman **çift yönlüdür** — her iki tarafı da tanımlarsınız ve her taraf diğerine başvurur.
...
Yerleşik bir Zyra nesnesiyle (Person, Company, vb.) ilişki oluşturmak için `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` kullanın:
...
## `yarn zyra add` ile varlıklar için iskelet oluşturma
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Nesne                | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Alan                 | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Mantık işlevi        | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Ön uç bileşeni       | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                  | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Beceri               | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Temsilci             | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Görünüm              | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Gezinme menüsü öğesi | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Sayfa düzeni         | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### İskelet oluşturucunun ürettikleri

Her varlık türünün kendi şablonu vardır. Örneğin, `yarn zyra add object` şunları sorar:
