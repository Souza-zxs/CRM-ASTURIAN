...
description: Nesne, alan, görünüm, mantık işlevi ve daha fazlasına ait varlık dosyalarını `yarn zyra dev:add` komutuyla etkileşimli olarak oluşturun.
...
| -------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Nesne                | `yarn zyra dev:add object`             | `src/objects/\<name>.ts`               |
| Alan                 | `yarn zyra dev:add field`              | `src/fields/\<name>.ts`                |
| Mantık işlevi        | `yarn zyra dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Ön uç bileşeni       | `yarn zyra dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                  | `yarn zyra dev:add role`               | `src/roles/\<name>.ts`                 |
| Beceri               | `yarn zyra dev:add skill`              | `src/skills/\<name>.ts`                |
| Temsilci             | `yarn zyra dev:add agent`              | `src/agents/\<name>.ts`                |
| Görünüm              | `yarn zyra dev:add view`               | `src/views/\<name>.ts`                 |
| Gezinme menüsü öğesi | `yarn zyra dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Sayfa düzeni         | `yarn zyra dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## İskelet oluşturucunun ürettikleri

Her varlık türünün kendi şablonu vardır. Örneğin, `yarn zyra dev:add object` şunları sorar:
