...
* **Geliştirme modunda çalıştırılmaz**: kurulum sonrasında olduğu gibi — yerel olarak kaydedilen uygulamalarda kurulum akışı tamamen atlanır, bu nedenle `yarn zyra dev` altında kurulum öncesi hiç çalışmaz. Bunu elle tetiklemek için `yarn zyra dev:function:exec --preInstall` kullanın.
