---
title: Zyra Uygulamaları
description: Zyra özelleştirmelerini kod olarak oluşturun ve yönetin.
---
...
* **Ön yüz bileşenleri** — Zyra'nin kullanıcı arayüzünde (kayıt sayfaları, komut menüsü, yan paneller) görüntülenen React bileşenleri
* **Yapay zekâ yetenekleri ve ajanları** — Zyra'nin yapay zekâsını özel yeteneklerle genişletin
...
Bu, yeni bir uygulamanın iskeletini oluşturur, isteğe bağlı olarak yerel bir Zyra sunucusunu başlatır ve dosyalarınızdaki değişiklikleri izlemeye başlar. Tam adım adım anlatım için [Başlarken](/l/tr/developers/extend/apps/getting-started) kılavuzuna bakın.
...
| `defineFrontComponent()`           | Zyra'nin kullanıcı arayüzündeki React bileşenleri        |
...
1. **`yarn zyra dev`** — kaynak dosyaları izler, değişiklikte yeniden derler, sunucuyla senkronize eder, tipli API istemcileri üretir
2. **`yarn zyra build`** — dağıtılabilir bir derleme üretir
3. **`yarn zyra deploy`** — uzak bir Zyra sunucusuna dağıtır
4. **`yarn zyra add`** — etkileşimli olarak yeni bir varlık iskeleti oluşturur

### CLI başvurusu

```bash filename="Terminal"
yarn zyra help          # Tüm komutları listele
yarn zyra server start  # Yerel geliştirme sunucusunu başlat
yarn zyra remote add    # Bir Zyra sunucusuna bağlan
yarn zyra exec -n fn    # Bir mantık fonksiyonunu çalıştır
yarn zyra logs -n fn    # Fonksiyon günlüklerini izle
```
