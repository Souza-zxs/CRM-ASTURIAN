...
description: Crie componentes React que renderizam dentro da UI do Zyra com isolamento em sandbox.
icon: window-maximize
---

Componentes de front-end são componentes React que renderizam diretamente dentro da UI do Zyra. Eles são executados em um Web Worker isolado usando Remote DOM — seu código é sandboxed, mas renderiza nativamente na página, não em um iframe.

## Onde os componentes de front-end podem ser usados

Os componentes de front-end podem ser renderizados em dois locais dentro do Zyra:
...
Após sincronizar com `yarn zyra dev` (ou executando uma única vez o `yarn zyra dev --once`), a ação rápida aparece no canto superior direito da página:
...
Como o componente retorna `null`, o Zyra ignora renderizar um contêiner para ele — nenhum espaço vazio aparece no layout. O componente ainda tem acesso a todos os hooks e à API de comunicação do host.

## Componentes Command do SDK

O pacote `zyra-sdk` fornece quatro componentes auxiliares Command projetados para componentes de front-end headless. Cada componente executa uma ação ao montar, trata erros exibindo uma notificação de snackbar e desmonta automaticamente o componente de front-end ao concluir.

Importe-os de `zyra-sdk/command`:
...
Uma função lógica declarada com `httpRouteTriggerSettings` é exposta sob o endpoint `/s/` em `${ZYRA_API_URL}/s\<path>`. Seu componente de front chama essa rota com o `RestApiClient` de `zyra-client-sdk/rest`, que autentica com o `ZYRA_APP_ACCESS_TOKEN` que a Zyra injeta no worker.

O `RestApiClient` foi criado exatamente para isso. Ele lê `ZYRA_API_URL` e `ZYRA_APP_ACCESS_TOKEN` do ambiente do worker, adiciona o cabeçalho `Authorization: Bearer`, serializa e analisa JSON e lança um `RestApiClientError` quando o token ou a URL estão ausentes ou a resposta não é 2xx — para que você não precise reimplementar esse boilerplate em todos os componentes.
...
O caminho passado para o cliente é o caminho público da rota — o `httpRouteTriggerSettings.path` da função de lógica, prefixado com `/s`. Mantenha `isAuthRequired: true`; o cliente fornece o token de acesso do app que o Zyra emite para o seu componente:
...
<Note>
`ZYRA_API_URL` e `ZYRA_APP_ACCESS_TOKEN` são injetados automaticamente — consulte [Variáveis de aplicação](#application-variables). Como as variáveis de aplicação secretas nunca são expostas aos componentes de front, mantenha as chaves de API e outra lógica sensível na função lógica, não no componente de front.
</Note>

### Referência do `RestApiClient`

Importe `RestApiClient` de `zyra-client-sdk/rest`. Ele pertence à mesma família de clientes que `CoreApiClient` e `MetadataApiClient`, mas tem como alvo as rotas HTTP do seu app em vez da API GraphQL.
...
| Variável                  | Descrição                                                     |
| ------------------------- | ------------------------------------------------------------- |
| `ZYRA_API_URL`          | URL base da API da Zyra                                     |
| `ZYRA_APP_ACCESS_TOKEN` | Token de curta duração limitado ao escopo do papel do seu app |

## API de comunicação do host

Componentes de front-end podem acionar navegação, modais e notificações usando funções de `zyra-sdk`:
...
* **Componentes de UI do Zyra** — importe de `zyra-sdk/ui` (Button, Tag, Status, Chip, Avatar e mais)
