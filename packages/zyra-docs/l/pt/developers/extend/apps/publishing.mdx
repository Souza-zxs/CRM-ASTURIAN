...
Isso compila seu código-fonte em TypeScript, transpila funções de lógica e componentes de front-end e grava tudo em `.zyra/output/`. Adicione `--tarball` para também gerar um pacote `.tgz` para distribuição manual ou para o comando de deploy.
...
2. Execute `yarn zyra deploy` (ou `yarn zyra deploy --remote production`)
...
<Note>
O servidor realiza a verificação definitiva — ele valida `engines.zyra` tanto no upload do tarball quanto na instalação no workspace. Se você implantar um tarball fora de banda ou instalar a partir do marketplace, o servidor ainda impõe a compatibilidade.
</Note>
...
2. Inicia uma instância de teste do Zyra isolada usando a ação composta `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (o equivalente em CI de `yarn zyra server start --test`).
...
2. Executa `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — o equivalente em CI de `yarn zyra deploy`.
3. Executa `zyrahq/zyra/.github/actions/install-zyra-app@main` para que a versão recém-implantada seja instalada no workspace de destino.
...
Para outros sistemas de CI (GitLab CI, CircleCI etc.), aplicam-se os mesmos três comandos: `yarn install`, `yarn zyra build` e, em seguida, `npm publish` a partir de `.zyra/output`.
...
Para instalar uma versão mais recente, implante ou publique-a primeiro e, em seguida, execute novamente `yarn zyra install`.
</Note>
