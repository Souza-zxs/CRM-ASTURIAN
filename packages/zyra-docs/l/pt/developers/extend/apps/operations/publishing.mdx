...
2. Inicia uma instância de teste do Zyra isolada usando a ação composta `./.github/actions/spawn-zyra-app-dev-test` (o equivalente em CI de `yarn zyra docker:start --test`).
...
2. Executa `./.github/actions/deploy-zyra-app` — o equivalente em CI de `yarn zyra app:publish --private`.
3. Executa `./.github/actions/install-zyra-app` para que a versão recém-implantada seja instalada no espaço de trabalho de destino.
...
Ambos os fluxos de trabalho fazem referência a ações reutilizáveis em `@main`, portanto as atualizações de ações são aplicadas automaticamente. Se você quiser builds determinísticos, substitua `@main` por um SHA de commit ou uma tag de release em cada linha `uses:`.
