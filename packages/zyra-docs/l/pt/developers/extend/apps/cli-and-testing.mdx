...
* **Incluídos nas compilações** — `yarn zyra build` agrupa todos os recursos públicos na saída de distribuição.

### Acessando recursos públicos com `getPublicAssetUrl`

Use o helper `getPublicAssetUrl` de `zyra-sdk` para obter a URL completa de um arquivo no seu diretório `public/`. Funciona tanto em **funções lógicas** quanto em **componentes de front-end**.
...
Ambos os ambientes têm `zyra-client-sdk/core` e `zyra-client-sdk/metadata` disponíveis como módulos pré-fornecidos — eles não são empacotados, mas resolvidos em tempo de execução pelo servidor.

## Testando seu aplicativo

O SDK fornece APIs programáticas que permitem compilar, implantar, instalar e desinstalar seu aplicativo a partir de código de teste. Em conjunto com [Vitest](https://vitest.dev/) e os clientes de API tipados, você pode escrever testes de integração que verificam que seu aplicativo funciona de ponta a ponta em um servidor Zyra real.
...
O subcaminho `zyra-sdk/cli` exporta funções que você pode chamar diretamente a partir do código de teste:
...
Certifique-se de que seu servidor Zyra local esteja em execução e, em seguida:
...
### Executando funções (`yarn zyra exec`)
...
### Visualizando logs de funções (`yarn zyra logs`)
...
<Note>
Isso é diferente de `yarn zyra server logs`, que mostra os logs do contêiner Docker. `yarn zyra logs` mostra os logs de execução de funções do seu aplicativo a partir do servidor Zyra.
</Note>

### Desinstalando um aplicativo (`yarn zyra uninstall`)
...
2. Inicializa um servidor Zyra temporário usando a ação `zyrahq/zyra/.github/actions/spawn-zyra-docker-image`
3. Instala as dependências com `yarn install --immutable`
4. Executa `yarn test` com `ZYRA_API_URL` e `ZYRA_API_KEY` injetados a partir das saídas da ação
...
Você não precisa configurar nenhum segredo — a ação `spawn-zyra-docker-image` inicia um servidor Zyra efêmero diretamente no runner e fornece os detalhes de conexão. O segredo `GITHUB_TOKEN` é fornecido automaticamente pelo GitHub.

Para fixar uma versão específica do Zyra em vez de `latest`, altere a variável de ambiente `ZYRA_VERSION` no topo do workflow.
