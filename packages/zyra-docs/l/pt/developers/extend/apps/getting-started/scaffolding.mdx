---
title: Scaffolding
description: Gere arquivos de entidade de forma interativa com `yarn zyra dev:add` — objetos, campos, visualizações, funções de lógica e mais.
...
| Função lógica             | `yarn zyra dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de front-end   | `yarn zyra dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Função                    | `yarn zyra dev:add role`               | `src/roles/\<name>.ts`                 |
| Habilidade                | `yarn zyra dev:add skill`              | `src/skills/\<name>.ts`                |
| Agente                    | `yarn zyra dev:add agent`              | `src/agents/\<name>.ts`                |
| Vista                     | `yarn zyra dev:add view`               | `src/views/\<name>.ts`                 |
| Item do menu de navegação | `yarn zyra dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout da página          | `yarn zyra dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## O que o scaffolder gera

Cada tipo de entidade tem seu próprio modelo. Por exemplo, `yarn zyra dev:add object` solicita:
