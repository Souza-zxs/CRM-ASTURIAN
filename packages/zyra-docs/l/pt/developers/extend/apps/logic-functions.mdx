...
import type { DatabaseEventPayload, ObjectRecordCreateEvent, CronPayload, RoutePayload } from 'zyra-sdk/define';
import { CoreApiClient, type Person } from 'zyra-client-sdk/core';
...
yarn zyra exec -n create-new-post-card -p '{"key": "value"}'
```

```bash filename="Terminal"
yarn zyra exec -y e56d363b-0bdc-4d8a-a393-6f0d1c75bdcf
```
...
Importe o tipo `RoutePayload` de `zyra-sdk`:
...
import { definePostInstallLogicFunction, type InstallPayload } from 'zyra-sdk/define';
...
* As variáveis de ambiente `APPLICATION_ID`, `APP_ACCESS_TOKEN` e `API_URL` estão disponíveis dentro do manipulador (assim como em qualquer outra função de lógica), então você pode chamar a API da Zyra com um token de acesso de aplicativo com escopo para o seu app.
...
* **Não executado no modo de desenvolvimento**: quando um app é registrado localmente (via `yarn zyra dev`), o servidor pula completamente o fluxo de instalação e sincroniza arquivos diretamente pelo watcher da CLI — portanto, a pós-instalação nunca é executada no modo de desenvolvimento, independentemente de `shouldRunSynchronously`. Use `yarn zyra exec --postInstall` para acioná-lo manualmente em um workspace em execução.
...
import { definePreInstallLogicFunction, type InstallPayload } from 'zyra-sdk/define';
...
* **Não é executada no modo de desenvolvimento**: igual à pós-instalação — o fluxo de instalação é totalmente ignorado para apps registrados localmente, portanto a pré-instalação nunca é executada com `yarn zyra dev`. Use `yarn zyra exec --preInstall` para acioná-lo manualmente.
...
`CoreApiClient` é o cliente principal para consultar e mutar dados do espaço de trabalho. Ele é **gerado a partir do schema do seu espaço de trabalho** durante `yarn zyra dev` ou `yarn zyra build`, então é totalmente tipado para corresponder aos seus objetos e campos.
...
<Note>
**CoreApiClient é gerado em tempo de dev/build.** Se você usá-lo sem executar primeiro `yarn zyra dev` ou `yarn zyra build`, ele lançará um erro. A geração ocorre automaticamente — a CLI analisa o schema GraphQL do seu espaço de trabalho e gera um cliente tipado usando `@genql/cli`.
</Note>
