...
description: Defina objetos, campos, papéis e metadados da aplicação com o Zyra SDK.
icon: database
---

O pacote `zyra-sdk` fornece funções `defineEntity` para declarar o modelo de dados da sua aplicação. Você deve usar `export default defineEntity({...})` para que o SDK detecte suas entidades. Essas funções validam sua configuração em tempo de compilação e oferecem autocompletar na IDE e segurança de tipos.
...
* O token em tempo de execução injetado como `ZYRA_APP_ACCESS_TOKEN` é derivado desse papel.
...
* Veja um exemplo funcional: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* Você pode criar novos objetos usando `yarn zyra add`, que orienta você sobre nomeação, campos e relacionamentos.

<Note>
**Os campos base são criados automaticamente.** Quando você define um objeto personalizado, o Zyra adiciona automaticamente campos padrão
...
Use `defineField()` para adicionar campos a objetos que não são seus — como objetos padrão do Zyra (Person, Company, etc.). ou a objetos de outros apps. Ao contrário dos campos inline em `defineObject()`, os campos independentes exigem um `objectUniversalIdentifier` para especificar qual objeto eles estendem:
...
Pontos-chave:
* `objectUniversalIdentifier` identifica o objeto de destino. Para objetos padrão, use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportado de `zyra-sdk`.
...
As relações conectam objetos entre si. No Zyra, as relações são sempre **bidirecionais** — você define ambos os lados, e cada lado faz referência ao outro.
...
Para criar uma relação com um objeto integrado do Zyra (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Gerando entidades com `yarn zyra add`
...
| ------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Objeto                    | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| Campo                     | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| Função lógica             | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de front-end   | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Papel                     | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| Habilidade                | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| Agente                    | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| Vista                     | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| Item do menu de navegação | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout da página          | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### O que o scaffolder gera

Cada tipo de entidade tem seu próprio modelo. Por exemplo, `yarn zyra add object` solicita:
