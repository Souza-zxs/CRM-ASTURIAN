...
```bash
"npx nx run asturian-front:lint # مرر --fix لإصلاح أخطاء التدقيق"
```
...
```bash
"npx nx run asturian-front:test # تشغيل اختبارات jest\nnpx nx run asturian-front:storybook:serve:dev # تشغيل storybook\nnpx nx run asturian-front:storybook:test # تشغيل الاختبارات # (يحتاج إلى تشغيل yarn storybook:serve:dev)\nnpx nx run asturian-front:storybook:coverage # (يحتاج إلى تشغيل yarn storybook:serve:dev)"
```
