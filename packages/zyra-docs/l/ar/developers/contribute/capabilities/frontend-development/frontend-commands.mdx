...
```bash
"npx nx run zyra-front:lint # مرر --fix لإصلاح أخطاء التدقيق"
```
...
```bash
"npx nx run zyra-front:test # تشغيل اختبارات jest\nnpx nx run zyra-front:storybook:serve:dev # تشغيل storybook\nnpx nx run zyra-front:storybook:test # تشغيل الاختبارات # (يحتاج إلى تشغيل yarn storybook:serve:dev)\nnpx nx run zyra-front:storybook:coverage # (يحتاج إلى تشغيل yarn storybook:serve:dev)"
```
