...
يقوم هذا بتجميع مصادر TypeScript، وتحويل دوال المنطق ومكوّنات الواجهة الأمامية، وكتابة كل شيء إلى `.zyra/output/`. أضِف `--tarball` لإنتاج حزمة `.tgz` أيضًا للتوزيع اليدوي أو لأمر deploy.
...
2. شغّل `yarn zyra deploy` (أو `yarn zyra deploy --remote production`)
...
1. يجلب مصدر تطبيقك.
2. ينشئ مثيلاً اختبارياً معزولاً من Zyra باستخدام الإجراء المركّب `zyrahq/zyra/.github/actions/spawn-zyra-app-dev-test@main` (المكافئ في CI للأمر `yarn zyra server start --test`).
...
2. يشغّل `zyrahq/zyra/.github/actions/deploy-zyra-app@main` — وهو المكافئ في CI للأمر `yarn zyra deploy`.
...
بالنسبة لأنظمة CI الأخرى (GitLab CI، وCircleCI، إلخ)، تنطبق الأوامر الثلاثة نفسها: `yarn install`، ثم `yarn zyra build`، ثم `npm publish` من `.zyra/output`.
...
لتثبيت إصدار أحدث، انشره (deploy) أو انشره إلى السجل (publish) أولًا، ثم أعد تشغيل `yarn zyra install`.
</Note>
