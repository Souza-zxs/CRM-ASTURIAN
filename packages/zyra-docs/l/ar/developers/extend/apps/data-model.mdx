...
description: عرّف الكائنات والحقول والأدوار وبيانات تعريف التطبيق باستخدام SDK الخاصة بـ Zyra.
icon: database
---

توفر حزمة `zyra-sdk` دوالّ `defineEntity` لتعريف نموذج بيانات تطبيقك. يجب عليك استخدام `export default defineEntity({...})` لكي يكتشف SDK الكيانات الخاصة بك. تتحقق هذه الدوال من تكوينك وقت البناء وتوفّر إكمالًا تلقائيًا في بيئة التطوير وأمان الأنواع.
...
* رمز وقت التشغيل المحقون باسم `ZYRA_APP_ACCESS_TOKEN` مستمد من هذا الدور.
...
* اطّلع على مثال عملي: [`hello-world/src/roles/function-role.ts`](https://github.com/zyrahq/zyra/blob/main/packages/zyra-apps/hello-world/src/roles/function-role.ts).
...
* يمكنك إنشاء كائنات جديدة باستخدام `yarn zyra add`، والذي يرشدك خلال التسمية والحقول والعلاقات.

<Note>
**يتم إنشاء الحقول الأساسية تلقائيًا.** عند تعريف كائن مخصص، يضيف Zyra تلقائيًا حقولًا قياسية
...
استخدم `defineField()` لإضافة حقول إلى كائنات لا تملكها — مثل كائنات Zyra القياسية (Person, Company, etc.) أو كائنات من تطبيقات أخرى. على خلاف الحقول المضمّنة في `defineObject()`، تتطلّب الحقول المستقلة `objectUniversalIdentifier` لتحديد الكائن الذي تقوم بتوسيعه:
...
النقاط الرئيسية:
* `objectUniversalIdentifier` يحدّد الكائن الهدف. بالنسبة للكائنات القياسية، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` المُصدَّر من `zyra-sdk`.
...
تربط العلاقات الكائنات معًا. في Zyra، تكون العلاقات دائمًا **ثنائية الاتجاه** — حيث تعرّف الجانبين، ويشير كل جانب إلى الآخر.
...
لإنشاء علاقة مع كائن Zyra مضمّن (Person, Company, etc.)، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## توليد قوالب الكيانات باستخدام `yarn zyra add`
...
| ------------------ | ------------------------------------ | ------------------------------------------------------- |
| كائن               | `yarn zyra add object`             | `src/objects/\<name>.ts`               |
| الحقل              | `yarn zyra add field`              | `src/fields/\<name>.ts`                |
| دالة منطقية        | `yarn zyra add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| مكوّن أمامي        | `yarn zyra add frontComponent`     | `src/front-components/\<name>.tsx`     |
| دور                | `yarn zyra add role`               | `src/roles/\<name>.ts`                 |
| مهارة              | `yarn zyra add skill`              | `src/skills/\<name>.ts`                |
| وكيل               | `yarn zyra add agent`              | `src/agents/\<name>.ts`                |
| عرض                | `yarn zyra add view`               | `src/views/\<name>.ts`                 |
| عنصر قائمة التنقّل | `yarn zyra add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| تخطيط الصفحة       | `yarn zyra add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### ما الذي تُنشئه أداة القوالب

لكل نوع كيان قالب خاص به. على سبيل المثال، يسأل `yarn zyra add object` عن:
