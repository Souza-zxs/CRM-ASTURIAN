...
const LOG_PREFIX = '[Zyra]';

let configLogged = false;

function getApiUrl() {
  return process.env.ZYRA_API_URL;
}

function getWorkspaceSubdomain() {
  return process.env.ZYRA_WORKSPACE_SUBDOMAIN;
}

function getApiKey() {
  return process.env.ZYRA_API_KEY;
}
...
function isConfigured() {
  ...
  if (!configLogged) {
    ...
    if (!configured) {
      console.log(
        `${LOG_PREFIX} Integration disabled — ZYRA_API_URL or ZYRA_API_KEY not set`,
      );
    } else {
      console.log(
