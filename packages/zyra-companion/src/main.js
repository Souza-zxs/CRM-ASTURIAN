...
const zyraClient = require('./zyra-client');
require('dotenv').config();

const zyraIconDataUrl = require('./assets/zyra-logo-256.png');

function getAppIcon() {
  try {
    return nativeImage.createFromDataURL(zyraIconDataUrl);
  } catch (error) {
...
// Check Zyra CRM integration status at startup
zyraClient.isConfigured();
...
app.whenReady().then(() => {
  if (process.platform === 'darwin' && app.dock) {
    ...
    if (dockIcon && !dockIcon.isEmpty()) {
      ...
      console.log('Zyra dock icon set successfully');
    } else {
...
// Poll Recall for the audio URL then call the Zyra end-recording logic function.
...
async function endCallRecordingWithRetry(windowId, maxAttempts = 10, delayMs = 5000) {
  try {
    ...
    if (!meeting?.zyraRecordId || !meeting?.recallRecordingId) {
      console.log('[Zyra] No Zyra record or Recall recording ID found, skipping end-recording');
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (audioUrl) {
          await zyraClient.endCallRecording({
            callRecordingId: meeting.zyraRecordId,
            audioUrl,
            transcriptUrl,
        ...
        console.error(`[Zyra] Attempt ${attempt}/${maxAttempts} — error fetching recording:`, error.message);
      }

      if (attempt < maxAttempts) {
        console.log(`[Zyra] Audio URL not ready, retrying in ${delayMs / 1000}s (${attempt}/${maxAttempts})`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }

    console.error('[Zyra] Audio URL never became available, giving up');
  } catch (error) {
    console.error('Failed to end callRecording in Zyra:', error.message);
  }
}
...
async function createMeetingNoteAndRecord(platformName) {
  ...
  try {
    ...
    // Create a call recording record in Zyra CRM
    if (zyraClient.isConfigured()) {
      try {
        const zyraRecord = await zyraClient.createCallRecording(meetingTitle);

        if (zyraRecord?.id) {
          newMeeting.zyraRecordId = zyraRecord.id;

          if (global.activeMeetingIds && global.activeMeetingIds[detectedMeeting.window.id]) {
            global.activeMeetingIds[detectedMeeting.window.id].zyraRecordId = zyraRecord.id;
          }

          console.log('Created callRecording in Zyra:', zyraRecord.id);
        }
      } catch (error) {
        console.error('Failed to create callRecording in Zyra:', error.message);
      }
    }
