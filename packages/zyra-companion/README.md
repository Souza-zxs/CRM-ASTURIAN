...
### Zyra CRM Integration (optional)

To automatically create `callRecording` records in Zyra when a meeting starts (and mark them as ended when the meeting closes), configure:

```
ZYRA_API_URL=http://localhost:3000
ZYRA_API_KEY=<your key>
```

The `call-recording` Zyra app must be installed in your workspace first (`packages/zyra-apps/internal/call-recording`). Generate an API key at `<your-zyra-instance>/settings/api-webhooks`.

To launch the Zyra Desktop application, start the server first, then the app:
