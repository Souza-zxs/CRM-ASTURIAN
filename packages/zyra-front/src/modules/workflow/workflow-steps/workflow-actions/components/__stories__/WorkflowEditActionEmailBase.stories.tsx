...
import { ComponentDecorator, RouterDecorator } from 'zyra-ui/testing';
...
const CONFIGURED_SEND_EMAIL_ACTION: WorkflowSendEmailAction = {
  ...
  settings: {
    input: {
      ...
      body: 'Dear Tim,\n\nWelcome to Zyra! We are excited to have you on board.\n\nBest regards,\nThe Team',
      files: [],
      inReplyTo: '',
    },
    outputSchema: {},
...
export const Configured: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const emailInput = await canvas.findByText('test@zyra.com');
    expect(emailInput).toBeVisible();

    const subjectInput = await canvas.findByText('Welcome to Zyra!');
