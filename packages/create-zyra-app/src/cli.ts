...
const program = new Command(packageJson.name)
  .description('CLI tool to initialize a new Zyra application')
  .version(
    packageJson.version,
    '-v, --version',
    'Output the current version of create-zyra-app.',
  )
  .argument('[directory]')
  .option('-n, --name <name>', 'Application name')
  .option('-d, --display-name <displayName>', 'Application display name')
  .option('--description <description>', 'Application description')
  .option('--url <url>', 'Zyra server URL (default: http://localhost:2020)')
