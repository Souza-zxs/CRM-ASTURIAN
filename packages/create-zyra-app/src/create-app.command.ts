...
import { isDefined, normalizeUrl } from 'zyra-shared/utils';
import {
...
const IMAGE = 'zyracrm/zyra-app-dev:latest';
...
export class CreateAppCommand {
  ...
  async execute(options: CreateAppOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (!skipLocalInstance) {
        this.logNextStep('Starting Zyra server');
      ...
      if (serverReady && authSucceeded) {
        ...
        if (!syncSucceeded) {
          this.logDetail('Sync failed. Run `yarn zyra dev --once` manually.');
          return;
        }
      } else {
  ...
  private async ensureDockerServer(
    ...
    this.logDetail('Ensuring latest Zyra server image...');
  ...
  private async openMainPage(
    ...
    try {
      ...
      const token = config.zyraCLIAccessToken ?? config.apiKey;
  ...
  private async syncApplication(appDirectory: string): Promise<boolean> {
    this.logDetail('Running `yarn zyra dev --once`...');
    return new Promise((resolve) => {
      const child = spawn('yarn', ['zyra', 'dev', '--once'], {
        cwd: appDirectory,
  ...
  private async tryExistingAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      for (const remoteName of remoteNames) {
        ...
        const token = remoteConfig.zyraCLIAccessToken ?? remoteConfig.apiKey;
  ...
  private async authenticateWithDevKey(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          '  Authentication failed. Run `yarn zyra remote:add` manually.',
        ),
      );
  ...
  private async authenticateWithOAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          `  OAuth failed: ${result.error.message}\n` +
            `  Run \`yarn zyra remote:add --url ${serverUrl}\` manually.`,
        ),
      );
      ...
      console.log(
        chalk.yellow(
          `  Authentication failed. Run \`yarn zyra remote:add --url ${serverUrl}\` manually.`,
        ),
      );
  ...
  private logSuccess(
    ...
    if (!authSucceeded) {
      console.log(chalk.white(`  ${stepNumber}. Connect to a Zyra instance`));
      console.log(
        chalk.cyan('     yarn zyra remote:add --url <your-instance-url>\n'),
      );
      stepNumber++;
    }

    console.log(chalk.white(`  ${stepNumber}. Start developing`));
    console.log(chalk.cyan('     yarn zyra dev\n'));
    stepNumber++;

    console.log(chalk.white(`  ${stepNumber}. Open your zyra instance`));
    console.log(chalk.cyan(`     ${serverUrl}\n`));

    console.log(
      chalk.gray(
        '  Documentation: https://docs.zyra.com/developers/extend/capabilities/apps',
      ),
    );
  }
}
