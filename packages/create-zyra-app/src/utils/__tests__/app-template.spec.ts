...
import createZyraAppPackageJson from 'package.json';
...
describe('copyBaseApplicationProject', () => {
  ...
  beforeEach(async () => {
    testAppDirectory = join(
      tmpdir(),
      `test-zyra-app-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('should update package.json with app name and SDK versions', async () => {
    ...
    expect(packageJson.devDependencies['zyra-sdk']).toBe(
      createZyraAppPackageJson.version,
    );
    expect(packageJson.devDependencies['zyra-client-sdk']).toBe(
      createZyraAppPackageJson.version,
    );
  });
