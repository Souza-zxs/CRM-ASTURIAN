...
`zyra-ui` is Zyra's UI library, the successor to the legacy `zyra-ui-deprecated` package
...
[`@base-ui/react`](https://base-ui.com), MIT) as the behavioral foundation; build Zyra's visual
design on top of it.
...
- **Modern, broad primitives** — Combobox/Autocomplete with built-in search, Select, Number Field, Navigation Menu, Toast, etc., several of which replace hand-rolled or single-purpose dependencies in `zyra-ui`.
...
```
packages/zyra-ui/
├── package.json            # public exports mirror zyra-ui's subpath map
...
`zyra-ui-deprecated` so the final swap is a codemod + dependency rename, not a rewrite of ~1,730 files.
...
**Internal changes vs `zyra-ui-deprecated`:** Linaria → SCSS Modules; hand-rolled behavior + `react-tooltip`
...
- Reuse `zyra-ui-deprecated`'s token values verbatim to guarantee identical design.
- Tokens live in `src/theme/` (`THEME_LIGHT` / `THEME_DARK`); the `--t-*` CSS variables and the `themeCssVariables` accessor are static files mirrored token-for-token from `zyra-ui-deprecated` (matching its own static-CSS approach).
- A theme parity test asserts the theme CSS and `themeCssVariables` stay identical to `zyra-ui-deprecated`'s `--t-*` values.

### Consuming the theme

- Wrap the app once in `ThemeProvider` and import `zyra-ui/theme-light.css` and `zyra-ui/theme-dark.css`.
...
and state-decoupling notes) is the remaining **Phase 4 prerequisite** (the `zyra-ui-deprecated` inventory itself is complete — every component is migrated).
...
### In `zyra-ui-deprecated` *(all migrated — predictions held; see Migration outcomes)*
...
- **Visual parity** — visual regression via Argos (self-hosted) plus a cross-package comparison project that diffs `zyra-ui` stories against `zyra-ui-deprecated` stories with identical names; a pixel-diff threshold is the per-component acceptance gate. See [Visual regression](#visual-regression) below.
- **Performance & size** — `size-limit` per entry point with budgets; tree-shaking fixtures (importing one component must not pull the library); build-time tracking; render benchmarks via React Profiler; load-time via Lighthouse/Playwright on the built Storybook. As one concrete benchmark, a dedicated **stress story** renders a very large number of a single component (e.g. 10,000 buttons) and measures total render time — compared against the `zyra-ui-deprecated` equivalent and gated against a budget to catch per-instance overhead regressions. *(Stress story not yet built.)*

CI surfaces a per-PR diff table (`zyra-ui-deprecated` vs `zyra-ui`) for size, a11y, and visual changes. *(Not yet built.)*

## Visual regression

Two Argos projects (on argos.zyra-internal.com) provide visual regression in CI:

1. **`zyra-ui`** — pixel diff of `zyra-ui` stories against the `main` branch baseline. Catches regressions introduced by a PR.
2. **`zyra-ui-vs-new-ui`** — cross-package comparison. The baseline is always `zyra-ui-deprecated` screenshots from `main`; PR builds upload `zyra-ui` screenshots and diff them against that baseline. This shows exactly which components still differ between the two implementations.

For the cross-package comparison to produce meaningful diffs, stories in `zyra-ui` must use the **same title hierarchy** as `zyra-ui-deprecated` (e.g. `UI/Input/Toggle/Toggle`). *(Done: all 70 story titles are byte-identical.)*

### Local visual diff

Run a pixel diff of `zyra-ui` components against `zyra-ui-deprecated` using the self-hosted Argos instance.

**Prerequisites:**
- AWS SSO configured and logged in (`aws sso login --profile zyra-dev`)
- `zyra-infra/super-cli` cloned (sibling of this repo)

**1. Start the Argos tunnel**

In the `zyra-infra/super-cli` directory:
...
Create a `.env` file in `packages/zyra-ui/` (gitignored):
...
From the repo root:
    ...
    npx nx storybook:visual-diff zyra-ui

This builds Storybook, captures screenshots of every story, and uploads
them to Argos with build name `<username>/zyra-ui`. The diff
compares against the latest approved baseline.

To run `zyra-ui-deprecated`'s visual diff in the same Argos instance (to build the
cross-package comparison baseline):

    npx nx storybook:visual-diff zyra-ui-deprecated
...
- Public package (remove `private`); ships as `zyra-ui` (the old package already moved to the `zyra-ui-deprecated` name and is removed after cut-over).
...
1. ~~Published package name.~~ **Resolved:** the rename already happened — this package owns `zyra-ui`; the old one is `zyra-ui-deprecated`.
